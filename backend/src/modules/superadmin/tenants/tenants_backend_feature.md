# Tenants Module - Backend Feature Guide

## 1. Overview
The **Tenants** module is responsible for managing tenants-related operations within the Smart Gym Management system. It exposes RESTful APIs for creating, reading, updating, and deleting tenants data, while maintaining strict access control based on user roles and tenant scopes.
...
├── tenants.exceptions.ts
├── tenants.interfaces.ts
├── tenants.module.ts
...
- Each service generally handles a single micro-feature (e.g., `create-tenants.service.ts`, `find-tenants.service.ts`).
...
- **Repositories (`tenants.repository.ts`)**: Abstracts direct database calls. For ERP modules, it relies on `TENANT_CONNECTION` to query the isolated tenant database. For Superadmin, it queries the master DB.
