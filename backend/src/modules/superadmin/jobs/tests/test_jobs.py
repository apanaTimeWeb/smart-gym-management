...
def test_get_job_by_id(auth_client, api_url):
    ...
    response = auth_client.get(f"{api_url}/superadmin/jobs/{fake_id}")
    assert response.status_code in [200, 404, 400]

def test_update_job(auth_client, api_url):
    fake_id = "00000000-0000-0000-0000-000000000000"
    response = auth_client.patch(f"{api_url}/superadmin/jobs/{fake_id}", json={
        "name": "Updated Name"
    })
    assert response.status_code in [200, 404, 400]
...
def test_delete_job(auth_client, api_url):
    fake_id = "00000000-0000-0000-0000-000000000000"
    response = auth_client.delete(f"{api_url}/superadmin/jobs/{fake_id}")
