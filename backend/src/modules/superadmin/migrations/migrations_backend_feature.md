# Migrations Module - Backend Feature Guide

## 1. Overview
The **Migrations** module is responsible for managing migrations-related operations within the Smart Gym Management system. It exposes RESTful APIs for creating, reading, updating, and deleting migrations data, while maintaining strict access control based on user roles and tenant scopes.
...
├── tests/
├── utils/
├── migrations.constants.ts
├── migrations.exceptions.ts
├── migrations.interfaces.ts
├── migrations.module.ts
├── migrations.repository.ts
```
...
- Each service generally handles a single micro-feature (e.g., `create-migrations.service.ts`, `find-migrations.service.ts`).
...
- **Repositories (`migrations.repository.ts`)**: Abstracts direct database calls. For ERP modules, it relies on `TENANT_CONNECTION` to query the isolated tenant database. For Superadmin, it queries the master DB.
