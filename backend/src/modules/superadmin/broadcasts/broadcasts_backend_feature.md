# Broadcasts Module - Backend Feature Guide

## 1. Overview
The **Broadcasts** module is responsible for managing broadcasts-related operations within the Smart Gym Management system. It exposes RESTful APIs for creating, reading, updating, and deleting broadcasts data, while maintaining strict access control based on user roles and tenant scopes.
...
├── broadcasts.exceptions.ts
├── broadcasts.interfaces.ts
...
- Each service generally handles a single micro-feature (e.g., `create-broadcasts.service.ts`, `find-broadcasts.service.ts`).
...
- **Repositories (`broadcasts.repository.ts`)**: Abstracts direct database calls. For ERP modules, it relies on `TENANT_CONNECTION` to query the isolated tenant database. For Superadmin, it queries the master DB.
