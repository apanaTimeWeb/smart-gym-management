# Invoices Module - Backend Feature Guide

## 1. Overview
The **Invoices** module is responsible for managing invoices-related operations within the Smart Gym Management system. It exposes RESTful APIs for creating, reading, updating, and deleting invoices data, while maintaining strict access control based on user roles and tenant scopes.
...
├── invoices.exceptions.ts
├── invoices.interfaces.ts
├── invoices.module.ts
...
- Each service generally handles a single micro-feature (e.g., `create-invoices.service.ts`, `find-invoices.service.ts`).
...
- **Repositories (`invoices.repository.ts`)**: Abstracts direct database calls. For ERP modules, it relies on `TENANT_CONNECTION` to query the isolated tenant database. For Superadmin, it queries the master DB.
