...
export class DashboardRepository {
  ...
  async getMemberCounts() {
    ...
    const active = await this.memberRepository.count({ where: { status: 'ACTIVE' as any } });
    const pending = await this.memberRepository.count({ where: { status: 'PENDING' as any } });
    const expired = await this.memberRepository.count({ where: { status: 'EXPIRED' as any } });
  ...
  async getStaffStats() {
    ...
    const active = await this.staffRepository.count({ where: { isActive: true } });
    return { total, active };
  }
  ...
  async getRecentMembers() {
    return this.memberRepository.find({ take: 5, order: { id: 'DESC' }, relations: ['plan'] });
  }

  async getRecentPayments() {
    return this.paymentRepository.find({ take: 5, order: { id: 'DESC' }, relations: ['member'] });
  }
