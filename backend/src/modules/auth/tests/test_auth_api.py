def test_login_non_existent_email(api_client, api_url):
    response = api_client.post(f"{api_url}/v1/auth/login", json={
...
def test_get_me_no_token(api_client, api_url):
    response = api_client.get(f"{api_url}/v1/auth/me")
    assert response.status_code == 401

def test_get_me_invalid_token(api_client, api_url):
    response = api_client.get(f"{api_url}/v1/auth/me", headers={
...
def test_get_me_valid_token(api_client, api_url):
    ...
    login_response = api_client.post(f"{api_url}/v1/auth/login", json={
        "email": "admin@gymsmart.com",
        "password": "superadmin123"
    })
    ...
    data = response.json().get("data", {})
    assert data.get("email") == "admin@gymsmart.com"
