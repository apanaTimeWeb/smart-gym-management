def test_get_all_workouts(auth_client, api_url):
    response = auth_client.get(f"{api_url}/v1/erp/workout?page=1&limit=20")
...
def test_create_workout(auth_client, api_url):
    response = auth_client.post(f"{api_url}/v1/erp/workout", json={
...
def test_update_workout(auth_client, api_url):
    response = auth_client.patch(f"{api_url}/v1/erp/workout/1", json={
        "difficulty": "Advanced",
...
def test_get_workout_by_id(auth_client, api_url):
    response = auth_client.get(f"{api_url}/v1/erp/workout/1")
    assert response.status_code in [200, 401, 404]

def test_delete_workout(auth_client, api_url):
    response = auth_client.delete(f"{api_url}/v1/erp/workout/1")
