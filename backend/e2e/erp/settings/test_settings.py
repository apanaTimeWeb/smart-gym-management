...
def test_create_setting(auth_client, api_url):
    ...
    create_resp = auth_client.post(f"{api_url}/erp/settings", json={
...
def test_get_setting_by_id(auth_client, api_url):
    ...
    if create_resp.status_code == 201:
        ...
        response = auth_client.get(f"{api_url}/erp/settings/{real_id}")
...
def test_update_setting(auth_client, api_url):
    ...
    if create_resp.status_code == 201:
        real_id = create_resp.json()["data"]["id"]
        response = auth_client.patch(f"{api_url}/erp/settings/{real_id}", json={
...
def test_delete_setting(auth_client, api_url):
    ...
    if create_resp.status_code == 201:
        real_id = create_resp.json()["data"]["id"]
        response = auth_client.delete(f"{api_url}/erp/settings/{real_id}")
        assert response.status_code in [200, 204]
        
        # Verify deletion
        fetch_resp = auth_client.get(f"{api_url}/erp/settings/{real_id}")
