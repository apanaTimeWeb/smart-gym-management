...
// FLOW: global-audit service -> GlobalAuditRepository -> TypeORM Repository<AuditLogEntity> -> PostgreSQL `audit_logs`.
...
import type { GlobalAuditListQuery, GlobalAuditCreateInput, GlobalAuditUpdateInput } from '@/modules/superadmin/global-audit/types/global-audit.interfaces';

@Injectable()
export class GlobalAuditRepository extends BaseRepository<AuditLogEntity> {
  constructor(@InjectRepository(AuditLogEntity) repository: Repository<AuditLogEntity>, transactionContext: TransactionContext) { super(repository, transactionContext); }

  /** Returns a validated, ordered, filtered page of active feature records. */
  async findPage(query: GlobalAuditListQuery): Promise<{ items: AuditLogEntity[]; total: number }> {
  ...
  async findById(id: string): Promise<AuditLogEntity | null> { return super.findById(id); }
  ...
  async createGlobalAudit(input: GlobalAuditCreateInput): Promise<AuditLogEntity> { const entity = this.activeRepository.create(input as never); return this.activeRepository.save(entity); }
