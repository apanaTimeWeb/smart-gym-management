...
export class AffiliatesPayoutService {
  ...
  async pay(id: string): Promise<ReturnType<typeof AffiliatesMapper.toDomain>> { const affiliate = await this.repository.findByIdOrThrow(id); const amount = affiliate.pendingPayout; const history = Array.isArray((affiliate as any).payoutHistory) ? (affiliate as any).payoutHistory : []; const entry = { id: randomUUID(), affiliateId: affiliate.id, affiliateName: affiliate.name, amount, method: 'BANK_TRANSFER', referenceId: `PAYOUT-${Date.now()}`, status: 'COMPLETED', paidAt: new Date().toISOString() }; await this.repository.payPendingPayout(id, entry); return AffiliatesMapper.toDomain(await this.repository.findByIdOrThrow(id)); }
