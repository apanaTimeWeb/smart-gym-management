...
import { TenantMessageChannel, TenantMessageStatus } from '@/modules/superadmin/messaging/messaging.entity';
...
export class MessagingWhatsAppCampaignService {
  ...
  async createMessagingWhatsAppCampaign(input: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
    ...
    return { id: created.id, status: created.status, queuedAt: created.createdAt.toISOString() };
  }
}
