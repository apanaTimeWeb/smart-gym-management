...
import type { MessagingListQuery, MessagingCreateInput, MessagingUpdateInput } from '@/modules/superadmin/messaging/types/messaging.interfaces';
...
export class MessagingRepository extends BaseRepository<TenantMessageEntity> {
  ...
  async createMessaging(input: MessagingCreateInput): Promise<TenantMessageEntity> { const entity = this.activeRepository.create(input as never); return this.activeRepository.save(entity); }
