...
// FLOW: plans service -> PlansRepository -> TypeORM Repository<SubscriptionPlanEntity> -> PostgreSQL `subscription_plans`.
...
import type { PlansListQuery, PlansCreateInput, PlansUpdateInput } from '@/modules/superadmin/saas-billing/plans/types/plans.interfaces';

@Injectable()
export class PlansRepository extends BaseRepository<SubscriptionPlanEntity> {
  constructor(@InjectRepository(SubscriptionPlanEntity) repository: Repository<SubscriptionPlanEntity>, transactionContext: TransactionContext) { super(repository, transactionContext); }

  /** Returns a validated, ordered, filtered page of active feature records. */
  async findPage(query: PlansListQuery): Promise<{ items: SubscriptionPlanEntity[]; total: number }> {
  ...
  async findById(id: string): Promise<SubscriptionPlanEntity | null> { return super.findById(id); }
  ...
  async createPlans(input: PlansCreateInput): Promise<SubscriptionPlanEntity> { const entity = this.activeRepository.create(input as never); return this.activeRepository.save(entity); }
