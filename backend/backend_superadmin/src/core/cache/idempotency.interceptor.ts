...
import { IDEMPOTENCY_REQUIRED } from '@/core/cache/idempotency.decorator';
...
export class IdempotencyInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    if (!key) throw new BadRequestException('Idempotency-Key header is required for this mutation');
  ...
  private resolveProtectedRequest(cached: string | null, key: string, next: CallHandler): Observable<unknown> {
    if (cached === '__IN_PROGRESS__') throw new ConflictException('Idempotent request is already processing');
