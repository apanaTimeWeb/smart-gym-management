...
@Injectable()
export class AdminDataExportRepository extends CoreTenantRepositoryBase<AdminDataExportEntity> {
   ...
   */
  async findAll(query: AdminDataExportQueryDto): Promise<CorePaginatedResult<AdminDataExportEntity>> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminDataExportEntity);
  ...
  async findById(id: string): Promise<AdminDataExportEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminDataExportEntity).findOne({ where: { id } });
  }
   ...
   */
  async createRecord(input: Record<string, unknown>): Promise<AdminDataExportEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminDataExportEntity);
   ...
   */
  async updateById(id: string, input: Record<string, unknown>): Promise<AdminDataExportEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminDataExportEntity);
   ...
   * @returns No value.
   */
  async markAsDeleted(id: string): Promise<AdminDataExportEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminDataExportEntity);
    const entity = await this.findByIdOrThrow(id);
   ...
   */
  async createExportJob(input: Record<string, unknown>): Promise<AdminDataExportEntity> {
   ...
   */
  async findFirstSnapshot(): Promise<AdminDataExportEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminDataExportEntity).findOne({ order: { createdAt: 'ASC' } });
  }
