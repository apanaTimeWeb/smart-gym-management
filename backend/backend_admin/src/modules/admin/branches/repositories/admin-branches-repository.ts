...
// FLOW: AdminBranchesService → AdminBranchesRepository → TypeORM → PostgreSQL branches.
...
@Injectable()
export class AdminBranchesRepository extends CoreTenantRepositoryBase<AdminBranchesEntity> {
   ...
   */
  async findAll(query: AdminBranchesQueryDto): Promise<CorePaginatedResult<AdminBranchesEntity>> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminBranchesEntity);
   ...
   */
  async findById(id: string): Promise<AdminBranchesEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminBranchesEntity).findOne({ where: { id } });
  }
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminBranchesEntity> {
   ...
   */
  async createRecord(input: Record<string, unknown>): Promise<AdminBranchesEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminBranchesEntity);
   ...
   */
  async updateById(id: string, input: Record<string, unknown>): Promise<AdminBranchesEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminBranchesEntity);
  ...
  async markAsDeleted(id: string): Promise<void> {
    await this.softDeleteRecord(id, AdminBranchesEntity);
  }
  /** @description Reads the first tenant-scoped snapshot used by read models and seed data.
   * @returns Snapshot entity or null.
   */
  async findFirstSnapshot(): Promise<AdminBranchesEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminBranchesEntity).findOne({ order: { createdAt: 'ASC' } });
  }
