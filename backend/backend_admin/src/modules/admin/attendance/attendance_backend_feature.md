...
| File | Responsibility |
|---|---|
| `admin-attendance.module.ts` | Single-purpose `admin-attendance.module.ts` implementation for the `attendance` feature. |
| `admin-attendance.seeder.ts` | Single-purpose `admin-attendance.seeder.ts` implementation for the `attendance` feature. |
| `controllers/admin-attendance-query.controller.ts` | Single-purpose `admin-attendance-query.controller.ts` implementation for the `attendance` feature. |
| `domain/admin-attendance.domain.ts` | Single-purpose `admin-attendance.domain.ts` implementation for the `attendance` feature. |
| `dtos/admin-attendance-id.dto.ts` | Single-purpose `admin-attendance-id.dto.ts` implementation for the `attendance` feature. |
| `dtos/admin-attendance-mutation.dto.ts` | Single-purpose `admin-attendance-mutation.dto.ts` implementation for the `attendance` feature. |
| `dtos/admin-attendance-query.dto.ts` | Single-purpose `admin-attendance-query.dto.ts` implementation for the `attendance` feature. |
| `dtos/admin-attendance-response.dto.ts` | Single-purpose `admin-attendance-response.dto.ts` implementation for the `attendance` feature. |
| `entities/admin-attendance-entity.ts` | Single-purpose `admin-attendance-entity.ts` implementation for the `attendance` feature. |
| `mappers/admin-attendance.mapper.ts` | Single-purpose `admin-attendance.mapper.ts` implementation for the `attendance` feature. |
| `repositories/admin-attendance-repository.ts` | Single-purpose `admin-attendance-repository.ts` implementation for the `attendance` feature. |
| `services/admin-attendance-query.service.ts` | Single-purpose `admin-attendance-query.service.ts` implementation for the `attendance` feature. |

## Feature Inventory
| Controller/Endpoint | HTTP | Path | Purpose | Request DTO | Response DTO |
|---|---|---|---|---|---|
| Controller | GET | `/api/v1/admin/attendance` | Provides the GET operation required by the Admin `attendance` frontend capability. | Feature DTO | Feature response DTO |
| Controller | GET | `/api/v1/admin/attendance/summary` | Provides the GET operation required by the Admin `attendance` frontend capability. | Feature DTO | Feature response DTO |
| Controller | GET | `/api/v1/admin/attendance/trend` | Provides the GET operation required by the Admin `attendance` frontend capability. | Feature DTO | Feature response DTO |
...
- `admin-attendance.module.ts` — Single responsibility within `attendance`. MUST remain scoped to this feature responsibility.
- `admin-attendance.seeder.ts` — Single responsibility within `attendance`. MUST remain scoped to this feature responsibility.
- `controllers/admin-attendance-query.controller.ts` — Single responsibility within `attendance`. MUST NOT contain business or persistence logic.
- `domain/admin-attendance.domain.ts` — Single responsibility within `attendance`. MUST remain scoped to this feature responsibility.
- `dtos/admin-attendance-id.dto.ts` — Single responsibility within `attendance`. MUST NOT contain business side effects.
- `dtos/admin-attendance-mutation.dto.ts` — Single responsibility within `attendance`. MUST NOT contain business side effects.
- `dtos/admin-attendance-query.dto.ts` — Single responsibility within `attendance`. MUST NOT contain business side effects.
- `dtos/admin-attendance-response.dto.ts` — Single responsibility within `attendance`. MUST NOT contain business side effects.
- `entities/admin-attendance-entity.ts` — Single responsibility within `attendance`. MUST remain scoped to this feature responsibility.
- `mappers/admin-attendance.mapper.ts` — Single responsibility within `attendance`. MUST NOT perform DB I/O.
- `repositories/admin-attendance-repository.ts` — Single responsibility within `attendance`. MUST NOT contain controller or UI logic.
- `services/admin-attendance-query.service.ts` — Single responsibility within `attendance`. MUST NOT perform direct TypeORM persistence.
...
|---|---|---|
| `/api/v1/admin/attendance` | GET | Feature DTO fields exactly matching frontend request usage. |
| `/api/v1/admin/attendance/summary` | GET | Feature DTO fields exactly matching frontend request usage. |
| `/api/v1/admin/attendance/trend` | GET | Feature DTO fields exactly matching frontend request usage. |

### Response Shape
| Endpoint | Response DTO fields | Notes |
|---|---|---|
| `/api/v1/admin/attendance` | Explicit frontend-consumed response DTO | Wrapped as `ApiResponse<T>`; pagination metadata is present only on paginated list responses. |
| `/api/v1/admin/attendance/summary` | Explicit frontend-consumed response DTO | Wrapped as `ApiResponse<T>`; pagination metadata is present only on paginated list responses. |
| `/api/v1/admin/attendance/trend` | Explicit frontend-consumed response DTO | Wrapped as `ApiResponse<T>`; pagination metadata is present only on paginated list responses. |
