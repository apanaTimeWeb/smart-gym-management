...
@Injectable()
export class AdminAttendanceRepository extends CoreTenantRepositoryBase<AdminAttendanceEntity> {
   ...
   */
  async findAll(query: AdminAttendanceQueryDto): Promise<CorePaginatedResult<AdminAttendanceEntity>> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminAttendanceEntity);
   ...
   */
  async findById(id: string): Promise<AdminAttendanceEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminAttendanceEntity).findOne({ where: { id } });
  }
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminAttendanceEntity> {
   ...
   */
  async createRecord(input: Record<string, unknown>): Promise<AdminAttendanceEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminAttendanceEntity);
   ...
   */
  async updateById(id: string, input: Record<string, unknown>): Promise<AdminAttendanceEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminAttendanceEntity);
   ...
   * @returns No value.
   */
  async markAsDeleted(id: string): Promise<AdminAttendanceEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminAttendanceEntity);
    const entity = await this.findByIdOrThrow(id);
   ...
   */
  async findFirstSnapshot(): Promise<AdminAttendanceEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminAttendanceEntity).findOne({ order: { createdAt: 'ASC' } });
  }
