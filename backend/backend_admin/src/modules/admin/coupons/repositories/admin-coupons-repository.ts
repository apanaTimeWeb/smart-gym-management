...
@Injectable()
export class AdminCouponsRepository extends CoreTenantRepositoryBase<AdminCouponsEntity> {
   ...
   */
  async findAll(query: AdminCouponsQueryDto): Promise<CorePaginatedResult<AdminCouponsEntity>> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminCouponsEntity);
   ...
   */
  async findById(id: string): Promise<AdminCouponsEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminCouponsEntity).findOne({ where: { id } });
  }
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminCouponsEntity> {
   ...
   */
  async createRecord(input: Record<string, unknown>): Promise<AdminCouponsEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminCouponsEntity);
   ...
   */
  async updateById(id: string, input: Record<string, unknown>): Promise<AdminCouponsEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminCouponsEntity);
   ...
   * @returns No value.
   */
  async markAsDeleted(id: string): Promise<AdminCouponsEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminCouponsEntity);
    const entity = await this.findByIdOrThrow(id);
   ...
   * @returns Updated entity.
   */
  async toggleActiveById(id: string): Promise<AdminCouponsEntity> {
    const entity = await this.findByIdOrThrow(id);
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminCouponsEntity);
   ...
   */
  async findFirstSnapshot(): Promise<AdminCouponsEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminCouponsEntity).findOne({ order: { createdAt: 'ASC' } });
  }
