...
@Injectable()
export class AdminNotificationsRepository extends CoreTenantRepositoryBase<AdminNotificationsEntity> {
   ...
   */
  async findAll(query: AdminNotificationsQueryDto): Promise<CorePaginatedResult<AdminNotificationsEntity>> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminNotificationsEntity);
   ...
   */
  async findById(id: string): Promise<AdminNotificationsEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminNotificationsEntity).findOne({ where: { id } });
  }
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminNotificationsEntity> {
   ...
   */
  async createRecord(input: Record<string, unknown>): Promise<AdminNotificationsEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminNotificationsEntity);
   ...
   */
  async updateById(id: string, input: Record<string, unknown>): Promise<AdminNotificationsEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminNotificationsEntity);
   ...
   * @returns No value.
   */
  async markAsDeleted(id: string): Promise<AdminNotificationsEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminNotificationsEntity);
    const entity = await this.findByIdOrThrow(id);
   ...
   */
  async markAsReadById(id: string): Promise<AdminNotificationsEntity> {
    const entity = await this.findByIdOrThrow(id);
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminNotificationsEntity);
    entity.read = true;
  ...
  async markAllRead(): Promise<number> {
    const source = await this.tenantManager.getCurrent();
    const result = await source.getRepository(AdminNotificationsEntity).createQueryBuilder()
   ...
   */
  async findFirstSnapshot(): Promise<AdminNotificationsEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminNotificationsEntity).findOne({ order: { createdAt: 'ASC' } });
  }
