...
export class AdminHrQueryService {
  ...
  async fetchStaff(query: AdminHrQueryDto): Promise<AdminHrStaffListResponseDto> {
    const snapshot = await this.repository.findFirstSnapshot(); return snapshot ? { staff: snapshot.payload.staff ?? [], total: Array.isArray(snapshot.payload.staff) ? snapshot.payload.staff.length : 0 } as AdminHrStaffListResponseDto : { staff: [], total: 0 } as AdminHrStaffListResponseDto;
  }
  ...
  async fetchStaffById(id: string): Promise<AdminHrStaffDto> {
    ...
    return this.mapper.toResponse(this.mapper.toDomain(entity)) as AdminHrStaffDto;
  }
  ...
  async fetchPayrolls(query: AdminHrQueryDto): Promise<AdminHrPayrollListResponseDto> {
    const snapshot = await this.repository.findFirstSnapshot(); return snapshot ? { payrolls: snapshot.payload.payrolls ?? [], total: Array.isArray(snapshot.payload.payrolls) ? snapshot.payload.payrolls.length : 0 } as AdminHrPayrollListResponseDto : { payrolls: [], total: 0 } as AdminHrPayrollListResponseDto;
  }
  ...
  async fetchSummary(query: AdminHrQueryDto): Promise<AdminHrSummaryDto> {
    const snapshot = await this.repository.findFirstSnapshot(); return snapshot ? (snapshot.payload.summary as AdminHrSummaryDto ?? {} as AdminHrSummaryDto) : {} as AdminHrSummaryDto;
  }
  ...
  async fetchLedger(query: AdminHrQueryDto): Promise<AdminHrLedgerEntryDto[]> {
    const snapshot = await this.repository.findFirstSnapshot(); return snapshot ? snapshot.payload.ledger as AdminHrLedgerEntryDto[] ?? [] : [];
  }
  ...
  async fetchPerformance(query: AdminHrQueryDto): Promise<AdminHrStaffPerformanceRecordDto[]> {
    const snapshot = await this.repository.findFirstSnapshot(); return snapshot ? snapshot.payload.performance as AdminHrStaffPerformanceRecordDto[] ?? [] : [];
  }
}
