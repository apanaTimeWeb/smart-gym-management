...
export class AdminPayoutsQueryService {
   ...
   */
  async fetchPayouts(query: AdminPayoutsQueryDto): Promise<unknown> {
    const snapshot = await this.repository.findFirstSnapshot(); return snapshot ? snapshot.payload['payouts'] ?? [] : [];
  }
   ...
   * @returns Frontend contract response.
   */
  async fetchPnl(query: AdminPayoutsQueryDto): Promise<unknown> {
    const snapshot = await this.repository.findFirstSnapshot(); return snapshot ? snapshot.payload['pnl'] ?? [] : [];
  }
   ...
   * @returns Frontend contract response.
   */
  async fetchKPIs(query: AdminPayoutsQueryDto): Promise<Record<string, unknown>> {
