# RESPONSIBILITY: Black-box API contract smoke tests for Admin finance.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_finance
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute fetchPayments', 'GET', '/api/v1/admin/finance/payments/fetchPayments'),
    ('Execute fetchSummary', 'GET', '/api/v1/admin/finance/summary'),
    ('Execute fetchPnl', 'GET', '/api/v1/admin/finance/pnl'),
    ('Execute fetchExpenses', 'GET', '/api/v1/admin/finance/payments/fetchExpenses'),
], ids=lambda item: item[0])
def test_finance_endpoint_contract(name, method, path):
