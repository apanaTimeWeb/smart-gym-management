# RESPONSIBILITY: Black-box API contract verification for the Admin subscriptions module.
# FLOW: pytest -> real HTTP request -> running backend controller -> canonical response assertions.
# MODULE: admin_subscriptions
...
ROUTES = ['/admin/subscriptions/fetchSubscription', '/admin/subscriptions/fetchPlans', '/admin/subscriptions/fetchInvoices', '/admin/subscriptions/fetchPaymentMethods', '/admin/subscriptions/fetchKPIs']
MUTATION_ROUTES = [('POST', '/admin/subscriptions/upgradePlan'), ('POST', '/admin/subscriptions/toggleAutoRenew'), ('POST', '/admin/subscriptions/setDefaultPaymentMethod'), ('DELETE', '/admin/subscriptions/removePaymentMethod')]
...
def test_admin_subscriptions_read_contract(path: str) -> None:
    if not TOKEN:
...
def test_admin_subscriptions_mutation_requires_idempotency_key(method: str, path: str) -> None:
    if not TOKEN:
        pytest.skip("E2E_ACCESS_TOKEN is required for live API verification")
