# RESPONSIBILITY: Black-box API contract smoke tests for Admin permissions.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_permissions
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute updateRolePermissions', 'POST', '/api/v1/admin/permissions/updateRolePermissions'),
    ('Execute updateGymOverride', 'POST', '/api/v1/admin/permissions/updateGymOverride'),
    ('Execute fetchPermissions', 'GET', '/api/v1/admin/permissions/fetchPermissions'),
], ids=lambda item: item[0])
def test_permissions_endpoint_contract(name, method, path):
