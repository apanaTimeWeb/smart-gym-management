# RESPONSIBILITY: Black-box API contract verification for the Admin permissions module.
...
ROUTES = ['/admin/permissions/fetchPermissions']
MUTATION_ROUTES = [('POST', '/admin/permissions/updateRolePermissions'), ('POST', '/admin/permissions/updateGymOverride')]
...
def test_admin_permissions_read_contract(path: str) -> None:
    if not TOKEN:
...
def test_admin_permissions_mutation_requires_idempotency_key(method: str, path: str) -> None:
    if not TOKEN:
        pytest.skip("E2E_ACCESS_TOKEN is required for live API verification")
