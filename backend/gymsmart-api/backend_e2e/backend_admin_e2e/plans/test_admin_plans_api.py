# RESPONSIBILITY: Black-box API contract smoke tests for Admin plans.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_plans
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute createPlan', 'POST', '/api/v1/admin/plans/createPlan'),
    ('Execute updatePlan', 'POST', '/api/v1/admin/plans/updatePlan'),
    ('Execute deletePlan', 'DELETE', '/api/v1/admin/plans/deletePlan'),
    ('Execute fetchAllPlans', 'GET', '/api/v1/admin/plans/fetchAllPlans'),
    ('Execute fetchPlanById', 'GET', '/api/v1/admin/plans/fetchPlanById'),
    ('Execute fetchPlanRevenue', 'GET', '/api/v1/admin/plans/fetchPlanRevenue'),
], ids=lambda item: item[0])
def test_plans_endpoint_contract(name, method, path):
