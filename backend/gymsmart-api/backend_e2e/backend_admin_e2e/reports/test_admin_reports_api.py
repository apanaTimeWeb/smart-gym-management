# RESPONSIBILITY: Black-box API contract verification for the Admin reports module.
...
ROUTES = ['/admin/reports/fetchReportData', '/admin/reports/revenue', '/admin/reports/attendance', '/admin/reports/members', '/admin/reports/payroll', '/admin/reports/pnl']
MUTATION_ROUTES = [('POST', '/admin/reports/exportReport')]
...
def test_admin_reports_read_contract(path: str) -> None:
    if not TOKEN:
...
def test_admin_reports_mutation_requires_idempotency_key(method: str, path: str) -> None:
    if not TOKEN:
        pytest.skip("E2E_ACCESS_TOKEN is required for live API verification")
