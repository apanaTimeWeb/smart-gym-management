# RESPONSIBILITY: Black-box API contract smoke tests for Admin reports.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_reports
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute exportReport', 'POST', '/api/v1/admin/reports/exportReport'),
    ('Execute fetchReportData', 'GET', '/api/v1/admin/reports/fetchReportData'),
    ('Fetch revenue report', 'GET', '/api/v1/admin/reports/revenue'),
    ('Fetch attendance report', 'GET', '/api/v1/admin/reports/attendance'),
    ('Fetch membership report', 'GET', '/api/v1/admin/reports/members'),
    ('Fetch payroll report', 'GET', '/api/v1/admin/reports/payroll'),
    ('Fetch PnL report', 'GET', '/api/v1/admin/reports/pnl'),
], ids=lambda item: item[0])
def test_reports_endpoint_contract(name, method, path):
