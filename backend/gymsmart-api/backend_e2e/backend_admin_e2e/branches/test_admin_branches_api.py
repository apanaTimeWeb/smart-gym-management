# RESPONSIBILITY: Black-box API contract smoke tests for Admin branches.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_branches
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute fetchBranches', 'GET', '/api/v1/admin/branches/fetchBranches'),
    ('Execute findById', 'GET', '/api/v1/admin/branches/{{id}}'),
], ids=lambda item: item[0])
def test_branches_endpoint_contract(name, method, path):
