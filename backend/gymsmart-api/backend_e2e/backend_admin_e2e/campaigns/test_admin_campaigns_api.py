# RESPONSIBILITY: Black-box API contract smoke tests for Admin campaigns.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_campaigns
...
@pytest.mark.parametrize("name,method,path", [
    ('Campaign audiences compatibility endpoint', 'GET', '/api/v1/api/admin/campaigns/audiences'),
    ('Campaign templates compatibility endpoint', 'GET', '/api/v1/api/admin/campaigns/templates'),
    ('Campaign recipients compatibility endpoint', 'GET', '/api/v1/api/admin/campaigns/recipients'),
    ('Execute fetchAudiences', 'GET', '/api/v1/admin/campaigns/audiences'),
    ('Execute fetchTemplates', 'GET', '/api/v1/admin/campaigns/templates'),
    ('Execute fetchRecipients', 'GET', '/api/v1/admin/campaigns/recipients'),
], ids=lambda item: item[0])
def test_campaigns_endpoint_contract(name, method, path):
