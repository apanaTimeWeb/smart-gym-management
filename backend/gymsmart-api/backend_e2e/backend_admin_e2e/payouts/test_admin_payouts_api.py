# RESPONSIBILITY: Black-box API contract smoke tests for Admin payouts.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_payouts
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute fetchPayouts', 'GET', '/api/v1/admin/payouts/fetchPayouts'),
    ('Execute fetchPnl', 'GET', '/api/v1/admin/payouts/fetchPnL'),
    ('Execute fetchKPIs', 'GET', '/api/v1/admin/payouts/fetchKPIs'),
], ids=lambda item: item[0])
def test_payouts_endpoint_contract(name, method, path):
