# RESPONSIBILITY: Black-box API contract smoke tests for Admin attendance.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_attendance
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute fetchAttendance', 'GET', '/api/v1/admin/attendance'),
    ('Execute fetchSummary', 'GET', '/api/v1/admin/attendance/summary'),
    ('Execute fetchTrend', 'GET', '/api/v1/admin/attendance/trend'),
], ids=lambda item: item[0])
def test_attendance_endpoint_contract(name, method, path):
