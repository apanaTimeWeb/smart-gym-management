# RESPONSIBILITY: Black-box API contract smoke tests for Admin members.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_members
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute listMembers', 'GET', '/api/v1/admin/members'),
    ('Execute fetchSummary', 'GET', '/api/v1/admin/members/summary'),
    ('Execute exportMembers', 'GET', '/api/v1/admin/members/export'),
    ('Execute findMemberById', 'GET', '/api/v1/admin/members/{{id}}'),
], ids=lambda item: item[0])
def test_members_endpoint_contract(name, method, path):
