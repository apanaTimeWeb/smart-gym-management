# RESPONSIBILITY: Black-box API contract smoke tests for Admin usage.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_usage
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute createUpgradeRequest', 'POST', '/api/v1/admin/usage/upgrade-request'),
    ('Execute fetchUsage', 'GET', '/api/v1/admin/usage'),
    ('Execute fetchPlans', 'GET', '/api/v1/admin/usage/plans'),
], ids=lambda item: item[0])
def test_usage_endpoint_contract(name, method, path):
