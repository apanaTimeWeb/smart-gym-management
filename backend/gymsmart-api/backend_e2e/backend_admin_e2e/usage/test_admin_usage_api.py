# RESPONSIBILITY: Black-box API contract verification for the Admin usage module.
...
ROUTES = ['/admin/usage', '/admin/usage/plans']
MUTATION_ROUTES = [('POST', '/admin/usage/upgrade-request')]
...
def test_admin_usage_read_contract(path: str) -> None:
    if not TOKEN:
...
def test_admin_usage_mutation_requires_idempotency_key(method: str, path: str) -> None:
    if not TOKEN:
        pytest.skip("E2E_ACCESS_TOKEN is required for live API verification")
