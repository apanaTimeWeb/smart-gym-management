# RESPONSIBILITY: Black-box API contract verification for the Admin data-export module.
# FLOW: pytest -> real HTTP request -> running backend controller -> canonical response assertions.
# MODULE: admin_data-export
...
ROUTES = ['/admin/data-export/fetchJobs', '/admin/data-export/fetchKPIs']
MUTATION_ROUTES = [('POST', '/admin/data-export/createExport'), ('DELETE', '/admin/data-export/deleteJob')]
...
def test_admin_data_export_read_contract(path: str) -> None:
    if not TOKEN:
...
def test_admin_data_export_mutation_requires_idempotency_key(method: str, path: str) -> None:
    if not TOKEN:
        pytest.skip("E2E_ACCESS_TOKEN is required for live API verification")
