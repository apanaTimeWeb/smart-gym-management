# RESPONSIBILITY: Black-box API contract smoke tests for Admin data-export.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_data_export
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute createExport', 'POST', '/api/v1/admin/data-export/createExport'),
    ('Execute deleteJob', 'DELETE', '/api/v1/admin/data-export/deleteJob'),
    ('Execute fetchJobs', 'GET', '/api/v1/admin/data-export/fetchJobs'),
    ('Execute fetchKPIs', 'GET', '/api/v1/admin/data-export/fetchKPIs'),
], ids=lambda item: item[0])
def test_data_export_endpoint_contract(name, method, path):
