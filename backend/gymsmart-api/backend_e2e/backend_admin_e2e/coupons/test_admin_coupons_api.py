# RESPONSIBILITY: Black-box API contract smoke tests for Admin coupons.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_coupons
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute createCoupon', 'POST', '/api/v1/admin/coupons/createCoupon'),
    ('Execute updateCoupon', 'POST', '/api/v1/admin/coupons/updateCoupon'),
    ('Execute deleteCoupon', 'DELETE', '/api/v1/admin/coupons/deleteCoupon'),
    ('Execute toggleCoupon', 'POST', '/api/v1/admin/coupons/toggleCoupon'),
    ('Execute fetchCoupons', 'GET', '/api/v1/admin/coupons/fetchCoupons'),
    ('Fetch coupon KPIs', 'GET', '/api/v1/admin/coupons/kpis'),
], ids=lambda item: item[0])
def test_coupons_endpoint_contract(name, method, path):
