# RESPONSIBILITY: Black-box API contract verification for the Admin coupons module.
...
ROUTES = ['/admin/coupons/fetchCoupons', '/admin/coupons/kpis']
MUTATION_ROUTES = [('POST', '/admin/coupons/createCoupon'), ('POST', '/admin/coupons/updateCoupon'), ('DELETE', '/admin/coupons/deleteCoupon'), ('POST', '/admin/coupons/toggleCoupon')]
...
def test_admin_coupons_read_contract(path: str) -> None:
    if not TOKEN:
...
def test_admin_coupons_mutation_requires_idempotency_key(method: str, path: str) -> None:
    if not TOKEN:
        pytest.skip("E2E_ACCESS_TOKEN is required for live API verification")
