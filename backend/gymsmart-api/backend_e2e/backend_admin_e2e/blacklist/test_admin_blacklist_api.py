# RESPONSIBILITY: Black-box API contract smoke tests for Admin blacklist.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_blacklist
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute addToBlacklist', 'POST', '/api/v1/admin/blacklist/addToBlacklist'),
    ('Execute removeFromBlacklist', 'DELETE', '/api/v1/admin/blacklist/removeFromBlacklist'),
    ('Execute toggleBlacklist', 'POST', '/api/v1/admin/blacklist/toggleBlacklist'),
    ('Execute propagateToAllBranches', 'POST', '/api/v1/admin/blacklist/propagateToAllBranches'),
    ('Execute fetchBlacklist', 'GET', '/api/v1/admin/blacklist/fetchBlacklist'),
    ('Execute fetchKPIs', 'GET', '/api/v1/admin/blacklist/fetchKPIs'),
], ids=lambda item: item[0])
def test_blacklist_endpoint_contract(name, method, path):
