# RESPONSIBILITY: Black-box API contract verification for the Admin blacklist module.
...
ROUTES = ['/admin/blacklist/fetchBlacklist', '/admin/blacklist/fetchKPIs']
MUTATION_ROUTES = [('POST', '/admin/blacklist/addToBlacklist'), ('DELETE', '/admin/blacklist/removeFromBlacklist'), ('POST', '/admin/blacklist/toggleBlacklist'), ('POST', '/admin/blacklist/propagateToAllBranches')]
...
def test_admin_blacklist_read_contract(path: str) -> None:
    if not TOKEN:
...
def test_admin_blacklist_mutation_requires_idempotency_key(method: str, path: str) -> None:
    if not TOKEN:
        pytest.skip("E2E_ACCESS_TOKEN is required for live API verification")
