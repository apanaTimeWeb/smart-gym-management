# RESPONSIBILITY: Black-box API contract smoke tests for Admin sales.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_sales
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute fetchOverview', 'GET', '/api/v1/admin/sales/overview'),
    ('Execute fetchReferralSources', 'GET', '/api/v1/admin/sales/referral-sources'),
    ('Execute fetchMembershipReport', 'GET', '/api/v1/admin/sales/membership-report'),
    ('Execute fetchPendingPayments', 'GET', '/api/v1/admin/sales/pending-payments'),
    ('Execute fetchAllMemberships', 'GET', '/api/v1/admin/sales/all-memberships'),
    ('Execute fetchStoreOrders', 'GET', '/api/v1/admin/sales/store-orders'),
    ('Execute fetchStoreSummary', 'GET', '/api/v1/admin/sales/store-summary'),
], ids=lambda item: item[0])
def test_sales_endpoint_contract(name, method, path):
