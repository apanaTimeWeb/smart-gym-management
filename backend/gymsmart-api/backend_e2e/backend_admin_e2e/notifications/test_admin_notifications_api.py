# RESPONSIBILITY: Black-box API contract smoke tests for Admin notifications.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_notifications
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute markRead', 'PATCH', '/api/v1/admin/notifications/{{id}}/read'),
    ('Execute markAllRead', 'PATCH', '/api/v1/admin/notifications/read-all'),
    ('Execute listNotifications', 'GET', '/api/v1/admin/notifications'),
], ids=lambda item: item[0])
def test_notifications_endpoint_contract(name, method, path):
