# RESPONSIBILITY: Black-box API contract smoke tests for Admin profile.
# FLOW: External pytest client -> real Admin API -> canonical response/error envelope.
# MODULE: backend_admin_e2e/admin_profile
...
@pytest.mark.parametrize("name,method,path", [
    ('Execute updateProfile', 'POST', '/api/v1/admin/adminProfile/updateProfile'),
    ('Execute updatePassword', 'POST', '/api/v1/admin/adminProfile/updatePassword'),
    ('Execute fetchProfile', 'GET', '/api/v1/admin/adminProfile/fetchProfile'),
], ids=lambda item: item[0])
def test_profile_endpoint_contract(name, method, path):
