 ...
 * Primary Intent: Executes the updateAffiliates use case within its owning backend boundary.
   ...
   */
  async updateAffiliates(id: string, input: SuperadminAffiliatesUpdateInput): Promise<SuperadminAffiliatesResponseDto> { const { bankDetails, ...rest } = input; const next = { ...rest, currency: input.currency?.toUpperCase(), ...(bankDetails !== undefined ? { bankDetails: bankDetails ? this.encryption.encrypt(typeof bankDetails === 'string' ? bankDetails : JSON.stringify(bankDetails)) : null } : {}) } as any; const updated = await this.repository.updateAffiliatesById(id, next); return SuperadminAffiliatesMapper.toResponse(SuperadminAffiliatesMapper.toDomain(updated), this.config.getOrThrow<string>('app.defaultCurrency')); }
}
