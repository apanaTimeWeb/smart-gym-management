...
import type { SuperadminAffiliatesCreateInput } from '@/backend_superadmin/superadmin_modules/affiliates/affiliates_types/superadmin-affiliates.interfaces';
/**
 * Primary Intent: Defines SuperadminAffiliatesCreateService as an explicit backend construct in its owning role/module boundary.
...
/**
 * Primary Intent: Executes the createAffiliates use case within the owning backend feature boundary.
 ...
 * Primary Intent: Executes the createAffiliates use case within its owning backend boundary.
   ...
   */
  async createAffiliates(input: SuperadminAffiliatesCreateInput): Promise<SuperadminAffiliatesResponseDto> { const currency = (input.currency ?? this.config.getOrThrow<string>('app.defaultCurrency')).toUpperCase(); const next = { ...input, currency, bankDetails: input.bankDetails ? this.encryption.encrypt(JSON.stringify(input.bankDetails)) : null }; const created = await this.repository.createAffiliates(next); return SuperadminAffiliatesMapper.toResponse(SuperadminAffiliatesMapper.toDomain(created), this.config.getOrThrow<string>('app.defaultCurrency')); }
}
