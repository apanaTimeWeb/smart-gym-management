...
/**
 * Primary Intent: Defines SuperadminSettingsGovernanceQueryController as an explicit backend construct in its owning role/module boundary.
...
/**
 * Primary Intent: Executes the governance use case within the owning backend feature boundary.
 ...
 */
  ...
  // SLA: FAST
@Get(['superadmin/settings/governance', 'api/superadmin/settings/governance'])
  @ApiResponse({ status: HttpStatus.OK, description: 'Successful response.' })
  @ApiOperation({ summary: 'governance' })
  /**
   * Primary Intent: Executes the governance use case within its owning backend boundary.
   * Edge Cases: Invalid input, missing resources, authorization failures, tenant mismatches, retries, and concurrency are handled according to the owning contract.
   * Side-Effects: Only documented persistence, cache, event, job, or external effects are permitted.
   * AI-Note: Preserve explicit return types, guard clauses, module isolation, and frozen API semantics.
   */
  async governance(@Query() query: SuperadminQueryDto): Promise<Awaited<ReturnType<SuperadminSettingsGovernanceService['findSettingsGovernance']>>> { void query; return await this.governanceService.findSettingsGovernance(); }
