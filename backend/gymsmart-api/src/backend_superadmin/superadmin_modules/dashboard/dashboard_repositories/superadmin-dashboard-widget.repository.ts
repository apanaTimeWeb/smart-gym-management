 ...
 * Primary Intent: Executes the queryBusinessAlerts use case within its owning backend boundary.
  ...
  private async queryBusinessAlerts(): Promise<Array<{ id: string; level: string; title: string; detail: string; count: number }>> {
    const rows = await this.dataSource.query<Array<{ id: string; level: string; title: string; detail: string; count: string | number }>>(`SELECT 'overdue-invoices' AS id,'CRITICAL' AS level,'Overdue invoices' AS title,'Invoices requiring payment recovery.' AS detail,COUNT(*)::int AS count FROM superadmin_saas_invoices WHERE deleted_at IS NULL AND status='OVERDUE' UNION ALL SELECT 'expiring-trials','WARNING','Trials expiring soon','Trial tenants ending within seven days.',COUNT(*)::int FROM tenants WHERE deleted_at IS NULL AND status='TRIAL' AND trial_ends_at >= CURRENT_TIMESTAMP AND trial_ends_at < CURRENT_TIMESTAMP+INTERVAL '7 days' UNION ALL SELECT 'critical-support','CRITICAL','Critical support tickets','Urgent or critical tickets awaiting resolution.',COUNT(*)::int FROM superadmin_support_tickets WHERE deleted_at IS NULL AND priority IN ('URGENT','CRITICAL') AND status IN ('OPEN','IN_PROGRESS','WAITING') ORDER BY count DESC,id`);
 ...
 * Primary Intent: Executes the queryHealth use case within its owning backend boundary.
  ...
  private async queryHealth(): Promise<DashboardHealthRow> {
    const [row] = await this.dataSource.query<DashboardHealthRow[]>(`SELECT COUNT(*)::int AS open_critical FROM superadmin_support_tickets WHERE deleted_at IS NULL AND priority IN ('URGENT','CRITICAL') AND status IN ('OPEN','IN_PROGRESS','WAITING')`);
