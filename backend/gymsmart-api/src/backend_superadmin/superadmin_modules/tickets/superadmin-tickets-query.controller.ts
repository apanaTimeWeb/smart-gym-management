...
import { SuperadminTicketsQueryDto } from '@/backend_superadmin/superadmin_modules/tickets/tickets_dtos/superadmin-tickets-query.dto';
...
/**
 * Primary Intent: Defines SuperadminTicketsQueryController as an explicit backend construct in its owning role/module boundary.
 ...
 */
   ...
   */
  async findAll(@Query() query: SuperadminTicketsQueryDto): Promise<Awaited<ReturnType<SuperadminTicketsListService['findTicketsPage']>>> { return await this.listService.findTicketsPage(query); }
/**
 ...
 */
   ...
   * AI-Note: Preserve explicit return types, guard clauses, module isolation, and frozen API semantics.
   */
  async findOne(@Param('id') id: string): Promise<Awaited<ReturnType<SuperadminTicketsFindService['findTicketsById']>>> { return await this.findService.findTicketsById(id); }
}
