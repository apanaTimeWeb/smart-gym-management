 ...
 * Primary Intent: Executes the `getExportStatus` responsibility owned by this superadmin-export-data.service construct.
  ...
  async getExportStatus(jobId: string): Promise<SuperadminExportDataStatusResponseDto> {
    ...
    return { jobId: job.id, status: String(job.status), ...(job.resultPath && token && job.downloadTokenHash ? { downloadUrl: `${process.env.API_URL || 'http://localhost:5000'}/superadmin/export-data/download/${job.id}/${token}`, expiresAt: job.expiresAt!.toISOString() } : {}), ...(job.errorCode ? { errorCode: job.errorCode } : {}) };
  }
