...
// FLOW: jobs service -> SuperadminJobsRepository -> TypeORM Repository<SuperadminJobsEntity> -> PostgreSQL `background_jobs`.
import { Injectable } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository } from 'typeorm';
import { BackgroundJobStatus } from '@/backend_superadmin/superadmin_modules/system-ops/jobs/superadmin-system-ops-jobs.entity';
...
import type { SuperadminJobsListQuery, SuperadminJobsCreateInput, SuperadminJobsUpdateInput } from '@/backend_superadmin/superadmin_modules/system-ops/jobs/types/superadmin-system-ops-jobs.interfaces';

@Injectable()
export class SuperadminJobsRepository extends BaseRepository<SuperadminJobsEntity> {
  constructor(@InjectRepository(SuperadminJobsEntity) repository: Repository<SuperadminJobsEntity>, transactionContext: SuperadminTransactionContext) { super(repository, transactionContext); }

  /** Returns a validated, ordered, filtered page of active feature records. */
  async findPage(query: SuperadminJobsListQuery): Promise<{ items: SuperadminJobsEntity[]; total: number }> {
  ...
  async findById(id: string): Promise<SuperadminJobsEntity | null> { return super.findById(id); }

  /** Returns one active record by id and throws when absent. */
  async findByIdOrThrow(id: string): Promise<SuperadminJobsEntity> { return super.findByIdOrThrow(id, 'Jobs record not found'); }

  /** Creates and persists a jobs record. */
  async createJobs(input: SuperadminJobsCreateInput): Promise<SuperadminJobsEntity> { const entity = this.activeRepository.create(input as {}); return this.activeRepository.save(entity); }

  /** Applies an intention-revealing update to a jobs record. */
  async updateJobsById(id: string, input: SuperadminJobsUpdateInput): Promise<SuperadminJobsEntity> { await this.activeRepository.update({ id } as never, input as never); return this.findByIdOrThrow(id); }
  ...
  async retryJobById(id: string): Promise<SuperadminJobsEntity> { const entity = await this.findByIdOrThrow(id); await this.activeRepository.update({ id } as never, { status: 'ACTIVE', attempts: entity.attempts + 1, error: null } as never); return this.findByIdOrThrow(id); }

  /** Cancels one job without deleting its audit history. */
  async cancelJobById(id: string): Promise<SuperadminJobsEntity> { await this.findByIdOrThrow(id); await this.activeRepository.update({ id } as never, { status: 'CANCELLED' } as never); return this.findByIdOrThrow(id); }
