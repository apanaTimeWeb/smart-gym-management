...
/**
 * Primary Intent: Defines SuperadminAnalyticsInsightsQueryController as an explicit backend construct in its owning role/module boundary.
...
/**
 * Primary Intent: Executes the retentionInsights use case within the owning backend feature boundary.
 * Edge Cases: Invalid inputs, missing resources, authorization failures, tenant mismatches, retries, and concurrent state are handled according to the feature contract.
 * Side-Effects: Persists only through the approved repository/orchestrator path and emits declared events/jobs when the feature requires them.
 * AI-Note: Preserve the method's explicit return type, guard-clause structure, dependency isolation, and frontend-frozen contract.
 */
  ...
  // SLA: FAST
@Get(['superadmin/analytics/retention-insights', 'superadmin/analytics/retention-insights'])
  @ApiResponse({ type: SuperadminAnalyticsRetentionInsightsResponseDto })
  @ApiOperation({ summary: 'retentionInsights' })
  /**
   * Primary Intent: Executes the retentionInsights use case within its owning backend boundary.
   * Edge Cases: Invalid input, missing resources, authorization failures, tenant mismatches, retries, and concurrency are handled according to the owning contract.
   * Side-Effects: Only documented persistence, cache, event, job, or external effects are permitted.
   * AI-Note: Preserve explicit return types, guard clauses, module isolation, and frozen API semantics.
   */
