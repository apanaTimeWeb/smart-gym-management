 ...
 * Primary Intent: Executes the getCohort use case within its owning backend boundary.
   ...
   * AI-Note: Preserve explicit return types, guard clauses, module isolation, and frozen API semantics.
   */
  private async getCohort(): Promise<SuperadminAnalyticsCohortRow[]> { return this.dataSource.query<SuperadminAnalyticsCohortRow[]>(`WITH cohort AS (SELECT DATE_TRUNC('month',created_at) cohort_month, COALESCE(last_active_at, CASE WHEN status='ACTIVE' THEN CURRENT_TIMESTAMP ELSE updated_at END) observed_at FROM tenants WHERE deleted_at IS NULL) SELECT TO_CHAR(cohort_month,'Mon YYYY') AS month, COUNT(*)::int total, ROUND(100.0*COUNT(*) FILTER (WHERE observed_at >= cohort_month + INTERVAL '1 month')/COUNT(*),1) m1, ROUND(100.0*COUNT(*) FILTER (WHERE observed_at >= cohort_month + INTERVAL '2 months')/COUNT(*),1) m2, ROUND(100.0*COUNT(*) FILTER (WHERE observed_at >= cohort_month + INTERVAL '3 months')/COUNT(*),1) m3, ROUND(100.0*COUNT(*) FILTER (WHERE observed_at >= cohort_month + INTERVAL '6 months')/COUNT(*),1) m6, ROUND(100.0*COUNT(*) FILTER (WHERE observed_at >= cohort_month + INTERVAL '12 months')/COUNT(*),1) m12 FROM cohort GROUP BY cohort_month ORDER BY cohort_month DESC LIMIT 12`); }
  /**
