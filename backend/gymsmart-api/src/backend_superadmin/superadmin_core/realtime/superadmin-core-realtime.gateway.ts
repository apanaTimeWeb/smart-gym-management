...
export class SuperadminRealtimeGateway implements OnGatewayConnection, OnGatewayDisconnect, OnModuleInit, OnModuleDestroy {
  @WebSocketServer() private server!: Server;
  private readonly subscriber: ReturnType<SuperadminRedisService['getClient']>;

  constructor(private readonly jwt: JwtService, private readonly config: ConfigService, private readonly redis: SuperadminRedisService, private readonly logger: PinoLogger) {
  ...
  handleConnection(client: Socket): void {
    try {
      ...
      this.logger.warn({ err: error, context: SuperadminRealtimeGateway.name }, 'WebSocket authentication rejected');
