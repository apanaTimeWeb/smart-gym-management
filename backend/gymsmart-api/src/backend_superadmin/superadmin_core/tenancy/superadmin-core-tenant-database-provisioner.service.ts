...
import { SuperadminTenantSchemaMigration202609210001 } from '@/backend_superadmin/superadmin_core/tenancy/superadmin-core-tenant-schema-migration';

@Injectable()
export class SuperadminTenantDatabaseProvisionerService {
  ...
  async provision(tenantId: string): Promise<string> {
    ...
    const tenant = new DataSource({
      ...
      entities: [],
      migrations: [SuperadminTenantSchemaMigration202609210001],
