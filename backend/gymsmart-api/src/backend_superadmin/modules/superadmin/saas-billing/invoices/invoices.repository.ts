...
// FLOW: invoices service -> InvoicesRepository -> TypeORM Repository<InvoicesEntity> -> PostgreSQL `saas_invoices`.
...
export class InvoicesRepository extends BaseRepository<InvoicesEntity> {
  constructor(@InjectRepository(InvoicesEntity) repository: Repository<InvoicesEntity>, transactionContext: TransactionContext) { super(repository, transactionContext); }

  /** Returns a validated, ordered, filtered page of active feature records. */
  async findPage(query: InvoicesListQuery): Promise<{ items: InvoicesEntity[]; total: number }> {
  ...
  async findById(id: string): Promise<InvoicesEntity | null> { return super.findById(id); }

  /** Returns one active record by id and throws when absent. */
  async findByIdOrThrow(id: string): Promise<InvoicesEntity> { return super.findByIdOrThrow(id, 'Invoices record not found'); }

  /** Creates and persists a invoices record. */
  async createInvoices(input: InvoicesCreateInput): Promise<InvoicesEntity> { const entity = this.activeRepository.create(input as {}); return this.activeRepository.save(entity); }

  /** Applies an intention-revealing update to a invoices record. */
  async updateInvoicesById(id: string, input: InvoicesUpdateInput): Promise<InvoicesEntity> { await this.activeRepository.update({ id } as never, input as never); return this.findByIdOrThrow(id); }
