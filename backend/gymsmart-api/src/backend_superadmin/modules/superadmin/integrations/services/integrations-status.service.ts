...
export class IntegrationsStatusService {
  ...
  async changeIntegrationsStatus(id: string, status: string): Promise<IntegrationsResponseDto> { return IntegrationsMapper.toResponse(IntegrationsMapper.toDomain(await this.repository.updateIntegrationsById(id, { status: status as IntegrationKeyStatus }))); }
}