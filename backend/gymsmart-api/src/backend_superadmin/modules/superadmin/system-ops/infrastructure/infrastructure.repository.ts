...
// FLOW: infrastructure service -> InfrastructureRepository -> TypeORM Repository<InfrastructureEntity> -> PostgreSQL `infrastructure_nodes`.
...
export class InfrastructureRepository extends BaseRepository<InfrastructureEntity> {
  constructor(@InjectRepository(InfrastructureEntity) repository: Repository<InfrastructureEntity>, transactionContext: TransactionContext) { super(repository, transactionContext); }

  /** Returns a validated, ordered, filtered page of active feature records. */
  async findPage(query: InfrastructureListQuery): Promise<{ items: InfrastructureEntity[]; total: number }> {
  ...
  async findById(id: string): Promise<InfrastructureEntity | null> { return super.findById(id); }

  /** Returns one active record by id and throws when absent. */
  async findByIdOrThrow(id: string): Promise<InfrastructureEntity> { return super.findByIdOrThrow(id, 'Infrastructure record not found'); }

  /** Creates and persists a infrastructure record. */
  async createInfrastructure(input: InfrastructureCreateInput): Promise<InfrastructureEntity> { const entity = this.activeRepository.create(input as {}); return this.activeRepository.save(entity); }

  /** Applies an intention-revealing update to a infrastructure record. */
  async updateInfrastructureById(id: string, input: InfrastructureUpdateInput): Promise<InfrastructureEntity> { await this.activeRepository.update({ id } as never, input as never); return this.findByIdOrThrow(id); }
