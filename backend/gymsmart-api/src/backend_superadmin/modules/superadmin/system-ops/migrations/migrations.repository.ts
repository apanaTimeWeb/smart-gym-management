...
// FLOW: migrations service -> MigrationsRepository -> TypeORM Repository<MigrationLogEntity> -> PostgreSQL `migration_logs`.
...
import type { MigrationsListQuery, MigrationsCreateInput, MigrationsUpdateInput } from '@/backend_superadmin/modules/superadmin/system-ops/migrations/types/migrations.interfaces';

@Injectable()
export class MigrationsRepository extends BaseRepository<MigrationLogEntity> {
  constructor(@InjectRepository(MigrationLogEntity) repository: Repository<MigrationLogEntity>, transactionContext: TransactionContext) { super(repository, transactionContext); }

  /** Returns a validated, ordered, filtered page of active feature records. */
  async findPage(query: MigrationsListQuery): Promise<{ items: MigrationLogEntity[]; total: number }> {
  ...
  async findById(id: string): Promise<MigrationLogEntity | null> { return super.findById(id); }
  ...
  async createMigrations(input: MigrationsCreateInput): Promise<MigrationLogEntity> { const entity = this.activeRepository.create(input as {}); return this.activeRepository.save(entity as any) as any; }
