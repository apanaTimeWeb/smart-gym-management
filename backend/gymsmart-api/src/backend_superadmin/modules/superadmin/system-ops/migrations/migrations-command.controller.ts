// RESPONSIBILITY: Owns POST/PATCH/DELETE endpoints for the migrations feature; business logic stays in micro-services.
...
import { MigrationsCreateDto } from '@/backend_superadmin/modules/superadmin/system-ops/migrations/dtos/migrations-create.dto';
...
import { MigrationsUpdateDto } from '@/backend_superadmin/modules/superadmin/system-ops/migrations/dtos/migrations-update.dto';
...
export class MigrationsCommandController {
  ...
  @Patch(':id/status')
    @UseGuards(RateLimitGuard)
    async changeStatus(@Param('id') id: string, @Body() body: { status: string }): Promise<unknown> { return this.statusService.changeMigrationsStatus(id, body.status); }
