...
export class SystemOpsRepository {
  constructor(@InjectRepository(SystemOpsContractSnapshotEntity) private readonly repository: Repository<SystemOpsContractSnapshotEntity>) {}
  ...
  async upsertSummary(payload: unknown): Promise<void> {
    ...
    await this.repository.insert(this.repository.create({ kind: 'summary', payload } as any));
  }
}
