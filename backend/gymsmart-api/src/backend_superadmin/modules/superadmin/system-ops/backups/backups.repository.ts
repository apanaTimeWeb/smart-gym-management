...
// FLOW: backups service -> BackupsRepository -> TypeORM Repository<BackupRecordEntity> -> PostgreSQL `backup_records`.
...
@Injectable()
export class BackupsRepository extends BaseRepository<BackupRecordEntity> {
  constructor(@InjectRepository(BackupRecordEntity) repository: Repository<BackupRecordEntity>, transactionContext: TransactionContext) { super(repository, transactionContext); }

  /** Returns a validated, ordered, filtered page of active feature records. */
  async findPage(query: BackupsListQuery): Promise<{ items: BackupRecordEntity[]; total: number }> {
  ...
  async findById(id: string): Promise<BackupRecordEntity | null> { return super.findById(id); }
  ...
  async createBackups(input: BackupsCreateInput): Promise<BackupRecordEntity> { const entity = this.activeRepository.create(input as {}); return this.activeRepository.save(entity as any) as any; }
  ...
  async findByTenantId(tenantId: string): Promise<BackupRecordEntity[]> { return this.activeRepository.find({ where: { tenantId, deletedAt: null } as never, order: { updatedAt: 'DESC' } as never }); }
