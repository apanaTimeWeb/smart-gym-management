...
export class FeaturesReleaseNoteService {
  ...
  async update(id: string, input: FeaturesReleaseNoteUpdateDto): Promise<ReturnType<FeaturesReleaseNoteService['toPublic']>> { await this.repository.findByIdOrThrow(id); const row = await this.repository.updateById(id, input); return this.toPublic(row); }
