...
export class MessagingStatusService {
  ...
  async changeMessagingStatus(id: string, status: string): Promise<MessagingResponseDto> { return MessagingMapper.toResponse(MessagingMapper.toDomain(await this.repository.updateMessagingById(id, { status: status as TenantMessageStatus }))); }
}