...
export class AffiliatesPayoutService {
  ...
  async history(): Promise<AffiliatePayoutRecordDto[]> { const rows = await this.repository.findPayoutHistory(); if (!rows.length) throw new NotFoundException('No payout history found'); return rows as unknown as AffiliatePayoutRecordDto[]; }
}
