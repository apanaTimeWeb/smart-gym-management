...
// FLOW: profile service -> ProfileRepository -> TypeORM Repository<ProfileEntity> -> PostgreSQL `superadmin_profiles`.
...
export class ProfileRepository extends BaseRepository<ProfileEntity> {
  constructor(@InjectRepository(ProfileEntity) repository: Repository<ProfileEntity>, transactionContext: TransactionContext) { super(repository, transactionContext); }

  /** Returns a validated, ordered, filtered page of active feature records. */
  async findPage(query: ProfileListQuery): Promise<{ items: ProfileEntity[]; total: number }> {
  ...
  async findById(id: string): Promise<ProfileEntity | null> { return super.findById(id); }

  /** Returns one active record by id and throws when absent. */
  async findByIdOrThrow(id: string): Promise<ProfileEntity> { return super.findByIdOrThrow(id, 'Profile record not found'); }

  /** Creates and persists a profile record. */
  async createProfile(input: ProfileCreateInput): Promise<ProfileEntity> { const entity = this.activeRepository.create(input as {}); return this.activeRepository.save(entity); }

  /** Applies an intention-revealing update to a profile record. */
  async updateProfileById(id: string, input: ProfileUpdateInput): Promise<ProfileEntity> { await this.activeRepository.update({ id } as never, input as never); return this.findByIdOrThrow(id); }
