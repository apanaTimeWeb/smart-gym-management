...
// FLOW: affiliates service -> AffiliatesRepository -> TypeORM Repository<AffiliatesEntity> -> PostgreSQL `affiliates`.
...
export class AffiliatesRepository extends BaseRepository<AffiliatesEntity> {
  constructor(@InjectRepository(AffiliatesEntity) repository: Repository<AffiliatesEntity>, transactionContext: TransactionContext) { super(repository, transactionContext); }

  /** Returns a validated, ordered, filtered page of active feature records. */
  async findPage(query: AffiliatesListQuery): Promise<{ items: AffiliatesEntity[]; total: number }> {
  ...
  async findById(id: string): Promise<AffiliatesEntity | null> { return super.findById(id); }

  /** Returns one active record by id and throws when absent. */
  async findByIdOrThrow(id: string): Promise<AffiliatesEntity> { return super.findByIdOrThrow(id, 'Affiliates record not found'); }

  /** Creates and persists a affiliates record. */
  async createAffiliates(input: AffiliatesCreateInput): Promise<AffiliatesEntity> { const entity = this.activeRepository.create({ name: input.name ?? '', email: input.email ?? '', phone: input.phone ?? '', referralCode: input.referralCode ?? '', totalReferred: 0, commissionEarned: 0, commissionRate: 0, pendingPayout: 0, bankDetails: {}, status: 'ACTIVE', joinedAt: input.joinedAt ?? new Date(), referralCount: 0, conversionRate: 0, payoutHistory: [] } as never); return this.activeRepository.save(entity as any); }

  /** Applies an intention-revealing update to a affiliates record. */
  async updateAffiliatesById(id: string, input: AffiliatesUpdateInput): Promise<AffiliatesEntity> { await this.activeRepository.update({ id } as never, input as never); return this.findByIdOrThrow(id); }
  ...
  async findPayoutHistory(): Promise<unknown[]> { const rows = await this.activeRepository.find({ where: { deletedAt: null } as never, order: { updatedAt: 'DESC' } as never }); return rows.flatMap((item: AffiliatesEntity) => Array.isArray(item.payoutHistory) ? item.payoutHistory : []); }
