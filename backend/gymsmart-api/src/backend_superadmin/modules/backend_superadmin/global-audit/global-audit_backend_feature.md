...
## Module Purpose
Owns the `global-audit` Superadmin feature and its frontend-aligned API contract. It keeps validation, business decisions, persistence, and response mapping in separate files so an AI can repair the feature without loading unrelated business modules. All data access uses the project-approved PostgreSQL/TypeORM repository boundary.
...
| File | Responsibility |
|---|---|
| `dtos/global-audit-create.dto.ts` | Validates one request or response contract at the module edge. |
| `dtos/global-audit-query.dto.ts` | Validates one request or response contract at the module edge. |
| `dtos/global-audit-update.dto.ts` | Validates one request or response contract at the module edge. |
| `global-audit-command.controller.ts` | Exposes the HTTP boundary and forwards requests to one or more local use-case services. |
| `global-audit-contract-snapshot.entity.ts` | Maps one PostgreSQL table or contract-snapshot table to TypeORM. |
| `global-audit-contract-snapshot.repository.ts` | Owns TypeORM queries and intention-revealing persistence mutations for this feature. |
| `global-audit-investigation-response.dto.ts` | Validates one request or response contract at the module edge. |
| `global-audit-query.controller.ts` | Exposes the HTTP boundary and forwards requests to one or more local use-case services. |
| `global-audit-special.controller.ts` | Exposes the HTTP boundary and forwards requests to one or more local use-case services. |
| `global-audit.constants.ts` | Documents the feature boundary, dependencies, forbidden operations, or API contract. |
| `global-audit.entity.ts` | Maps one PostgreSQL table or contract-snapshot table to TypeORM. |
| `global-audit.exceptions.ts` | Documents the feature boundary, dependencies, forbidden operations, or API contract. |
| `global-audit.mapper.ts` | Translates persistence entities to domain-safe values without leaking ORM concerns. |
| `global-audit.module.ts` | Registers this feature's controllers, providers, repositories, and TypeORM entities. |
| `global-audit.repository.ts` | Owns TypeORM queries and intention-revealing persistence mutations for this feature. |
| `global-audit.seeder.ts` | Documents the feature boundary, dependencies, forbidden operations, or API contract. |
| `global-audit_backend_feature.md` | Documents the feature boundary, dependencies, forbidden operations, or API contract. |
| `global-audit_collection.json` | Documents the feature boundary, dependencies, forbidden operations, or API contract. |
| `global-audit_dependencies.md` | Documents the feature boundary, dependencies, forbidden operations, or API contract. |
| `global-audit_forbidden.md` | Documents the feature boundary, dependencies, forbidden operations, or API contract. |
| `responses/global-audit-response.dto.ts` | Validates one request or response contract at the module edge. |
| `services/global-audit-create.service.ts` | Owns one focused business use case and contains no ORM query construction. |
| `services/global-audit-delete.service.ts` | Owns one focused business use case and contains no ORM query construction. |
| `services/global-audit-find.service.ts` | Owns one focused business use case and contains no ORM query construction. |
| `services/global-audit-investigation.service.ts` | Owns one focused business use case and contains no ORM query construction. |
| `services/global-audit-list.service.ts` | Owns one focused business use case and contains no ORM query construction. |
| `services/global-audit-update.service.ts` | Owns one focused business use case and contains no ORM query construction. |
| `types/global-audit.enums.ts` | Documents the feature boundary, dependencies, forbidden operations, or API contract. |
| `types/global-audit.interfaces.ts` | Documents the feature boundary, dependencies, forbidden operations, or API contract. |

## Feature Inventory
| Controller/Endpoint | HTTP | Path | Purpose | Request DTO | Response DTO |
|---|---|---|---|---|---|
| `global-audit-command.controller.ts` / `create` | POST | `/superadmin/global-audit` | Creates a resource after DTO validation and persists it through the feature repository. | `GlobalAuditCreateDto` | `unknown` |
| `global-audit-command.controller.ts` / `update` | PATCH | `/superadmin/global-audit/:id` | Updates only the fields permitted by the feature DTO and returns the refreshed resource. | `GlobalAuditUpdateDto` | `unknown` |
| `global-audit-command.controller.ts` / `remove` | DELETE | `/superadmin/global-audit/:id` | Soft-deletes the resource and keeps the historical row recoverable. | `None` | `void` |
| `global-audit-query.controller.ts` / `findAll` | GET | `/superadmin/global-audit` | Returns a paginated collection using the feature query contract. | `None` | `unknown` |
| `global-audit-query.controller.ts` / `findOne` | GET | `/superadmin/global-audit/:id` | Returns one active resource after resource and authorization checks. | `None` | `unknown` |
| `global-audit-special.controller.ts` / `investigation` | GET | `/superadmin/global-audit/investigation` | Returns audit investigation changes, anomalies, and filter vocabulary. | `None` | `Record<string, unknown` |
...
CODEOWNERS path: `src/modules/backend_superadmin/global-audit/` -> the Superadmin reviewers defined by `CODEOWNERS`.
