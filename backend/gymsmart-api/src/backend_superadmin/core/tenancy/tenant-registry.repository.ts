...
import { TenantRegistryNotFoundException } from '@/backend_superadmin/core/tenancy/tenant-registry.exceptions';

@Injectable()
export class TenantRegistryRepository {
  constructor(private readonly dataSource: DataSource, private readonly transactionContext: TransactionContext) {}
  ...
  async findByIdOrThrow(id: string): Promise<{ id: string; databaseName: string }> {
    ...
    if (!tenant) throw new TenantRegistryNotFoundException();
    return tenant;
  }
