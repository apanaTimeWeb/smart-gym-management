...
export class TenantDatasourceResolverService implements OnModuleDestroy {
  ...
  constructor(private readonly config: ConfigService, private readonly authorization: TenantAuthorizationService) {}
  ...
  private async createTenantDataSource(tenantId: string): Promise<DataSource> {
    ...
    const dataSource = new DataSource({ type: 'postgres', url: baseUrl.toString(), synchronize: false, migrationsRun: false, entities: [join(__dirname, '../../modules/**/*.entity.{js,ts}')], extra: this.poolOptions() });
