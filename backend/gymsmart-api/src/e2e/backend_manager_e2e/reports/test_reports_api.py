# RESPONSIBILITY: Black-box API contract checks for the Manager reports feature.
...
def test_get_reports_summary_contract():
    response = _call('GET', 'reports/summary')
...
def test_get_reports_export_contract():
    response = _call('GET', 'reports/export')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
