# RESPONSIBILITY: Black-box API contract checks for the Manager profile feature.
...
def test_patch_profile_contract():
    response = _call('PATCH', 'profile')
...
def test_patch_profile_password_contract():
    response = _call('PATCH', 'profile/password')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
