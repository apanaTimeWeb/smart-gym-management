# RESPONSIBILITY: Black-box API contract checks for the Manager notifications feature.
...
def test_get_notifications_contract():
    response = _call('GET', 'notifications')
...
def test_get_notifications_kpis_contract():
    response = _call('GET', 'notifications/kpis')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_patch_notifications_id_read_param_contract():
    response = _call('PATCH', 'notifications/:id/read')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_patch_notifications_read_all_contract():
    response = _call('PATCH', 'notifications/read-all')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_delete_notifications_id_param_contract():
    response = _call('DELETE', 'notifications/:id')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
