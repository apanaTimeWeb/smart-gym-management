# RESPONSIBILITY: Black-box API contract checks for the Manager attendance feature.
...
def test_post_attendance_contract():
    response = _call('POST', 'attendance')
...
def test_get_attendance_contract():
    response = _call('GET', 'attendance')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_attendance_stats_contract():
    response = _call('GET', 'attendance/stats')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_attendance_history_contract():
    response = _call('GET', 'attendance/history')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_attendance_members_contract():
    response = _call('GET', 'attendance/members')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_attendance_staff_contract():
    response = _call('GET', 'attendance/staff')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
