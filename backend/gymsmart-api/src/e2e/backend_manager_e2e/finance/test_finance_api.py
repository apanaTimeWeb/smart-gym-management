# RESPONSIBILITY: Black-box API contract checks for the Manager finance feature.
...
def test_get_finance_payments_contract():
    response = _call('GET', 'finance/payments')
...
def test_get_finance_payments_member_memberId_param_contract():
    response = _call('GET', 'finance/payments/member/:memberId')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_finance_summary_contract():
    response = _call('GET', 'finance/summary')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_finance_export_contract():
    response = _call('GET', 'finance/export')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_post_finance_payments_contract():
    response = _call('POST', 'finance/payments')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
