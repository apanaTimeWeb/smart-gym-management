# RESPONSIBILITY: Black-box API contract checks for the Manager maintenance feature.
...
def test_get_manager_maintenance_contract():
    response = _call('GET', '/manager/maintenance')
...
def test_post_manager_maintenance_contract():
    response = _call('POST', '/manager/maintenance')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_post_manager_maintenance_id_resolve_param_contract():
    response = _call('POST', '/manager/maintenance/:id/resolve')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
