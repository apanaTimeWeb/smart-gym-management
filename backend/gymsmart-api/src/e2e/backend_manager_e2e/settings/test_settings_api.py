# RESPONSIBILITY: Black-box API contract checks for the Manager settings feature.
...
def test_patch_settings_contract():
    response = _call('PATCH', 'settings')
...
def test_get_settings_contract():
    response = _call('GET', 'settings')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
