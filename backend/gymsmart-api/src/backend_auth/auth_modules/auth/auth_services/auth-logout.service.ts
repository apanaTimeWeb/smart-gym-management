...
export class AuthLogoutService {
  ...
  async logout(userId: string, role: AuthRole, sessionId: string): Promise<void> {
    ...
    await this.audit.record({ actorId: userId, actorRole: AuthAuditRoleMapper(role), action: AuthConstants.AUDIT.LOGOUT, entityType: 'auth_refresh_session', entityId: sessionId, oldValue: {}, newValue: { revoked: true }, ipAddress: this.audit.getRequestMetadata().ipAddress });
  }
}
