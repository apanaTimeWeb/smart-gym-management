...
@Injectable()
/**
 * @description Defines the AdminNotificationsRepository boundary for the admin_notifications backend feature.
 * @remarks Keep this class focused on its declared responsibility; preserve tenant, contract, security, and AI-context invariants when modifying it.
 */
export class AdminNotificationsRepository extends AdminCoreTenantRepositoryBase<AdminNotificationsEntity> {
  constructor(tenantManager: AdminCoreTenantDataSourceManager, private readonly mapper: AdminNotificationsMapper) { super(tenantManager); }
   ...
   */
  async findAll(query: AdminNotificationsQueryDto): Promise<AdminCorePaginatedResult<AdminNotificationsDomainModel>> {
    const repository = await this.repositoryFor(AdminNotificationsEntity);
   ...
   */
  async findById(id: string): Promise<AdminNotificationsDomainModel | null> {
    const repository = await this.repositoryFor(AdminNotificationsEntity);
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminNotificationsDomainModel> {
    const domain = await this.findById(id);
    if (!domain) throw new NotFoundException(`notifications record not found.`);
    return domain;
  }
   ...
   */
  private async findEntityByIdOrThrow(id: string): Promise<AdminNotificationsEntity> {
    const repository = await this.repositoryFor(AdminNotificationsEntity);
    const entity = await repository.findOne({ where: { id } });
    if (!entity) throw new NotFoundException(`notifications record not found.`);
   ...
   */
  async createRecord(input: AdminNotificationsMutationDto): Promise<AdminNotificationsDomainModel> {
    const repository = await this.repositoryFor(AdminNotificationsEntity);
   ...
   */
  async updateById(id: string, input: AdminNotificationsMutationDto): Promise<AdminNotificationsDomainModel> {
    const repository = await this.repositoryFor(AdminNotificationsEntity);
   ...
   * @returns No value.
   */
  async markAsDeleted(id: string): Promise<AdminNotificationsDomainModel> {
    const repository = await this.repositoryFor(AdminNotificationsEntity);
    const entity = await this.findEntityByIdOrThrow(id);
    this.captureMutationBefore(entity);
   ...
   */
  async updateReadById(id: string): Promise<AdminNotificationsDomainModel> {
    const entity = await this.findEntityByIdOrThrow(id);
    this.captureMutationBefore(entity);
    const repository = await this.repositoryFor(AdminNotificationsEntity);
    entity.read = true;
    entity.payload = { ...entity.payload, read: true };
    return this.mapper.toDomain(await repository.save(entity));
  }

  /** @description Marks all current-tenant notifications as read.
   * @returns Count of changed records.
   */
  async updateAllRead(): Promise<number> {
    const repository = await this.repositoryFor(AdminNotificationsEntity);
    const result = await repository.createQueryBuilder()
      .update(AdminNotificationsEntity).set({ read: true })
      .where('read = :read', { read: false }).execute();
  ...
  async findLatestSnapshot(query?: AdminNotificationsQueryDto): Promise<AdminNotificationsDomainModel | null> {
    const repository = await this.repositoryFor(AdminNotificationsEntity);
