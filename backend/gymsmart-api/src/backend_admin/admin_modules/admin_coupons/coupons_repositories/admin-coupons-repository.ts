...
@Injectable()
/**
 * @description Defines the AdminCouponsRepository boundary for the admin_coupons backend feature.
 * @remarks Keep this class focused on its declared responsibility; preserve tenant, contract, security, and AI-context invariants when modifying it.
 */
export class AdminCouponsRepository extends AdminCoreTenantRepositoryBase<AdminCouponsEntity> {
  constructor(tenantManager: AdminCoreTenantDataSourceManager, private readonly mapper: AdminCouponsMapper) { super(tenantManager); }
   ...
   */
  async findAll(query: AdminCouponsQueryDto): Promise<AdminCorePaginatedResult<AdminCouponsDomainModel>> {
    const repository = await this.repositoryFor(AdminCouponsEntity);
    const builder = repository.createQueryBuilder('entity');
    const window = resolveCouponsAdminQueryWindow(query);
   ...
   */
  async findById(id: string): Promise<AdminCouponsDomainModel | null> {
    const repository = await this.repositoryFor(AdminCouponsEntity);
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminCouponsDomainModel> {
    const domain = await this.findById(id);
    if (!domain) throw new NotFoundException(`coupons record not found.`);
    return domain;
  }
   ...
   */
  private async findEntityByIdOrThrow(id: string): Promise<AdminCouponsEntity> {
    const repository = await this.repositoryFor(AdminCouponsEntity);
    const entity = await repository.findOne({ where: { id } });
    if (!entity) throw new NotFoundException(`coupons record not found.`);
   ...
   */
  async createRecord(input: AdminCouponsMutationDto): Promise<AdminCouponsDomainModel> {
    const repository = await this.repositoryFor(AdminCouponsEntity);
   ...
   */
  async updateById(id: string, input: AdminCouponsMutationDto): Promise<AdminCouponsDomainModel> {
    const repository = await this.repositoryFor(AdminCouponsEntity);
   ...
   * @returns No value.
   */
  async deleteCoupon(id: string): Promise<AdminCouponsDomainModel> {
    const repository = await this.repositoryFor(AdminCouponsEntity);
    const entity = await this.findEntityByIdOrThrow(id);
    this.captureMutationBefore(entity);
   ...
   * @returns Updated domain model.
   */
  async updateCouponActiveById(id: string): Promise<AdminCouponsDomainModel> {
    const entity = await this.findEntityByIdOrThrow(id);
    this.captureMutationBefore(entity);
    const repository = await this.repositoryFor(AdminCouponsEntity);
  ...
  async findLatestReadModel(query?: AdminCouponsQueryDto): Promise<AdminCouponsDomainModel | null> {
    const repository = await this.repositoryFor(AdminCouponsEntity);
