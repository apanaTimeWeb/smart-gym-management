...
@Injectable()
/**
 * @description Defines the AdminSettingsRepository boundary for the admin_settings backend feature.
 * @remarks Keep this class focused on its declared responsibility; preserve tenant, contract, security, and AI-context invariants when modifying it.
 */
export class AdminSettingsRepository extends AdminCoreTenantRepositoryBase<AdminSettingsEntity> {
  constructor(tenantManager: AdminCoreTenantDataSourceManager, private readonly mapper: AdminSettingsMapper) { super(tenantManager); }
   ...
   */
  async findAll(query: AdminSettingsQueryDto): Promise<AdminCorePaginatedResult<AdminSettingsDomainModel>> {
    const repository = await this.repositoryFor(AdminSettingsEntity);
   ...
   */
  async findById(id: string): Promise<AdminSettingsDomainModel | null> {
    const repository = await this.repositoryFor(AdminSettingsEntity);
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminSettingsDomainModel> {
    const domain = await this.findById(id);
    if (!domain) throw new NotFoundException(`settings record not found.`);
    return domain;
  }
   ...
   */
  private async findEntityByIdOrThrow(id: string): Promise<AdminSettingsEntity> {
    const repository = await this.repositoryFor(AdminSettingsEntity);
    const entity = await repository.findOne({ where: { id } });
    if (!entity) throw new NotFoundException(`settings record not found.`);
   ...
   */
  async createRecord(input: AdminSettingsMutationDto): Promise<AdminSettingsDomainModel> {
    const repository = await this.repositoryFor(AdminSettingsEntity);
   ...
   */
  async updateById(id: string, input: Partial<AdminSettingsMutationDto>): Promise<AdminSettingsDomainModel> {
    const repository = await this.repositoryFor(AdminSettingsEntity);
  ...
  async markAsDeleted(id: string): Promise<void> {
    await this.softDeleteRecord(id, AdminSettingsEntity);
  }
  ...
  async findLatestSnapshot(query?: AdminSettingsQueryDto): Promise<AdminSettingsDomainModel | null> {
    const repository = await this.repositoryFor(AdminSettingsEntity);
