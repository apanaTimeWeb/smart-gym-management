...
@Injectable()
/**
 * @description Defines the AdminBlacklistRepository boundary for the admin_blacklist backend feature.
 * @remarks Keep this class focused on its declared responsibility; preserve tenant, contract, security, and AI-context invariants when modifying it.
 */
export class AdminBlacklistRepository extends AdminCoreTenantRepositoryBase<AdminBlacklistEntity> {
  constructor(tenantManager: AdminCoreTenantDataSourceManager, private readonly mapper: AdminBlacklistMapper) { super(tenantManager); }
   ...
   */
  async findAll(query: AdminBlacklistQueryDto): Promise<AdminCorePaginatedResult<AdminBlacklistDomainModel>> {
    const repository = await this.repositoryFor(AdminBlacklistEntity);
   ...
   */
  async findById(id: string): Promise<AdminBlacklistDomainModel | null> {
    const repository = await this.repositoryFor(AdminBlacklistEntity);
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminBlacklistDomainModel> {
    const domain = await this.findById(id);
    if (!domain) throw new NotFoundException(`blacklist record not found.`);
    return domain;
  }
   ...
   */
  private async findEntityByIdOrThrow(id: string): Promise<AdminBlacklistEntity> {
    const repository = await this.repositoryFor(AdminBlacklistEntity);
    const entity = await repository.findOne({ where: { id } });
   ...
   */
  async createRecord(input: AdminBlacklistMutationDto): Promise<AdminBlacklistDomainModel> {
    const repository = await this.repositoryFor(AdminBlacklistEntity);
   ...
   */
  async updateById(id: string, input: AdminBlacklistMutationDto): Promise<AdminBlacklistDomainModel> {
    const repository = await this.repositoryFor(AdminBlacklistEntity);
   ...
   * @returns Updated domain model.
   */
  async updateActiveById(id: string): Promise<AdminBlacklistDomainModel> {
    const entity = await this.findEntityByIdOrThrow(id);
    this.captureMutationBefore(entity);
    const repository = await this.repositoryFor(AdminBlacklistEntity);
   ...
   */
  async updatePropagationById(id: string): Promise<AdminBlacklistDomainModel> {
    const entity = await this.findEntityByIdOrThrow(id);
    this.captureMutationBefore(entity);
    const repository = await this.repositoryFor(AdminBlacklistEntity);
  ...
  async findLatestSnapshot(query?: AdminBlacklistQueryDto): Promise<AdminBlacklistDomainModel | null> {
    const repository = await this.repositoryFor(AdminBlacklistEntity);
