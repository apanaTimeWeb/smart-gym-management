...
export interface SalesAdminQueryWindow {
  from?: Date;
  toExclusive?: Date;
}
 ...
 */
export function resolveSalesAdminQueryWindow(input: { startDate?: string; endDate?: string; dateFrom?: string; dateTo?: string; range?: string; dateRange?: string; period?: string; month?: string }): SalesAdminQueryWindow {
  const now = new Date();
  ...
  if (from && toExclusive && from >= toExclusive) throw new Error('SALES.QUERY.INVALID_DATE_RANGE');
