...
export interface PlansAdminQueryWindow {
  from?: Date;
  toExclusive?: Date;
}
 ...
 */
export function resolvePlansAdminQueryWindow(input: { startDate?: string; endDate?: string; dateFrom?: string; dateTo?: string; range?: string; dateRange?: string; period?: string; month?: string }): PlansAdminQueryWindow {
  const now = new Date();
  ...
  if (from && toExclusive && from >= toExclusive) throw new Error('PLANS.QUERY.INVALID_DATE_RANGE');
