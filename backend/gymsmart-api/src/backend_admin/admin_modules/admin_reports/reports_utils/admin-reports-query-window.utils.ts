...
export interface ReportsAdminQueryWindow {
  from?: Date;
  toExclusive?: Date;
}
 ...
 */
export function resolveReportsAdminQueryWindow(input: { startDate?: string; endDate?: string; dateFrom?: string; dateTo?: string; range?: string; dateRange?: string; period?: string; month?: string }): ReportsAdminQueryWindow {
  const now = new Date();
  ...
  if (from && toExclusive && from >= toExclusive) throw new Error('REPORTS.QUERY.INVALID_DATE_RANGE');
