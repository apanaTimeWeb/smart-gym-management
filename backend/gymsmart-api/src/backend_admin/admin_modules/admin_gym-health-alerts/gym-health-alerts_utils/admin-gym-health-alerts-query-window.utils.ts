...
export interface GymHealthAlertsAdminQueryWindow {
  from?: Date;
  toExclusive?: Date;
}
 ...
 */
export function resolveGymHealthAlertsAdminQueryWindow(input: { startDate?: string; endDate?: string; dateFrom?: string; dateTo?: string; range?: string; dateRange?: string; period?: string; month?: string }): GymHealthAlertsAdminQueryWindow {
  const now = new Date();
  ...
  if (from && toExclusive && from >= toExclusive) throw new Error('GYM_HEALTH_ALERTS.QUERY.INVALID_DATE_RANGE');
