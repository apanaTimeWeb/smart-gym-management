...
@Injectable()
/**
 * @description Defines the AdminGymHealthAlertsRepository boundary for the admin_gym-health-alerts backend feature.
 * @remarks Keep this class focused on its declared responsibility; preserve tenant, contract, security, and AI-context invariants when modifying it.
 */
export class AdminGymHealthAlertsRepository extends AdminCoreTenantRepositoryBase<AdminGymHealthAlertsEntity> {
  constructor(tenantManager: AdminCoreTenantDataSourceManager, private readonly mapper: AdminGymHealthAlertsMapper) { super(tenantManager); }
   ...
   */
  async findAll(query: AdminGymHealthAlertsQueryDto): Promise<AdminCorePaginatedResult<AdminGymHealthAlertsDomainModel>> {
    const repository = await this.repositoryFor(AdminGymHealthAlertsEntity);
    const builder = repository.createQueryBuilder('entity');
    const window = resolveGymHealthAlertsAdminQueryWindow(query);
   ...
   */
  async findById(id: string): Promise<AdminGymHealthAlertsDomainModel | null> {
    const repository = await this.repositoryFor(AdminGymHealthAlertsEntity);
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminGymHealthAlertsDomainModel> {
    const domain = await this.findById(id);
    if (!domain) throw new NotFoundException(`gym-health-alerts record not found.`);
    return domain;
  }
   ...
   */
  private async findEntityByIdOrThrow(id: string): Promise<AdminGymHealthAlertsEntity> {
    const repository = await this.repositoryFor(AdminGymHealthAlertsEntity);
    const entity = await repository.findOne({ where: { id } });
    if (!entity) throw new NotFoundException(`gym-health-alerts record not found.`);
   ...
   */
  async createRecord(input: Record<string, unknown>): Promise<AdminGymHealthAlertsDomainModel> {
    const repository = await this.repositoryFor(AdminGymHealthAlertsEntity);
   ...
   */
  async updateById(id: string, input: Record<string, unknown>): Promise<AdminGymHealthAlertsDomainModel> {
    const repository = await this.repositoryFor(AdminGymHealthAlertsEntity);
   ...
   * @returns No value.
   */
  async markAsDeleted(id: string): Promise<AdminGymHealthAlertsDomainModel> {
    const repository = await this.repositoryFor(AdminGymHealthAlertsEntity);
    const entity = await this.findEntityByIdOrThrow(id);
    this.captureMutationBefore(entity);
   ...
   */
  async updateResolveAlertById(id: string): Promise<AdminGymHealthAlertsDomainModel> {
    const entity = await this.findEntityByIdOrThrow(id);
    this.captureMutationBefore(entity);
    const repository = await this.repositoryFor(AdminGymHealthAlertsEntity);
    entity.payload = { ...entity.payload, isResolved: true, status: 'resolved' };
    entity.status = 'resolved' as any;
    return this.mapper.toDomain(await repository.save(entity));
  }
   ...
   * @returns Updated alert.
   */
  async updateDismissAlertById(id: string): Promise<AdminGymHealthAlertsDomainModel> {
    const entity = await this.findEntityByIdOrThrow(id);
    this.captureMutationBefore(entity);
    const repository = await this.repositoryFor(AdminGymHealthAlertsEntity);
    entity.payload = { ...entity.payload, isResolved: false, status: 'dismissed' };
    entity.status = 'dismissed' as any;
    return this.mapper.toDomain(await repository.save(entity));
  }
  ...
  async findLatestReadModel(query?: AdminGymHealthAlertsQueryDto): Promise<AdminGymHealthAlertsDomainModel | null> {
    const repository = await this.repositoryFor(AdminGymHealthAlertsEntity);
