...
- Frontend Admin `data-export` route/page and its API client operations.
...
### Runtime Events
- `ADMIN.EXPORT.CREATED` — emitted after the export job record and enqueue transaction commits.
- `ADMIN.EXPORT.COMPLETED` — emitted after artifact persistence and completion state are durable.
- `ADMIN.EXPORT.NOTIFICATION_READY` — not emitted by the current worker because no approved delivery consumer is supplied in this backend scope; email/WhatsApp delivery remains an explicitly outside-scope infrastructure dependency.
