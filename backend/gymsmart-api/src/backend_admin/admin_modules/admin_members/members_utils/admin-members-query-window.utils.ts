...
export interface MembersAdminQueryWindow {
  from?: Date;
  toExclusive?: Date;
}
 ...
 */
export function resolveMembersAdminQueryWindow(input: { startDate?: string; endDate?: string; dateFrom?: string; dateTo?: string; range?: string; dateRange?: string; period?: string; month?: string }): MembersAdminQueryWindow {
  const now = new Date();
  ...
  if (from && toExclusive && from >= toExclusive) throw new Error('MEMBERS.QUERY.INVALID_DATE_RANGE');
