...
export interface AttendanceAdminQueryWindow {
  from?: Date;
  toExclusive?: Date;
}
 ...
 */
export function resolveAttendanceAdminQueryWindow(input: { startDate?: string; endDate?: string; dateFrom?: string; dateTo?: string; range?: string; dateRange?: string; period?: string; month?: string }): AttendanceAdminQueryWindow {
  const now = new Date();
  ...
  if (from && toExclusive && from >= toExclusive) throw new Error('ATTENDANCE.QUERY.INVALID_DATE_RANGE');
