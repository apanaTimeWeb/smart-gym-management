...
@Injectable()
/**
 * @description Defines the AdminProfileRepository boundary for the admin_profile backend feature.
 * @remarks Keep this class focused on its declared responsibility; preserve tenant, contract, security, and AI-context invariants when modifying it.
 */
export class AdminProfileRepository extends AdminCoreTenantRepositoryBase<AdminProfileEntity> {
  constructor(tenantManager: AdminCoreTenantDataSourceManager, private readonly mapper: AdminProfileMapper) { super(tenantManager); }
   ...
   */
  async findAll(query: AdminProfileQueryDto): Promise<AdminCorePaginatedResult<AdminProfileDomainModel>> {
    const repository = await this.repositoryFor(AdminProfileEntity);
   ...
   */
  async findById(id: string): Promise<AdminProfileDomainModel | null> {
    const repository = await this.repositoryFor(AdminProfileEntity);
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminProfileDomainModel> {
    const domain = await this.findById(id);
    if (!domain) throw new NotFoundException(`profile record not found.`);
    return domain;
  }
   ...
   */
  private async findEntityByIdOrThrow(id: string): Promise<AdminProfileEntity> {
    const repository = await this.repositoryFor(AdminProfileEntity);
    const entity = await repository.findOne({ where: { id } });
   ...
   */
  async createRecord(input: AdminProfileMutationDto): Promise<AdminProfileDomainModel> {
    const repository = await this.repositoryFor(AdminProfileEntity);
   ...
   */
  async updateById(id: string, input: AdminProfileMutationDto): Promise<AdminProfileDomainModel> {
    const repository = await this.repositoryFor(AdminProfileEntity);
  ...
  async markAsDeleted(id: string): Promise<void> {
    await this.softDeleteRecord(id, AdminProfileEntity);
  }
  ...
  async findLatestSnapshot(query?: AdminProfileQueryDto): Promise<AdminProfileDomainModel | null> {
    const repository = await this.repositoryFor(AdminProfileEntity);
