// RESPONSIBILITY: Owns all TypeORM persistence operations for Admin branches; services never call save() directly.
...
@Injectable()
/**
 * @description Defines the AdminBranchesRepository boundary for the admin_branches backend feature.
 * @remarks Keep this class focused on its declared responsibility; preserve tenant, contract, security, and AI-context invariants when modifying it.
 */
export class AdminBranchesRepository extends AdminCoreTenantRepositoryBase<AdminBranchesEntity> {
  constructor(tenantManager: AdminCoreTenantDataSourceManager, private readonly mapper: AdminBranchesMapper) { super(tenantManager); }
   ...
   */
  async findAll(query: AdminBranchesQueryDto): Promise<AdminCorePaginatedResult<AdminBranchesDomainModel>> {
    const repository = await this.repositoryFor(AdminBranchesEntity);
    const builder = repository.createQueryBuilder('entity');
    const window = resolveBranchesAdminQueryWindow(query);
   ...
   */
  async findById(id: string): Promise<AdminBranchesDomainModel | null> {
    const repository = await this.repositoryFor(AdminBranchesEntity);
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminBranchesDomainModel> {
    const domain = await this.findById(id);
    if (!domain) throw new NotFoundException(`branches record not found.`);
    return domain;
  }
   ...
   */
  private async findEntityByIdOrThrow(id: string): Promise<AdminBranchesEntity> {
    const repository = await this.repositoryFor(AdminBranchesEntity);
    const entity = await repository.findOne({ where: { id } });
    if (!entity) throw new NotFoundException(`branches record not found.`);
  ...
  async findLatestReadModel(query?: AdminBranchesQueryDto): Promise<AdminBranchesDomainModel | null> {
    const repository = await this.repositoryFor(AdminBranchesEntity);
