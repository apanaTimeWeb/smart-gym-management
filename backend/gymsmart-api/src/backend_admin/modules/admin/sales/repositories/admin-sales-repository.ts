...
@Injectable()
export class AdminSalesRepository extends CoreTenantRepositoryBase<AdminSalesEntity> {
   ...
   */
  async findAll(query: AdminSalesQueryDto): Promise<CorePaginatedResult<AdminSalesEntity>> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminSalesEntity);
   ...
   */
  async findById(id: string): Promise<AdminSalesEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminSalesEntity).findOne({ where: { id } });
  }
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminSalesEntity> {
   ...
   */
  async createRecord(input: Record<string, unknown>): Promise<AdminSalesEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminSalesEntity);
   ...
   */
  async updateById(id: string, input: Record<string, unknown>): Promise<AdminSalesEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminSalesEntity);
  ...
  async markAsDeleted(id: string): Promise<void> {
    await this.softDeleteRecord(id, AdminSalesEntity);
  }
  /** @description Reads the first tenant-scoped snapshot used by read models and seed data.
   * @returns Snapshot entity or null.
   */
  async findFirstSnapshot(): Promise<AdminSalesEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminSalesEntity).findOne({ order: { createdAt: 'ASC' } });
  }
