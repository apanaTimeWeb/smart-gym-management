...
@Injectable()
export class AdminDashboardRepository extends CoreTenantRepositoryBase<AdminDashboardEntity> {
   ...
   */
  async findAll(query: AdminDashboardQueryDto): Promise<CorePaginatedResult<AdminDashboardEntity>> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminDashboardEntity);
   ...
   */
  async findById(id: string): Promise<AdminDashboardEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminDashboardEntity).findOne({ where: { id } });
  }
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminDashboardEntity> {
   ...
   */
  async createRecord(input: Record<string, unknown>): Promise<AdminDashboardEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminDashboardEntity);
   ...
   */
  async updateById(id: string, input: Record<string, unknown>): Promise<AdminDashboardEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminDashboardEntity);
  ...
  async markAsDeleted(id: string): Promise<void> {
    await this.softDeleteRecord(id, AdminDashboardEntity);
  }
  /** @description Reads the first tenant-scoped snapshot used by read models and seed data.
   * @returns Snapshot entity or null.
   */
  async findFirstSnapshot(): Promise<AdminDashboardEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminDashboardEntity).findOne({ order: { createdAt: 'ASC' } });
  }
