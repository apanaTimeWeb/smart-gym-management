...
| File | Responsibility |
|---|---|
| `admin-dashboard.module.ts` | Single-purpose `admin-dashboard.module.ts` implementation for the `dashboard` feature. |
| `admin-dashboard.seeder.ts` | Single-purpose `admin-dashboard.seeder.ts` implementation for the `dashboard` feature. |
| `controllers/admin-dashboard-query.controller.ts` | Single-purpose `admin-dashboard-query.controller.ts` implementation for the `dashboard` feature. |
| `domain/admin-dashboard.domain.ts` | Single-purpose `admin-dashboard.domain.ts` implementation for the `dashboard` feature. |
| `dtos/admin-dashboard-id.dto.ts` | Single-purpose `admin-dashboard-id.dto.ts` implementation for the `dashboard` feature. |
| `dtos/admin-dashboard-mutation.dto.ts` | Single-purpose `admin-dashboard-mutation.dto.ts` implementation for the `dashboard` feature. |
| `dtos/admin-dashboard-query.dto.ts` | Single-purpose `admin-dashboard-query.dto.ts` implementation for the `dashboard` feature. |
| `dtos/admin-dashboard-response.dto.ts` | Single-purpose `admin-dashboard-response.dto.ts` implementation for the `dashboard` feature. |
| `entities/admin-dashboard-entity.ts` | Single-purpose `admin-dashboard-entity.ts` implementation for the `dashboard` feature. |
| `mappers/admin-dashboard.mapper.ts` | Single-purpose `admin-dashboard.mapper.ts` implementation for the `dashboard` feature. |
| `repositories/admin-dashboard-repository.ts` | Single-purpose `admin-dashboard-repository.ts` implementation for the `dashboard` feature. |
| `services/admin-dashboard-query.service.ts` | Single-purpose `admin-dashboard-query.service.ts` implementation for the `dashboard` feature. |

## Feature Inventory
| Controller/Endpoint | HTTP | Path | Purpose | Request DTO | Response DTO |
|---|---|---|---|---|---|
| Controller | GET | `/api/v1/admin/dashboard/fetchDashboardStats` | Provides the GET operation required by the Admin `dashboard` frontend capability. | Feature DTO | Feature response DTO |
...
- `admin-dashboard.module.ts` — Single responsibility within `dashboard`. MUST remain scoped to this feature responsibility.
- `admin-dashboard.seeder.ts` — Single responsibility within `dashboard`. MUST remain scoped to this feature responsibility.
- `controllers/admin-dashboard-query.controller.ts` — Single responsibility within `dashboard`. MUST NOT contain business or persistence logic.
- `domain/admin-dashboard.domain.ts` — Single responsibility within `dashboard`. MUST remain scoped to this feature responsibility.
- `dtos/admin-dashboard-id.dto.ts` — Single responsibility within `dashboard`. MUST NOT contain business side effects.
- `dtos/admin-dashboard-mutation.dto.ts` — Single responsibility within `dashboard`. MUST NOT contain business side effects.
- `dtos/admin-dashboard-query.dto.ts` — Single responsibility within `dashboard`. MUST NOT contain business side effects.
- `dtos/admin-dashboard-response.dto.ts` — Single responsibility within `dashboard`. MUST NOT contain business side effects.
- `entities/admin-dashboard-entity.ts` — Single responsibility within `dashboard`. MUST remain scoped to this feature responsibility.
- `mappers/admin-dashboard.mapper.ts` — Single responsibility within `dashboard`. MUST NOT perform DB I/O.
- `repositories/admin-dashboard-repository.ts` — Single responsibility within `dashboard`. MUST NOT contain controller or UI logic.
- `services/admin-dashboard-query.service.ts` — Single responsibility within `dashboard`. MUST NOT perform direct TypeORM persistence.
...
|---|---|---|
| `/api/v1/admin/dashboard/fetchDashboardStats` | GET | Feature DTO fields exactly matching frontend request usage. |

### Response Shape
| Endpoint | Response DTO fields | Notes |
|---|---|---|
| `/api/v1/admin/dashboard/fetchDashboardStats` | Explicit frontend-consumed response DTO | Wrapped as `ApiResponse<T>`; pagination metadata is present only on paginated list responses. |
