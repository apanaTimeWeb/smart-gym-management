...
@Injectable()
export class AdminPlansRepository extends CoreTenantRepositoryBase<AdminPlansEntity> {
   ...
   */
  async findAll(query: AdminPlansQueryDto): Promise<CorePaginatedResult<AdminPlansEntity>> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminPlansEntity);
   ...
   */
  async findById(id: string): Promise<AdminPlansEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminPlansEntity).findOne({ where: { id } });
  }
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminPlansEntity> {
   ...
   */
  async createRecord(input: Record<string, unknown>): Promise<AdminPlansEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminPlansEntity);
   ...
   */
  async updateById(id: string, input: Record<string, unknown>): Promise<AdminPlansEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminPlansEntity);
   ...
   */
  async findFirstSnapshot(): Promise<AdminPlansEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminPlansEntity).findOne({ order: { createdAt: 'ASC' } });
  }
