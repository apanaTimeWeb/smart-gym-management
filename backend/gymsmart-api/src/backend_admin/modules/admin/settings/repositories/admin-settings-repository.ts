...
@Injectable()
export class AdminSettingsRepository extends CoreTenantRepositoryBase<AdminSettingsEntity> {
   ...
   */
  async findAll(query: AdminSettingsQueryDto): Promise<CorePaginatedResult<AdminSettingsEntity>> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminSettingsEntity);
   ...
   */
  async findById(id: string): Promise<AdminSettingsEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminSettingsEntity).findOne({ where: { id } });
  }
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminSettingsEntity> {
   ...
   */
  async createRecord(input: Record<string, unknown>): Promise<AdminSettingsEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminSettingsEntity);
   ...
   */
  async updateById(id: string, input: Record<string, unknown>): Promise<AdminSettingsEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminSettingsEntity);
   ...
   */
  async findFirstSnapshot(): Promise<AdminSettingsEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminSettingsEntity).findOne({ order: { createdAt: 'ASC' } });
  }
