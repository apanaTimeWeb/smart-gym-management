...
@Injectable()
export class AdminProfileRepository extends CoreTenantRepositoryBase<AdminProfileEntity> {
   ...
   */
  async findAll(query: AdminProfileQueryDto): Promise<CorePaginatedResult<AdminProfileEntity>> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminProfileEntity);
   ...
   */
  async findById(id: string): Promise<AdminProfileEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminProfileEntity).findOne({ where: { id } });
  }
   ...
   */
  async findByIdOrThrow(id: string): Promise<AdminProfileEntity> {
   ...
   */
  async createRecord(input: Record<string, unknown>): Promise<AdminProfileEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminProfileEntity);
   ...
   */
  async updateById(id: string, input: Record<string, unknown>): Promise<AdminProfileEntity> {
    const source = await this.tenantManager.getCurrent();
    const repository = source.getRepository(AdminProfileEntity);
   ...
   */
  async findFirstSnapshot(): Promise<AdminProfileEntity | null> {
    const source = await this.tenantManager.getCurrent();
    return source.getRepository(AdminProfileEntity).findOne({ order: { createdAt: 'ASC' } });
  }
