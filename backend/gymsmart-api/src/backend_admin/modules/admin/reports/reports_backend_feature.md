...
| File | Responsibility |
|---|---|
| `admin-reports.module.ts` | Single-purpose `admin-reports.module.ts` implementation for the `reports` feature. |
| `admin-reports.seeder.ts` | Single-purpose `admin-reports.seeder.ts` implementation for the `reports` feature. |
| `controllers/admin-reports-command.controller.ts` | Single-purpose `admin-reports-command.controller.ts` implementation for the `reports` feature. |
| `controllers/admin-reports-query.controller.ts` | Single-purpose `admin-reports-query.controller.ts` implementation for the `reports` feature. |
| `domain/admin-reports.domain.ts` | Single-purpose `admin-reports.domain.ts` implementation for the `reports` feature. |
| `dtos/admin-reports-id.dto.ts` | Single-purpose `admin-reports-id.dto.ts` implementation for the `reports` feature. |
| `dtos/admin-reports-mutation.dto.ts` | Single-purpose `admin-reports-mutation.dto.ts` implementation for the `reports` feature. |
| `dtos/admin-reports-query.dto.ts` | Single-purpose `admin-reports-query.dto.ts` implementation for the `reports` feature. |
| `dtos/admin-reports-response.dto.ts` | Single-purpose `admin-reports-response.dto.ts` implementation for the `reports` feature. |
| `entities/admin-reports-entity.ts` | Single-purpose `admin-reports-entity.ts` implementation for the `reports` feature. |
| `mappers/admin-reports.mapper.ts` | Single-purpose `admin-reports.mapper.ts` implementation for the `reports` feature. |
| `repositories/admin-reports-repository.ts` | Single-purpose `admin-reports-repository.ts` implementation for the `reports` feature. |
| `services/admin-reports-command.service.ts` | Single-purpose `admin-reports-command.service.ts` implementation for the `reports` feature. |
| `services/admin-reports-query.service.ts` | Single-purpose `admin-reports-query.service.ts` implementation for the `reports` feature. |

## Feature Inventory
| Controller/Endpoint | HTTP | Path | Purpose | Request DTO | Response DTO |
|---|---|---|---|---|---|
| Controller | GET | `/api/v1/admin/reports/fetchReportData` | Provides the GET operation required by the Admin `reports` frontend capability. | Feature DTO | Feature response DTO |
| Controller | POST | `/api/v1/admin/reports/exportReport` | Provides the POST operation required by the Admin `reports` frontend capability. | Feature DTO | Feature response DTO |
...
- `admin-reports.module.ts` — Single responsibility within `reports`. MUST remain scoped to this feature responsibility.
- `admin-reports.seeder.ts` — Single responsibility within `reports`. MUST remain scoped to this feature responsibility.
- `controllers/admin-reports-command.controller.ts` — Single responsibility within `reports`. MUST NOT contain business or persistence logic.
- `controllers/admin-reports-query.controller.ts` — Single responsibility within `reports`. MUST NOT contain business or persistence logic.
- `domain/admin-reports.domain.ts` — Single responsibility within `reports`. MUST remain scoped to this feature responsibility.
- `dtos/admin-reports-id.dto.ts` — Single responsibility within `reports`. MUST NOT contain business side effects.
- `dtos/admin-reports-mutation.dto.ts` — Single responsibility within `reports`. MUST NOT contain business side effects.
- `dtos/admin-reports-query.dto.ts` — Single responsibility within `reports`. MUST NOT contain business side effects.
- `dtos/admin-reports-response.dto.ts` — Single responsibility within `reports`. MUST NOT contain business side effects.
- `entities/admin-reports-entity.ts` — Single responsibility within `reports`. MUST remain scoped to this feature responsibility.
- `mappers/admin-reports.mapper.ts` — Single responsibility within `reports`. MUST NOT perform DB I/O.
- `repositories/admin-reports-repository.ts` — Single responsibility within `reports`. MUST NOT contain controller or UI logic.
- `services/admin-reports-command.service.ts` — Single responsibility within `reports`. MUST NOT perform direct TypeORM persistence.
- `services/admin-reports-query.service.ts` — Single responsibility within `reports`. MUST NOT perform direct TypeORM persistence.
...
|---|---|---|
| `/api/v1/admin/reports/fetchReportData` | GET | Feature DTO fields exactly matching frontend request usage. |
| `/api/v1/admin/reports/exportReport` | POST | Feature DTO fields exactly matching frontend request usage. |

### Response Shape
| Endpoint | Response DTO fields | Notes |
|---|---|---|
| `/api/v1/admin/reports/fetchReportData` | Explicit frontend-consumed response DTO | Wrapped as `ApiResponse<T>`; pagination metadata is present only on paginated list responses. |
| `/api/v1/admin/reports/exportReport` | Explicit frontend-consumed response DTO | Wrapped as `ApiResponse<T>`; pagination metadata is present only on paginated list responses. |
