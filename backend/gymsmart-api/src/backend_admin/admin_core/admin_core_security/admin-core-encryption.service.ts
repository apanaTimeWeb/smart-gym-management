...
// FLOW: Domain sensitive value â†’ AdminCoreEncryptionService â†’ validated ConfigService secret â†’ encrypted DB payload.
...
@Injectable()
/**
 * @description Defines the AdminCoreEncryptionService boundary for the admin_core_security backend feature.
 * @remarks Keep this class focused on its declared responsibility; preserve tenant, contract, security, and AI-context invariants when modifying it.
 */
export class AdminCoreEncryptionService {
  ...
  private key(): Buffer {
    const raw = this.config.getOrThrow<string>('runtime.dataEncryptionKey');
