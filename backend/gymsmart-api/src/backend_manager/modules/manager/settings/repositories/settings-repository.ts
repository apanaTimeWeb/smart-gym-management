// RESPONSIBILITY: Named TypeORM repository methods for Manager settings; user input is filtered through feature allowlists.
// FLOW: Use-case -> SettingsRepository -> CoreBaseRepository -> trusted tenant DataSource.
...
import { SettingsMapper } from '@/modules/manager/settings/mappers/settings-mapper';
import type { SettingsDomainData } from '@/modules/manager/settings/settings.interfaces';
...
export class SettingsRepository extends CoreBaseRepository<SettingsEntity> {
  constructor(tenants: import('@/core/database/core-tenant-datasource.service').CoreTenantDatasourceService) { super(tenants, SettingsEntity); }
