...
describe('ScheduleDeleteShiftService', () => {
  it('returns the dependency result and invokes the intended boundary exactly once', async () => {
    ...
    const dependency = { softDeleteScheduleById: jest.fn().mockResolvedValue(expected) };
    ...
    expect(dependency.softDeleteScheduleById).toHaveBeenCalledTimes(1);
    expect(dependency.softDeleteScheduleById).toHaveBeenCalledWith(expect.anything());
  });

  it('propagates a downstream failure instead of converting it to a false success', async () => {
    const failure = new Error('dependency failure');
    const dependency = { softDeleteScheduleById: jest.fn().mockRejectedValue(failure) };
    const service = new ScheduleDeleteShiftService(dependency as never);
