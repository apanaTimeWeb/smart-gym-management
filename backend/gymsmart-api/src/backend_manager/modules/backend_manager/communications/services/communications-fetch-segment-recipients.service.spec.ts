...
describe('CommunicationsFetchSegmentRecipientsService', () => {
  it('returns the dependency result and invokes the intended boundary exactly once', async () => {
    ...
    const result = await service.fetchSegmentRecipients('test-id' as never, {} as never);
  ...
  it('propagates a downstream failure instead of converting it to a false success', async () => {
    ...
    const service = new CommunicationsFetchSegmentRecipientsService(dependency as never);
    await expect(service.fetchSegmentRecipients('test-id' as never, {} as never)).rejects.toBe(failure);
    expect(dependency.findCommunicationsList).toHaveBeenCalledTimes(1);
  });
});
