...
import { HrAllowedSortFields } from '@/backend_manager/modules/backend_manager/hr/hr.constants';
import { HrEntity } from '@/backend_manager/modules/backend_manager/hr/hr.entity';
import { HrMapper } from '@/backend_manager/modules/backend_manager/hr/mappers/hr-mapper';

import type { CoreTransactionContext } from '@/backend_manager/core/database/core-transaction-context';
import type { CoreJsonObject } from '@/backend_manager/core/types/json-value.types';
import type { HrDomainData, HrListResult } from '@/backend_manager/modules/backend_manager/hr/hr.interfaces';
...
export class HrRepository extends CoreBaseRepository<HrEntity> {
  ...
  async findHrById(id: string): Promise<HrDomainData | null> {
    ...
    return row ? HrMapper.toDomain({ ...row, payload: this.revealSensitivePayload(row.payload) } as any) : null;
  }
  ...
  async createHr(data: CoreJsonObject, context: CoreTransactionContext): Promise<HrDomainData> {
    ...
    return HrMapper.toDomain({ ...(await repository.save(row)), payload: this.revealSensitivePayload(row.payload) } as any);
  }
  ...
  async findHrList(query: CoreJsonObject): Promise<HrListResult> {
    ...
    return { data: rows.map((row: HrEntity) => HrMapper.toDomain({ ...row, payload: this.revealSensitivePayload(row.payload) } as any)), meta: buildPaginationMeta(total,page,limit) as any  } as any;
  }
