...
- `store-create-order.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-create-order.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-create-product.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-create-product.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-delete-product.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-delete-product.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-fetch-orders.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-fetch-orders.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-fetch-products.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-fetch-products.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-fetch-store-summary.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-fetch-store-summary.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-orchestrator.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-update-product.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-update-product.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-repository.ts` — Owns the `repositories` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-mapper.ts` — Owns the `mappers` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-create-order.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-create-order.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-create-product.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-create-product.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-delete-product.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-fetch-orders.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-fetch-products.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-fetch-store-summary.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-query.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-update-product.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `store-update-product.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.

## V1 AUTHORITATIVE IMPLEMENTATION MAP

- **Owning table:** `manager_stores`.
...
|---|---|---|
| `GET` | `/api/v1/manager/store/products` | `store` command/query controller |
| `POST` | `/api/v1/manager/store/products` | `store` command/query controller |
| `PATCH` | `/api/v1/manager/store/products/:id` | `store` command/query controller |
| `DELETE` | `/api/v1/manager/store/products/:id` | `store` command/query controller |
| `GET` | `/api/v1/manager/store/orders` | `store` command/query controller |
| `POST` | `/api/v1/manager/store/orders` | `store` command/query controller |
| `GET` | `/api/v1/manager/store/summary` | `store` command/query controller |
