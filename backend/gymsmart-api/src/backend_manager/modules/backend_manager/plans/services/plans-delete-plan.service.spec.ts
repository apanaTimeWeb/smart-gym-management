...
describe('PlansDeletePlanService', () => {
  it('returns the dependency result and invokes the intended boundary exactly once', async () => {
    ...
    const dependency = { softDeletePlansById: jest.fn().mockResolvedValue(expected) };
    const service = new PlansDeletePlanService(dependency as never);
    const result = await service.deletePlan('test-id' as never);
    expect(result).toEqual(expected);
    expect(dependency.softDeletePlansById).toHaveBeenCalledTimes(1);
    expect(dependency.softDeletePlansById).toHaveBeenCalledWith(expect.anything());
  });

  it('propagates a downstream failure instead of converting it to a false success', async () => {
    const failure = new Error('dependency failure');
    const dependency = { softDeletePlansById: jest.fn().mockRejectedValue(failure) };
    const service = new PlansDeletePlanService(dependency as never);
