...
describe('PlansRenewMembershipService', () => {
  it('returns the dependency result and invokes the intended boundary exactly once', async () => {
    ...
    const result = await service.renewMembership({} as never, 'test-id' as never);
  ...
  it('propagates a downstream failure instead of converting it to a false success', async () => {
    ...
    const service = new PlansRenewMembershipService(dependency as never);
    await expect(service.renewMembership({} as never, 'test-id' as never)).rejects.toBe(failure);
    expect(dependency.createPlans).toHaveBeenCalledTimes(1);
  });
});
