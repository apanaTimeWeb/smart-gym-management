...
import { PlansMapper } from '@/backend_manager/modules/backend_manager/plans/mappers/plans-mapper';
import { PlansAllowedSortFields } from '@/backend_manager/modules/backend_manager/plans/plans.constants';
...
import type { PlansDomainData, PlansListResult } from '@/backend_manager/modules/backend_manager/plans/plans.interfaces';
...
export class PlansRepository extends CoreBaseRepository<PlansEntity> {
  ...
  async findPlansList(query: CoreJsonObject): Promise<PlansListResult> {
    ...
    return { data: rows.map(PlansMapper.toDomain), meta: buildPaginationMeta(total,page,limit) as any } as any;
  }
}
