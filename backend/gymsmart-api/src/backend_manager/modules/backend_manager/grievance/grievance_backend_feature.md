...
- **DB Entity**: `GrievanceEntity` -> `manager_grievances` in the trusted tenant database.
...
|---|---|---|
| `GET /manager/grievance` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `POST /manager/grievance` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `POST /manager/grievance/:id/resolve` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |

## Database Constraints
- `PK_manager_grievances`
- `IDX_manager_grievances_created_at`
- `IDX_manager_grievances_updated_at`
- `IDX_manager_grievances_status`
- `CHK_manager_grievances_payload_object`
...
|---|---|---|
| `/manager/grievance` | `GET` | `—` |
| `/manager/grievance` | `POST` | `CreateGrievanceTicketPayload` |
| `/manager/grievance/:id/resolve` | `POST` | `{ resolutionNote: string }` |

### Response Shape
| Endpoint | Response | UI usage |
|---|---|---|
| `/manager/grievance` | `GrievanceTicket[]` | No UI-derived field rows; response type remains the frontend contract source. |
| `/manager/grievance` | `GrievanceTicket` | No UI-derived field rows; response type remains the frontend contract source. |
| `/manager/grievance/:id/resolve` | `GrievanceTicket` | No UI-derived field rows; response type remains the frontend contract source. |

## UI-Required Fields
- `GET` -> `data[].memberName` -> `memberName`
- `GET` -> `data[].category` -> `category`
- `GET` -> `data[].issue` -> `issue`
- `GET` -> `data[].status` -> `status`
- `GET` -> `data[].loggedAt` -> `loggedAt`
- `GET` -> `data[].resolutionNote` -> `resolutionNote`
...
### Frozen API Contract
The canonical endpoint surface is `/api/v1/manager/grievance` plus the exact paths implemented by the feature command/query controllers. Request DTOs are strict and unknown properties are rejected globally. Success/error responses are wrapped by the global canonical `ApiResponse<T>` contract.
...
- `grievance-manager-grievance-api-create-grievance-ticket.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-manager-grievance-api-fetch-grievance-tickets.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-manager-grievance-api-resolve-grievance-ticket.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-orchestrator.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-repository.ts` — Owns the `repositories` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-mapper.ts` — Owns the `mappers` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-ManagerGrievanceApi-createGrievanceTicket-request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-ManagerGrievanceApi-resolveGrievanceTicket-request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-manager-grievance-api-create-grievance-ticket.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-manager-grievance-api-create-grievance-ticket.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-manager-grievance-api-fetch-grievance-tickets.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-manager-grievance-api-resolve-grievance-ticket.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-manager-grievance-api-resolve-grievance-ticket.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `grievance-query.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.

## V1 AUTHORITATIVE IMPLEMENTATION MAP

- **Owning table:** `manager_grievances`.
...
|---|---|---|
| `GET` | `/manager/grievance` | `grievance` command/query controller |
| `POST` | `/manager/grievance` | `grievance` command/query controller |
| `POST` | `/manager/grievance/:id/resolve` | `grievance` command/query controller |
