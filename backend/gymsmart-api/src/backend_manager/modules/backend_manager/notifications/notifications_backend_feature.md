...
- **DB Entity**: `NotificationsEntity` -> `manager_notifications` in the trusted tenant database.
...
|---|---|---|
| `GET /api/v1/manager/notifications` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `GET /api/v1/manager/notifications/kpis` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `PATCH /api/v1/manager/notifications/:id/read` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `PATCH /api/v1/manager/notifications/read-all` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `DELETE /api/v1/manager/notifications/:id` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |

## Database Constraints
- `PK_manager_notifications`
- `IDX_manager_notifications_created_at`
- `IDX_manager_notifications_updated_at`
- `IDX_manager_notifications_status`
- `CHK_manager_notifications_payload_object`
...
|---|---|---|
| `/api/v1/manager/notifications` | `GET` | `{ page?, limit?, search?, status?, priority?, type? }` |
| `/api/v1/manager/notifications/kpis` | `GET` | `—` |
| `/api/v1/manager/notifications/:id/read` | `PATCH` | `{ id: string }` |
| `/api/v1/manager/notifications/read-all` | `PATCH` | `—` |
| `/api/v1/manager/notifications/:id` | `DELETE` | `{ id: string }` |

### Response Shape
| Endpoint | Response | UI usage |
|---|---|---|
| `/api/v1/manager/notifications` | `{ notifications: Notification[]; total: number }` | data.notifications[].title (title); data.notifications[].message (message); data.notifications[].type (type); data.notifications[].priority (priority); data.notifications[].status (status); data.notifications[].createdAt (createdAt); data.notifications[].memberName (memberName) |
| `/api/v1/manager/notifications/kpis` | `NotificationKPIData` | data.total (total); data.unread (unread); data.highPriority (highPriority); data.todayCount (todayCount) |
| `/api/v1/manager/notifications/:id/read` | `null` | No UI-derived field rows; response type remains the frontend contract source. |
| `/api/v1/manager/notifications/read-all` | `null` | No UI-derived field rows; response type remains the frontend contract source. |
| `/api/v1/manager/notifications/:id` | `null` | No UI-derived field rows; response type remains the frontend contract source. |

## UI-Required Fields
- `/api/v1/manager/notifications/kpis` -> `data.total` -> `total`
- `/api/v1/manager/notifications/kpis` -> `data.unread` -> `unread`
- `/api/v1/manager/notifications/kpis` -> `data.highPriority` -> `highPriority`
- `/api/v1/manager/notifications/kpis` -> `data.todayCount` -> `todayCount`
- `/api/v1/manager/notifications` -> `data.notifications[].title` -> `title`
- `/api/v1/manager/notifications` -> `data.notifications[].message` -> `message`
- `/api/v1/manager/notifications` -> `data.notifications[].type` -> `type`
- `/api/v1/manager/notifications` -> `data.notifications[].priority` -> `priority`
- `/api/v1/manager/notifications` -> `data.notifications[].status` -> `status`
- `/api/v1/manager/notifications` -> `data.notifications[].createdAt` -> `createdAt`
- `/api/v1/manager/notifications` -> `data.notifications[].memberName` -> `memberName`
...
### Frozen API Contract
The canonical endpoint surface is `/api/v1/manager/notifications` plus the exact paths implemented by the feature command/query controllers. Request DTOs are strict and unknown properties are rejected globally. Success/error responses are wrapped by the global canonical `ApiResponse<T>` contract.
...
- `notifications-delete-notification.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-delete-notification.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-fetch-manager-notifications.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-fetch-manager-notifications.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-fetch-notification-k-p-is.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-fetch-notification-k-p-is.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-mark-all-notifications-read.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-mark-all-notifications-read.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-mark-notification-read.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-mark-notification-read.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-orchestrator.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-repository.ts` — Owns the `repositories` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-mapper.ts` — Owns the `mappers` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-delete-notification.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-fetch-manager-notifications.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-fetch-notification-k-p-is.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-mark-all-notifications-read.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-mark-all-notifications-read.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-mark-notification-read.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-mark-notification-read.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `notifications-query.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.

## V1 AUTHORITATIVE IMPLEMENTATION MAP

- **Owning table:** `manager_notifications`.
...
|---|---|---|
| `GET` | `/api/v1/manager/notifications` | `notifications` command/query controller |
| `GET` | `/api/v1/manager/notifications/kpis` | `notifications` command/query controller |
| `PATCH` | `/api/v1/manager/notifications/:id/read` | `notifications` command/query controller |
| `PATCH` | `/api/v1/manager/notifications/read-all` | `notifications` command/query controller |
| `DELETE` | `/api/v1/manager/notifications/:id` | `notifications` command/query controller |
