...
- `workout-create-exercise.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-create-exercise.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-create-workout.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-create-workout.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-delete-exercise.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-delete-exercise.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-delete-workout.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-delete-workout.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-fetch-assignments.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-fetch-assignments.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-fetch-exercises.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-fetch-exercises.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-fetch-workouts.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-fetch-workouts.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-orchestrator.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-update-exercise.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-update-exercise.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-update-workout.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-update-workout.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-repository.ts` — Owns the `repositories` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-mapper.ts` — Owns the `mappers` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-create-exercise.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-create-exercise.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-create-workout.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-create-workout.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-delete-exercise.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-delete-workout.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-fetch-assignments.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-fetch-exercises.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-fetch-workouts.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-query.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-update-exercise.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-update-exercise.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-update-workout.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `workout-update-workout.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.

## V1 AUTHORITATIVE IMPLEMENTATION MAP

- **Owning table:** `manager_workouts`.
...
|---|---|---|
| `GET` | `/api/v1/manager/workouts` | `workout` command/query controller |
| `POST` | `/api/v1/manager/workouts` | `workout` command/query controller |
| `PATCH` | `/api/v1/manager/workouts/:id` | `workout` command/query controller |
| `DELETE` | `/api/v1/manager/workouts/:id` | `workout` command/query controller |
| `GET` | `/api/v1/manager/workouts/exercises` | `workout` command/query controller |
| `POST` | `/api/v1/manager/workouts/exercises` | `workout` command/query controller |
| `PATCH` | `/api/v1/manager/workouts/exercises/:id` | `workout` command/query controller |
| `GET` | `/api/v1/manager/workout/assignments` | `workout` command/query controller |
| `DELETE` | `/api/v1/manager/workouts/exercises/:id` | `workout` command/query controller |
