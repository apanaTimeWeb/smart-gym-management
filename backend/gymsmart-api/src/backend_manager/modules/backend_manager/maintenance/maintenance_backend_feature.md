...
- **DB Entity**: `MaintenanceEntity` -> `manager_maintenances` in the trusted tenant database.
...
|---|---|---|
| `GET /manager/maintenance` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `POST /manager/maintenance` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `POST /manager/maintenance/:id/resolve` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |

## Database Constraints
- `PK_manager_maintenances`
- `IDX_manager_maintenances_created_at`
- `IDX_manager_maintenances_updated_at`
- `IDX_manager_maintenances_status`
- `CHK_manager_maintenances_payload_object`
...
|---|---|---|
| `/manager/maintenance` | `GET` | `—` |
| `/manager/maintenance` | `POST` | `CreateMaintenanceTicketPayload` |
| `/manager/maintenance/:id/resolve` | `POST` | `—` |

### Response Shape
| Endpoint | Response | UI usage |
|---|---|---|
| `/manager/maintenance` | `MaintenanceTicket[]` | No UI-derived field rows; response type remains the frontend contract source. |
| `/manager/maintenance` | `MaintenanceTicket` | No UI-derived field rows; response type remains the frontend contract source. |
| `/manager/maintenance/:id/resolve` | `MaintenanceTicket` | No UI-derived field rows; response type remains the frontend contract source. |

## UI-Required Fields
- `GET` -> `data[].title` -> `title`
- `GET` -> `data[].equipment` -> `equipment`
- `GET` -> `data[].priority` -> `priority`
- `GET` -> `data[].status` -> `status`
- `GET` -> `data[].assignedVendor` -> `assignedVendor`
- `GET` -> `data[].estimatedCost` -> `estimatedCost`
- `GET` -> `data[].reportedAt` -> `reportedAt`
...
### Frozen API Contract
The canonical endpoint surface is `/api/v1/manager/maintenance` plus the exact paths implemented by the feature command/query controllers. Request DTOs are strict and unknown properties are rejected globally. Success/error responses are wrapped by the global canonical `ApiResponse<T>` contract.
...
- `maintenance-manager-maintenance-api-create-maintenance-ticket.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-manager-maintenance-api-fetch-maintenance-issues.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-manager-maintenance-api-resolve-maintenance-ticket.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-orchestrator.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-repository.ts` — Owns the `repositories` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-mapper.ts` — Owns the `mappers` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-ManagerMaintenanceApi-createMaintenanceTicket-request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-ManagerMaintenanceApi-resolveMaintenanceTicket-request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-manager-maintenance-api-create-maintenance-ticket.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-manager-maintenance-api-create-maintenance-ticket.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-manager-maintenance-api-fetch-maintenance-issues.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-manager-maintenance-api-resolve-maintenance-ticket.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-manager-maintenance-api-resolve-maintenance-ticket.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `maintenance-query.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.

## V1 AUTHORITATIVE IMPLEMENTATION MAP

- **Owning table:** `manager_maintenances`.
...
|---|---|---|
| `GET` | `/manager/maintenance` | `maintenance` command/query controller |
| `POST` | `/manager/maintenance` | `maintenance` command/query controller |
| `POST` | `/manager/maintenance/:id/resolve` | `maintenance` command/query controller |
