...
- **DB Entity**: `ReferralsEntity` -> `manager_referrals` in the trusted tenant database.
...
|---|---|---|
| `GET /api/v1/manager/referrals/kpis` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `GET /api/v1/manager/referrals` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `POST /api/v1/manager/referrals` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `POST /api/v1/manager/referrals/:referralId/claim` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |

## Database Constraints
- `PK_manager_referrals`
- `IDX_manager_referrals_created_at`
- `IDX_manager_referrals_updated_at`
- `IDX_manager_referrals_status`
- `CHK_manager_referrals_payload_object`
...
|---|---|---|
| `/api/v1/manager/referrals/kpis` | `GET` | `—` |
| `/api/v1/manager/referrals` | `GET` | `{ page: number; limit: number; search?: string; status?: string }` |
| `/api/v1/manager/referrals` | `POST` | `CreateReferralDto` |
| `/api/v1/manager/referrals/:referralId/claim` | `POST` | `{ referralId: string }` |

### Response Shape
| Endpoint | Response | UI usage |
|---|---|---|
| `/api/v1/manager/referrals/kpis` | `ManagerReferralsKPIs` | data.totalReferrals (totalReferrals); data.totalConverted (totalConverted); data.pendingRewards (pendingRewards); data.claimedRewards (claimedRewards) |
| `/api/v1/manager/referrals` | `ManagerReferral[]` | data[].referrerName (referrerName); data[].refereeName (refereeName); data[].refereePhone (refereePhone); data[].dateReferred (dateReferred); data[].status (status); data[].rewardStatus (rewardStatus); data[].rewardAmount (rewardAmount) |
| `/api/v1/manager/referrals` | `ManagerReferral` | data[].referrerName (referrerName); data[].refereeName (refereeName); data[].refereePhone (refereePhone); data[].dateReferred (dateReferred); data[].status (status); data[].rewardStatus (rewardStatus); data[].rewardAmount (rewardAmount) |
| `/api/v1/manager/referrals/:referralId/claim` | `ManagerReferral` | No UI-derived field rows; response type remains the frontend contract source. |

## UI-Required Fields
- `/api/v1/manager/referrals/kpis` -> `data.totalReferrals` -> `totalReferrals`
- `/api/v1/manager/referrals/kpis` -> `data.totalConverted` -> `totalConverted`
- `/api/v1/manager/referrals/kpis` -> `data.pendingRewards` -> `pendingRewards`
- `/api/v1/manager/referrals/kpis` -> `data.claimedRewards` -> `claimedRewards`
- `/api/v1/manager/referrals` -> `data[].referrerName` -> `referrerName`
- `/api/v1/manager/referrals` -> `data[].refereeName` -> `refereeName`
- `/api/v1/manager/referrals` -> `data[].refereePhone` -> `refereePhone`
- `/api/v1/manager/referrals` -> `data[].dateReferred` -> `dateReferred`
- `/api/v1/manager/referrals` -> `data[].status` -> `status`
- `/api/v1/manager/referrals` -> `data[].rewardStatus` -> `rewardStatus`
- `/api/v1/manager/referrals` -> `data[].rewardAmount` -> `rewardAmount`
...
### Frozen API Contract
The canonical endpoint surface is `/api/v1/manager/referrals` plus the exact paths implemented by the feature command/query controllers. Request DTOs are strict and unknown properties are rejected globally. Success/error responses are wrapped by the global canonical `ApiResponse<T>` contract.
...
- `referrals-claim-reward.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-claim-reward.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-create-referral.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-create-referral.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-fetch-referral-k-p-is.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-fetch-referral-k-p-is.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-fetch-referrals.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-fetch-referrals.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-orchestrator.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-repository.ts` — Owns the `repositories` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-mapper.ts` — Owns the `mappers` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-claim-reward.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-claim-reward.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-create-referral.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-create-referral.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-fetch-referral-k-p-is.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-fetch-referrals.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `referrals-query.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.

## V1 AUTHORITATIVE IMPLEMENTATION MAP

- **Owning table:** `manager_referrals`.
...
|---|---|---|
| `GET` | `/api/v1/manager/referrals/kpis` | `referrals` command/query controller |
| `GET` | `/api/v1/manager/referrals` | `referrals` command/query controller |
| `POST` | `/api/v1/manager/referrals` | `referrals` command/query controller |
| `POST` | `/api/v1/manager/referrals/:referralId/claim` | `referrals` command/query controller |
