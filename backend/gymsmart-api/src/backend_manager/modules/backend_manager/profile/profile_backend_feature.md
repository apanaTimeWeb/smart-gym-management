...
- **DB Entity**: `ProfileEntity` -> `manager_profiles` in the trusted tenant database.
...
|---|---|---|
| `GET /api/v1/manager/profile` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `PATCH /api/v1/manager/profile` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |
| `PATCH /api/v1/manager/profile/password` | `MANAGER` | Tenant context is resolved from the master DB; resource-ID routes must resolve the resource inside the trusted tenant before mutation. |

## Database Constraints
- `PK_manager_profiles`
- `IDX_manager_profiles_created_at`
- `IDX_manager_profiles_updated_at`
- `IDX_manager_profiles_status`
- `CHK_manager_profiles_payload_object`
...
|---|---|---|
| `/api/v1/manager/profile` | `GET` | `—` |
| `/api/v1/manager/profile` | `PATCH` | `UpdateManagerProfilePayload` |
| `/api/v1/manager/profile/password` | `PATCH` | `UpdateManagerPasswordPayload` |

### Response Shape
| Endpoint | Response | UI usage |
|---|---|---|
| `/api/v1/manager/profile` | `ManagerProfileData` | data.name (name); data.email (email); data.phone (phone) |
| `/api/v1/manager/profile` | `ManagerProfileData` | data.name (name); data.email (email); data.phone (phone) |
| `/api/v1/manager/profile/password` | `Record<string, unknown>` | No UI-derived field rows; response type remains the frontend contract source. |

## UI-Required Fields
- `/api/v1/manager/profile` -> `data.name` -> `name`
- `/api/v1/manager/profile` -> `data.email` -> `email`
- `/api/v1/manager/profile` -> `data.phone` -> `phone`
- `client form + PATCH /profile/password` -> `request.currentPassword` -> `currentPassword`
- `client form + PATCH /profile/password` -> `request.newPassword` -> `newPassword`
...
### Frozen API Contract
The canonical endpoint surface is `/api/v1/manager/profile` plus the exact paths implemented by the feature command/query controllers. Request DTOs are strict and unknown properties are rejected globally. Success/error responses are wrapped by the global canonical `ApiResponse<T>` contract.
...
- `profile-fetch-profile.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-fetch-profile.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-orchestrator.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-update-password.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-update-password.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-update-profile.service.spec.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-update-profile.service.ts` — Owns the `services` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-repository.ts` — Owns the `repositories` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-mapper.ts` — Owns the `mappers` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-fetch-profile.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-query.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-update-password.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-update-password.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-update-profile.request.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.
- `profile-update-profile.response.dto.ts` — Owns the `dtos` responsibility for this feature; MUST NOT absorb unrelated feature logic.

## V1 AUTHORITATIVE IMPLEMENTATION MAP

- **Owning table:** `manager_profiles`.
...
|---|---|---|
| `GET` | `/api/v1/manager/profile` | `profile` command/query controller |
| `PATCH` | `/api/v1/manager/profile` | `profile` command/query controller |
| `PATCH` | `/api/v1/manager/profile/password` | `profile` command/query controller |
