...
import { PtMapper } from '@/backend_manager/modules/backend_manager/pt/mappers/pt-mapper';
import { PtAllowedSortFields } from '@/backend_manager/modules/backend_manager/pt/pt.constants';
...
import type { PtDomainData, PtListResult } from '@/backend_manager/modules/backend_manager/pt/pt.interfaces';
...
export class PtRepository extends CoreBaseRepository<PtEntity> {
  ...
  async findPtList(query: CoreJsonObject): Promise<PtListResult> {
    ...
    return { data: rows.map(PtMapper.toDomain), meta: buildPaginationMeta(total,page,limit) as any } as any;
  }
}
