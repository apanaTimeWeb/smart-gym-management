# RESPONSIBILITY: Black-box API contract checks for the Manager expenses feature.
...
def test_get_expenses_contract():
    response = _call('GET', 'expenses')
...
def test_get_expenses_id_param_contract():
    response = _call('GET', 'expenses/:id')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_expenses_stats_contract():
    response = _call('GET', 'expenses/stats')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_post_expenses_contract():
    response = _call('POST', 'expenses')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_patch_expenses_id_param_contract():
    response = _call('PATCH', 'expenses/:id')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_delete_expenses_id_param_contract():
    response = _call('DELETE', 'expenses/:id')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
