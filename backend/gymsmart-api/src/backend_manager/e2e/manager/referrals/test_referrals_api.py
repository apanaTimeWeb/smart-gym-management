# RESPONSIBILITY: Black-box API contract checks for the Manager referrals feature.
...
def test_post_referrals_contract():
    response = _call('POST', 'referrals')
...
def test_post_referrals_referralId_claim_param_contract():
    response = _call('POST', 'referrals/:referralId/claim')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_referrals_kpis_contract():
    response = _call('GET', 'referrals/kpis')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_referrals_contract():
    response = _call('GET', 'referrals')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
