# RESPONSIBILITY: Black-box API contract checks for the Manager pt feature.
...
def test_post_pt_assignments_contract():
    response = _call('POST', 'pt/assignments')
...
def test_patch_pt_assignments_assignmentId_complete_session_param_contract():
    response = _call('PATCH', 'pt/assignments/:assignmentId/complete-session')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_pt_kpis_contract():
    response = _call('GET', 'pt/kpis')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_pt_workload_contract():
    response = _call('GET', 'pt/workload')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_pt_packages_contract():
    response = _call('GET', 'pt/packages')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_get_pt_assignments_contract():
    response = _call('GET', 'pt/assignments')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
