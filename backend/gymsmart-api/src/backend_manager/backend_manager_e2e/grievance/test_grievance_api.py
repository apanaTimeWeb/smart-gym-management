# RESPONSIBILITY: Black-box API contract checks for the Manager grievance feature.
...
def test_get_manager_grievance_contract():
    response = _call('GET', '/manager/grievance')
...
def test_post_manager_grievance_contract():
    response = _call('POST', '/manager/grievance')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
...
def test_post_manager_grievance_id_resolve_param_contract():
    response = _call('POST', '/manager/grievance/:id/resolve')
    assert response.status_code not in {HTTPStatus.NOT_FOUND}
    if response.headers.get('content-type','').startswith('application/json'):
        body=response.json()
        assert isinstance(body, dict)
