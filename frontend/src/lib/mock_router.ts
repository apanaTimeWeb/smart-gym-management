...
export async function routeMockRequest<T>(
  ...
  if (path.includes('/plans') && !path.includes('/superadmin')) return MockDB.handleCrud('mock_plans', method, path, body, generate(5, i => ({ id: `plan-${i}`, name: `ERP Plan ${i + 1}`, price: 1000 * (i + 1), duration: `${i+1} Months`, status: 'active' }))) as unknown as ApiResponse<T>;
