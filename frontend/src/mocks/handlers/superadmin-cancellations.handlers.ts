...
const MOCK_SUPERADMIN_CANCELLATIONS_KPIS: CancellationsKpiData = {
  totalAtRisk: 2,
  criticalCount: 1,
  highCount: 1,
...
export const superadminCancellationsHandlers = [
  http.get(BASE_URL, async () => {
    await delay(400);
    return HttpResponse.json<ApiResponse<CancellationsAlert[]>>({
      success: true,
      message: 'Success',
      data: mockAlerts,
    });
  ...
  http.get(`${BASE_URL}/kpi`, async () => {
    await delay(400);
    return HttpResponse.json<ApiResponse<CancellationsKpiData>>({
      success: true,
      message: 'Success',
      data: MOCK_SUPERADMIN_CANCELLATIONS_KPIS,
    });
  }),
