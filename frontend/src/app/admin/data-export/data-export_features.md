...
| `data_export_api/` | API client — requestExport, fetchHistory, getDownloadUrl | `AdminDataExportApi.ts` |
...
| `data_export_types/` | TypeScript types: ExportRequest, ExportStatus, ExportFormat, ExportDataType | `AdminDataExportTypes.ts` |
...
All Admin frontend-first API fixtures and MSW transport handlers are owned by `admin/data-export/data-export_mocks/fixtures/AdminDataExportMockFixtures.ts` and `admin/data-export/data-export_mocks/handlers/AdminDataExportMockHandlers.ts`. These files provide populated success responses and are the only module-owned mock transport source for Admin. Global MSW bootstrap may register these handlers, but must not contain Admin business data.
