  ...
  return (
    <div className="bg-card rounded-xl border border-border overflow-hidden">
      <div className="px-5 py-4 border-b border-border flex items-center gap-2">
        <Building2 size={16} className="text-primary" />
        <h3 className="text-sm font-semibold text-primary">Gym-Specific Overrides</h3>
        <span className="ml-auto text-xs bg-warning-bg text-warning px-2 py-0.5 rounded-full border border-border">{overrides.length} active</span>
      </div>
      <div className="divide-y divide-border">
        {overrides.map((override) => {
          ...
          return (
            <button
              type="button"
              key={`${override.gymId}-${override.role}`}
              onClick={() => { setSelectedGymId(override.gymId); setActiveRole(override.role); }}
              className={`w-full px-5 py-3.5 flex items-center gap-3 text-left hover:bg-surface-highlight motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${isSelected ? 'bg-surface-highlight' : ''}`}
            >
