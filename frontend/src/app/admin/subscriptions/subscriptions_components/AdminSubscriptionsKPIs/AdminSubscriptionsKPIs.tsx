...
export default function AdminSubscriptionsKPIs() {
  ...
  return (
    <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
      ...
      <AdminStatCard title={`Monthly Spend${dateSuffix}`} value={fmt(kpis.monthlySpend)} icon={IndianRupee} iconBg="bg-success" iconColor="text-success" />
      <AdminStatCard title={`Total Invoices${dateSuffix}`} value={kpis.totalInvoices} change="all time" changeType="neutral" icon={FileText} iconBg="bg-info" iconColor="text-info" />
      ...
      <AdminStatCard title={`Days to Renewal${dateSuffix}`} value={kpis.daysUntilRenewal} change="days remaining" changeType="neutral" icon={Calendar} iconBg="bg-warning" iconColor="text-warning" />
      <AdminStatCard title={`Save with Annual${dateSuffix}`} value={fmt(kpis.savedWithAnnual)} change="vs monthly billing" changeType="up" icon={TrendingDown} iconBg="bg-success" iconColor="text-success" />
    </div>
  );
}
