...
export default function AdminProfileMain() {
  ...
  if (profileQuery.isPending) {
    return <div className="max-w-3xl mx-auto space-y-6 motion-safe:animate-pulse"><div className="h-8 w-48 bg-skeleton-base rounded-lg" /><div className="h-24 bg-skeleton-base rounded-xl" /><div className="h-64 bg-skeleton-base rounded-xl" /></div>;
  }

  if (profileQuery.isError) {
    return <div className="max-w-3xl mx-auto bg-card border border-border rounded-xl p-8 text-center"><p className="text-sm text-danger">{profileQuery.error instanceof Error ? profileQuery.error.message : 'Profile could not be loaded.'}</p><button type="button" onClick={() => profileQuery.refetch()} className="mt-4 px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">Retry</button></div>;
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-primary-subtle/20 border border-border rounded-xl p-6 flex items-center gap-5 shadow-sm">
      ...
      <div className="flex gap-1 bg-input border border-border rounded-xl p-1 w-fit">
        {PROFILE_TABS.map(({ id, label, icon: Icon }) => (
          <button type="button" key={id} onClick={() => setActiveTab(id)} aria-pressed={activeTab === id} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${activeTab === id ? 'bg-card text-foreground shadow-sm' : 'text-secondary hover:text-foreground'}`}>
        ...
        <form onSubmit={handleSaveProfile} className="bg-card border border-border rounded-xl p-6 shadow-sm space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
              ...
              <input id="admin-profile-name" type="text" {...profileForm.register('name')} aria-invalid={profileForm.formState.errors.name ? 'true' : 'false'} aria-describedby={profileForm.formState.errors.name ? 'admin-profile-name-error' : undefined} className="w-full px-4 py-2.5 bg-input border border-border rounded-lg text-foreground text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" />
            ...
            <div>
              ...
              <input id="admin-profile-phone" type="tel" {...profileForm.register('phone')} placeholder="+91 XXXXX XXXXX" aria-invalid={profileForm.formState.errors.phone ? 'true' : 'false'} className="w-full px-4 py-2.5 bg-input border border-border rounded-lg text-foreground text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" />
          ...
          <div className="flex flex-col sm:flex-row justify-end pt-2">
            <button type="submit" disabled={savingProfile || !profileForm.formState.isDirty} className="min-w-36 inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-primary hover:bg-primary-hover text-primary-foreground font-semibold rounded-lg text-sm shadow-lg shadow-primary/20 motion-safe:transition-colors disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
              {savingProfile ? <Loader2 size={16} strokeWidth={2} className="motion-safe:animate-spin" /> : <Save size={16} strokeWidth={2} />} Save Changes
            </button>
          </div>
        </form>
      )}
        ...
        <form onSubmit={handleChangePassword} className="bg-card border border-border rounded-xl p-6 shadow-sm space-y-5">
          ...
          <div className="flex flex-col sm:flex-row justify-end pt-2">
            <button type="submit" disabled={savingPassword || !passwordForm.formState.isDirty} className="min-w-36 inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-primary hover:bg-primary-hover text-primary-foreground font-semibold rounded-lg text-sm shadow-lg shadow-primary/20 motion-safe:transition-colors disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
              {savingPassword ? <Loader2 size={16} strokeWidth={2} className="motion-safe:animate-spin" /> : <Lock size={16} strokeWidth={2} />} Update Password
            </button>
          </div>
        </form>
      )}
