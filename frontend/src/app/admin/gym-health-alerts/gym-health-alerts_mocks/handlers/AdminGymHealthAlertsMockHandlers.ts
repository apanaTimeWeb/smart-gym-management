...
export const adminGymHealthAlertsMockHandlers = [
  http.get('*/admin/gym-health-alerts/fetchAlerts', ({ request }) => { const url=new URL(request.url); const search=(url.searchParams.get('search')??'').toLowerCase(); const severity=url.searchParams.get('severity'); const alertType=url.searchParams.get('alertType'); const status=url.searchParams.get('resolved'); const all=healthAlertsState.filter(a => (!search || `${a.gymName} ${a.title} ${a.description}`.toLowerCase().includes(search)) && (!severity || severity==='all' || a.severity===severity) && (!status || status==='all' || (status==='resolved' ? a.isResolved : !a.isResolved))); const page=Math.max(1,Number(url.searchParams.get('page'))||1), limit=Math.max(1,Number(url.searchParams.get('limit'))||10); return paged(all,page,limit); }),
