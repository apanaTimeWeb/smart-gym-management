...
| `payouts_api/` | API client for payout endpoints | `AdminPayoutsApi.ts` |
...
| `payouts_types/` | TypeScript types: Payout, PayoutStatus, PayoutKPIData, PnLStatement | `AdminPayoutsTypes.ts` |
...
All Admin frontend-first API fixtures and MSW transport handlers are owned by `admin/payouts/payouts_mocks/fixtures/AdminPayoutsMockFixtures.ts` and `admin/payouts/payouts_mocks/handlers/AdminPayoutsMockHandlers.ts`. These files provide populated success responses and are the only module-owned mock transport source for Admin. Global MSW bootstrap may register these handlers, but must not contain Admin business data.
