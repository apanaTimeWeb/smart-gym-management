...
export default function AdminPayoutsSummaryTable() {
  ...
  return <div className="space-y-3">
    ...
    {logic.status === 'pending' ? <AdminTableSkeleton rows={6} cols={HEADERS.length} /> : <div className="bg-card rounded-xl border border-border overflow-hidden">
      <div className="overflow-x-auto"><table data-admin-responsive-table className="w-full"><thead><tr className="bg-primary/5 border-b border-border">
        ...
        {logic.payouts.length === 0 ? <tr><td colSpan={HEADERS.length} className="px-4 py-14 text-center text-sm text-secondary">No payout records found for the current filters.</td></tr> : logic.payouts.map((p) => <tr key={`${p.gymId}-${p.month}`} className="hover:bg-primary/5 motion-safe:transition-colors">
