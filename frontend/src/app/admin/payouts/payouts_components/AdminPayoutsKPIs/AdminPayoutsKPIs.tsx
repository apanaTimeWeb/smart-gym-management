...
export default function AdminPayoutsKPIs() {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      <AdminStatCard title={`Net Profit${dateSuffix}`} value={formatCurrency(kpis.totalNetProfit)} change="after all deductions" changeType="up" icon={TrendingUp} iconBg="bg-success" iconColor="text-success" />
      ...
      <AdminStatCard title={`Total Expenses${dateSuffix}`} value={formatCurrency(kpis.totalExpenses)} change="payroll + ops" changeType="down" icon={TrendingDown} iconBg="bg-danger" iconColor="text-danger" />
      <AdminStatCard title={`Pending Payouts${dateSuffix}`} value={kpis.pendingPayouts} change="awaiting transfer" changeType="neutral" icon={Clock} iconBg="bg-warning" iconColor="text-warning" />
    </div>
  );
}