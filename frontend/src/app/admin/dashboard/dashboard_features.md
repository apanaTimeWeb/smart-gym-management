...
All Admin frontend-first API fixtures and MSW transport handlers are owned by `admin/dashboard/dashboard_mocks/fixtures/AdminDashboardMockFixtures.ts` and `admin/dashboard/dashboard_mocks/handlers/AdminDashboardMockHandlers.ts`. These files provide populated success responses and are the only module-owned mock transport source for Admin. Global MSW bootstrap may register these handlers, but must not contain Admin business data.
