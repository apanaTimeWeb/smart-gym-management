...
import { useAdminQrScannerLogic } from '@/app/admin/admin_components/AdminQrScanner/useAdminQrScannerLogic';
import {
  ADMIN_QR_MOCK_ACTIVE_NAME,
  ADMIN_QR_MOCK_ACTIVE_ID,
  ADMIN_QR_MOCK_PT_INFO,
  ADMIN_QR_MOCK_AVATAR_URL,
  ADMIN_QR_STATUS_LABELS,
} from '@/app/admin/admin_components/AdminQrScanner/AdminQrScannerConstants';
import type { AdminQrScannerModalProps } from '@/app/admin/admin_components/AdminQrScanner/AdminQrScannerTypes';

export default function AdminQrScannerModal({ open, onClose }: AdminQrScannerModalProps) {
  const { status, history, handleSimulateScan, handleCheckIn, resetStatus } = useAdminQrScannerLogic();
      ...
      <div className="w-full max-w-5xl max-h-screen bg-overlay rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row border border-border motion-safe:animate-in motion-safe:fade-in motion-safe:zoom-in-95">
        ...
        <div className="flex-1 bg-card relative flex flex-col items-center justify-center p-8 border-b md:border-b-0 md:border-r border-border min-h-96">
          <button
            ...
            className="absolute top-6 left-6 p-3 bg-card/10 hover:bg-card/20 text-primary-foreground rounded-full motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          >
            <X size={24} />
          </button>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="px-6 py-3 bg-card/10 hover:bg-card/20 text-primary-foreground text-sm font-bold rounded-xl motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary disabled:opacity-50"
            >
              Simulate Active
            </button>
            <button
            ...
            {(status === 'ACTIVE' || status === 'EXPIRED') && (
              <div className="flex flex-col items-center text-center motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-4">
                ...
                <div className={`w-32 h-32 rounded-full mb-4 border-4 overflow-hidden relative ${status === 'ACTIVE' ? 'border-success' : 'border-danger'}`}>
                  ...
                  <Image
                    src={ADMIN_QR_MOCK_AVATAR_URL}
                    alt={`${ADMIN_QR_MOCK_ACTIVE_NAME} profile photo for face verification`}
                    fill
                    sizes="128px"
                ...
                <h3 className="text-2xl font-black text-foreground">{ADMIN_QR_MOCK_ACTIVE_NAME}</h3>
                <p className="text-sm font-bold text-secondary mb-1">{ADMIN_QR_MOCK_ACTIVE_ID}</p>
                ...
                <div className={`mt-3 px-4 py-1.5 rounded-full text-sm font-bold flex items-center gap-2 ${status === 'ACTIVE' ? 'bg-success-bg text-success' : 'bg-danger-bg text-danger'}`}>
                  ...
                  {ADMIN_QR_STATUS_LABELS[status]}
                </div>

                {/* Yellow upsell banner — only shown for active members with PT info */}
                {status === 'ACTIVE' && (
                  <p className="text-xs text-warning font-medium mt-3 bg-warning-bg px-3 py-1 rounded-lg">
                    {ADMIN_QR_MOCK_PT_INFO}
                  </p>
                )}
