...
export default function AdminConfirmModal({ isOpen, title, message, onConfirm, onCancel, confirmText = 'Confirm', cancelText = 'Cancel', type = 'danger', requireTyping = false, confirmationPhrase = 'CONFIRM' }: AdminConfirmModalProps) {
  ...
  return <div data-admin-dialog="true" tabIndex={-1} className="fixed inset-0 z-40 flex items-center justify-center bg-overlay backdrop-blur-sm p-4 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-200" role="dialog" aria-modal="true" aria-labelledby="admin-confirm-title">
    <div className="bg-card/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-border/5 w-full max-w-sm overflow-hidden">
      <div className="p-6">
        ...
        {requireTyping && <div className="mt-5"><label htmlFor="admin-confirm-input" className="text-xs font-semibold text-secondary">Type {confirmationPhrase} to continue</label><input id="admin-confirm-input" value={typedValue} onChange={(event) => setTypedValue(event.target.value)} autoComplete="off" className="mt-2 w-full px-3 py-2.5 bg-input border border-border rounded-xl text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={`Type ${confirmationPhrase} to confirm`} /></div>}
        <div className="flex gap-3 mt-6"><button type="button" onClick={onCancel} className="flex-1 py-2.5 border border-border rounded-xl text-sm font-semibold text-foreground hover:bg-card/5 motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">{cancelText}</button><button type="button" onClick={onConfirm} disabled={!canConfirm} className={`flex-1 py-2.5 rounded-xl text-sm font-semibold text-primary-foreground motion-safe:transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary disabled:opacity-50 disabled:cursor-not-allowed ${type === 'danger' ? 'bg-danger' : type === 'warning' ? 'bg-warning' : 'bg-info'}`}>{confirmText}</button></div>
      </div>
    </div>
  </div>;
}
