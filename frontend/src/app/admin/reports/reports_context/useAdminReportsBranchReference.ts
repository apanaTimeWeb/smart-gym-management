...
export function useAdminReportsBranchReference() {
  return useQuery({ queryKey: ['admin','reports','branch-reference'], queryFn: async () => (await AdminReportsBranchReferenceApi.fetchReportBranchReferences()).data?.items ?? [], staleTime: 300000 });
}
