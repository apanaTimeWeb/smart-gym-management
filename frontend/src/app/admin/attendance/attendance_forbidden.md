...
7. **NEVER hardcode API endpoint strings in `AdminAttendanceApi.ts`.** When replacing mock functions with real `apiFetch` calls, import endpoint strings from `attendance_url_config.ts`.
