...
| `branches_components/AdminBranchesCard/` | Card component displaying KPIs for a single branch | `AdminBranchesCard.tsx` |
| `branches_components/AdminBranchesDetailDrawer/` | Slide-in drawer showing full branch profile (staff, revenue, members) | `AdminBranchesDetailDrawer.tsx` |
...
|---|---|
| `branches_components/AdminBranchesCard/AdminBranchesCard.tsx` | Renders/orchestrates this module's documented UI section; no direct business API logic unless explicitly designated by the module architecture. |
| `branches_components/AdminBranchesDetailDrawer/AdminBranchesDetailDrawer.tsx` | Renders/orchestrates this module's documented UI section; no direct business API logic unless explicitly designated by the module architecture. |
...
All Admin frontend-first API fixtures and MSW transport handlers are owned by `admin/branches/branches_mocks/fixtures/AdminBranchesMockFixtures.ts` and `admin/branches/branches_mocks/handlers/AdminBranchesMockHandlers.ts`. These files provide populated success responses and are the only module-owned mock transport source for Admin. Global MSW bootstrap may register these handlers, but must not contain Admin business data.
