          ...
          return (
            <div key={plan.name} className={`rounded-xl border p-4 space-y-3 motion-safe:transition-all motion-safe:duration-base ${plan.isCurrent ? 'border-primary bg-primary-subtle' : 'border-border bg-input hover:border-focus'}`}>
              <div className="flex items-center justify-between">
                <p className="text-sm font-bold text-on-primary">{plan.name}</p>
                {plan.isCurrent && <span className="text-xs font-bold text-on-primary bg-primary-subtle px-2 py-0.5 rounded-full">Current</span>}
              </div>
              <p className="text-xl font-bold text-on-primary">{plan.price === 0 ? 'Custom' : `${formatCurrency(plan.price)}/mo`}</p>
              ...
              {!plan.isCurrent && (
                <button
                  ...
                  className="w-full min-h-11 py-2 rounded-lg text-xs font-bold border border-primary text-on-primary hover:bg-primary-subtle motion-safe:transition-all motion-safe:duration-base motion-safe:active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary disabled:opacity-60 disabled:cursor-not-allowed"
                >
