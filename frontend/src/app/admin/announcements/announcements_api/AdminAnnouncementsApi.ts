...
export const announcementsApi = {
  ...
  createAnnouncement: async (payload: AnnouncementFormValues, idempotencyKey?: string) => apiFetch<ApiResponse<Announcement>>(`${AdminAnnouncementsUrlConfig.api.base}/createAnnouncement`, { method: 'POST', body: JSON.stringify(payload), dataSchema: announcementSchema,
      headers: idempotencyKey ? { 'Idempotency-Key': idempotencyKey } : undefined
}),
  updateAnnouncement: async (id: string, payload: AnnouncementFormValues, idempotencyKey?: string) => apiFetch<ApiResponse<Announcement>>(`${AdminAnnouncementsUrlConfig.api.base}/updateAnnouncement`, { method: 'POST', body: JSON.stringify({ id, ...payload }), dataSchema: announcementSchema,
      headers: idempotencyKey ? { 'Idempotency-Key': idempotencyKey } : undefined
}),
  ...
  togglePin: async (id: string, idempotencyKey?: string) => apiFetch<ApiResponse<Announcement>>(`${AdminAnnouncementsUrlConfig.api.base}/togglePin`, { method: 'POST', body: JSON.stringify({ id }), dataSchema: announcementSchema,
      headers: idempotencyKey ? { 'Idempotency-Key': idempotencyKey } : undefined
}),
};
