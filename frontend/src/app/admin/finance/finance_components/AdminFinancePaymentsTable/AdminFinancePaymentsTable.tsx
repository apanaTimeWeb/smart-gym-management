      ...
      <div className="overflow-x-auto" aria-busy="true" aria-label="Loading payments">
        <table className="w-full">
          <thead className="bg-input text-secondary">
            <tr>{PAYMENTS_TABLE_HEADERS.map((header) => <th key={header} className="text-left text-xs font-semibold uppercase tracking-wider px-4 py-3">{header}</th>)}</tr>
          </thead>
  ...
  return (
    <>
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-input text-secondary">
            <tr>{PAYMENTS_TABLE_HEADERS.map((header) => <th key={header} scope="col" className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">{header}</th>)}</tr>
          </thead>
          <tbody className="divide-y divide-border">
            {payments.length === 0 ? (
