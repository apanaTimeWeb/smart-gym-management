...
import AdminHrStaffSortIndicator from '@/app/admin/hr/hr_components/AdminHrStaffTable/AdminHrStaffSortIndicator';
  ...
  const renderHeader = (header: string) => {
    ...
    return (
      <button
        ...
        className="motion-safe:transition-all motion-safe:duration-base ease-in-out inline-flex min-h-11 items-center gap-1.5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded"
          ...
          <tbody className="divide-y divide-border">
            {staff.map((member) => (
              <tr key={member.id} className="motion-safe:transition-colors hover:bg-surface-hover cursor-pointer motion-safe:duration-base" role="button" tabIndex={0} aria-label={`Open profile for ${member.name || 'staff member'}`} onClick={() => openProfile(member)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openProfile(member); } }}>
                ...
                <td className="px-4 py-3 text-sm text-secondary">{maskSensitiveData(member.phone)}</td><td className="px-4 py-3 text-sm font-medium text-success">{formatCurrency(member.salary)}</td><td className="px-4 py-3 text-sm font-medium text-primary text-right">{member.advanceSalary && member.advanceSalary > 0 ? formatCurrency(member.advanceSalary) : '—'}</td><td className="px-4 py-3 text-sm text-secondary">{member.joinDate ? format(new Date(member.joinDate), 'dd MMM yyyy') : displayValue(null)}</td>
                <td className="px-4 py-3 text-right"><div className="flex items-center justify-end gap-2"><button type="button" onClick={(event) => { event.stopPropagation(); toggleStaffStatus(member); }} className={`min-h-11 min-w-11 p-1.5 rounded-lg motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${member.isActive === false ? 'text-success hover:bg-success-bg' : 'text-danger hover:bg-surface-hover'}`} title={member.isActive === false ? 'Activate Staff' : 'Suspend Staff'} aria-label={member.isActive === false ? `Activate ${member.name}` : `Suspend ${member.name}`}>{member.isActive === false ? <PlayCircle size={16} /> : <Ban size={16} />}</button><button type="button" onClick={(event) => { event.stopPropagation(); openEdit(member); }} className="min-h-11 min-w-11 p-1.5 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary motion-safe:transition-colors text-secondary hover:text-primary hover:bg-primary-subtle motion-safe:duration-base" title="Edit" aria-label={`Edit ${member.name}`}><Edit2 size={16} /></button><button type="button" onClick={(event) => { event.stopPropagation(); deleteStaff(member.id); }} className="min-h-11 min-w-11 p-1.5 rounded motion-safe:transition-colors text-danger hover:bg-surface-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danger motion-safe:duration-base" title="Delete" aria-label={`Delete ${member.name}`}><Trash2 size={16} /></button></div></td>
              </tr>
            ))}
