          ...
          <tbody className="divide-y divide-border">
            {staff.map((member) => (
              <tr key={member.id} className="motion-safe:transition-colors hover:bg-surface-hover cursor-pointer motion-safe:duration-base" role="button" tabIndex={0} aria-label={`Open profile for ${member.name || 'staff member'}`} onClick={() => openProfile(member)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openProfile(member); } }}>
                <td className="px-4 py-3"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm bg-primary-subtle text-on-primary">{(member.name || '?').charAt(0).toUpperCase()}</div><div><p className="text-sm font-medium text-primary">{displayValue(member.name)}</p><p className="text-xs text-secondary">{displayValue(member.email)}</p></div></div></td>
                <td className="px-4 py-3 text-sm text-secondary">{member.role === 'Manager' && member.assignedBranches && member.assignedBranches.length > 0 ? <div className="flex flex-col"><span className="font-medium text-primary">{displayValue(member.primaryBranchId || member.assignedBranches[0])}</span>{member.assignedBranches.length > 1 && <span className="text-xs bg-primary-subtle text-on-primary px-1.5 py-0.5 rounded-full mt-1 w-max">+{member.assignedBranches.length - 1} More</span>}</div> : displayValue(member.branch)}</td>
