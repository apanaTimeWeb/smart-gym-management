              ...
              {loading ? (
                ...
                sortedLedger.map((entry) => (
                  <tr key={entry.id} className="hover:bg-surface-hover motion-safe:transition-colors motion-safe:duration-base">
                    ...
                    <td className="p-4 text-on-primary text-right">{entry.credit ? formatMoney(entry.credit) : '—'}</td>
                    <td className="p-4 text-on-primary text-right">{entry.debit ? formatMoney(entry.debit) : '—'}</td>
