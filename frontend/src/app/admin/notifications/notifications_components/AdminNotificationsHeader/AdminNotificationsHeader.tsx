...
// EFFECT: Synchronizes this component effect with its declared React dependencies in notifications/notifications_components/AdminNotificationsHeader/AdminNotificationsHeader.tsx.
  useEffect(() => {
      ...
      {showNotifications && (
        <div className="absolute right-0 mt-2 w-80 bg-popover rounded-xl shadow-popover border border-border overflow-hidden z-30">
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            ...
            <button onClick={() => setShowNotifications(false)} className="min-h-11 min-w-11 motion-safe:transition-all motion-safe:duration-base ease-in-out text-secondary hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-page" aria-label="Close notifications"><X size={15} /></button>
          </div>
          <div className="max-h-72 overflow-y-auto">
            {notifications.length === 0 ? (
              <div className="p-4 text-center text-sm text-secondary">No new notifications</div>
            ) : notifications.map((n) => (
              <button type="button" key={n.id} onClick={() => n.unread && markAsRead(n.id)} className={`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-page w-full text-left px-4 py-3 border-b border-border hover:bg-input motion-safe:transition-colors cursor-pointer relative group ${n.unread ? 'bg-primary-subtle' : ''}`}>
