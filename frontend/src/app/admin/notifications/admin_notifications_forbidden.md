# Forbidden Patterns for `admin/notifications`
...
2. **No Relative Imports:** Never use `./` or `../../` in any file. Always use absolute paths starting with `@/app/admin/notifications/...`.
...
5. **No Server Data in Context/Zustand:** Do not store API responses, API loading, or API error state in React Context or Zustand. TanStack Query owns server state; Zustand is only for module UI/client state.
...
8. **No Localized API Calls:** UI components should never call `apiFetch` directly. They must trigger actions in custom hooks or stores, which then communicate with `notifications_api`.
