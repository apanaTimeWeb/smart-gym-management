...
export function useManagerProfileLogic() {
  ...
  const displayInitial = (user?.name ?? 'M').charAt(0).toUpperCase();

  async function handleSaveProfile(e: React.FormEvent) {
    e.preventDefault();
    setSaving(true);
    try {
      const res = await managerProfileApi.updateProfile({ name, phone });
  ...
  async function handleChangePassword(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const res = await managerProfileApi.updatePassword({ currentPassword, newPassword, confirmPassword });
