# Manager Referrals Theme Contract

This module strictly adheres to the global design system defined in `web_global_design.md`.

## Allowed Variables
The Referrals module is allowed to use the following semantic tokens:
...
- `--success`: Used for success badges (e.g., successful referral).
- `--success-bg`: Subtle background for success badges.
- `--warning`: Used for pending badges.
- `--warning-bg`: Subtle background for warning badges.
- `--danger`: Used for rejected badges.
- `--danger-bg`: Subtle background for rejected badges.
