...
import { useManagerInquiriesMutations } from "@/app/manager/inquiries/inquiries_hooks/ManagerUseManagerInquiriesMutations";
import { useInquiriesQuery, useInquiryStatsQuery } from "@/app/manager/inquiries/inquiries_hooks/ManagerUseManagerInquiriesQueries";
...
export function useManagerInquiriesLogic(): ManagerInquiriesViewModel {
  ...
  const deleteInquiry = useCallback(async (id: string) => { await removeInquiry({ id, idempotencyKey: createManagerIdempotencyKey() }); }, [removeInquiry]);
