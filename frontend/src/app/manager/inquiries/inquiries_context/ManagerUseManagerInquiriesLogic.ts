// DATA FLOW: Manager module state/API data → useManagerInquiriesLogic → owning Manager UI components.
/** Manages UseInquiriesLogic for the Manager module. */
...
export function useManagerInquiriesLogic(): InquiriesContextType {
  ...
  const { createInquiry, updateInquiry, deleteInquiry: removeInq, convertLead: convertLeadMutation, isCreating, isUpdating, isConverting } = useManagerInquiriesMutations({
    showToast,
    onSuccessCallback: () => {
      setShowModal(false);
      setConvertLead(null);
  ...
  return {
    ...
    convertLeadMutation, isConverting,
