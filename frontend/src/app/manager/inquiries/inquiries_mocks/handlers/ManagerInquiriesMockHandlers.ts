...
export const managerInquiriesHandlers = [
  http.get(`/api/v1/manager/inquiries`, ({ request }) => {
  ...
  http.post(`/api/v1/manager/inquiries/:id/convert`, async ({ params }) => {
  ...
  http.post(`/api/v1/manager/inquiries`, async ({ request }) => {
  ...
  http.patch(`/api/v1/manager/inquiries/:id`, async ({ request, params }) => {
    const body = await request.json() as Partial<InquiryFormValues>;
    const idx = mockInquiries.findIndex(i => i.id === params.id);
  ...
  http.delete(`/api/v1/manager/inquiries/:id`, ({ params }) => {
