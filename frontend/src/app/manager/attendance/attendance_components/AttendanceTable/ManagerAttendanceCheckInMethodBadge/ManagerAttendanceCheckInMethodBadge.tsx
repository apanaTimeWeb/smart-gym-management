...
export function ManagerAttendanceCheckInMethodBadge({ method }: { method?: CheckInMethod }) {
  ...
  const config: Record<CheckInMethod, { icon: ReactNode; label: string; class: string }> = {
    QR:        { icon: <QrCode size={18} />,       label: 'QR',       class: 'bg-info text-info' },
    Manual:    { icon: <Edit size={18} />,          label: 'Manual',   class: 'bg-warning text-warning' },
    Biometric: { icon: <Fingerprint size={18} />,   label: 'Biometric', class: 'bg-success text-success' } };
