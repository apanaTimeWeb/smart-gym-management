// RESPONSIBILITY: Framework route boundary for the Manager attendance module; renders the route-level shell, loading, error, or 404 state.
...
export default function NotFound() {
  return (
    <div className="flex min-h-screen w-full flex-col items-center justify-center p-8 text-center">
      ...
      <Link href={MANAGER_NAV_GROUPS[0]?.items[0]?.href ?? '/manager'} className="px-6 py-2 bg-primary text-on-primary rounded-lg hover:bg-primary-hover motion-safe:transition-colors">
        Return to Dashboard
      </Link>
    </div>
  );
