   ...
   <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
     ...
     {activeTab === 'View Assigned Plans' && (
       <div className="p-5">
         ...
         {assignmentsLoading ? <div className="h-32 bg-input rounded-xl animate-pulse" aria-label="Loading assigned plans" /> : assignmentsError ? <p role="alert" className="text-sm text-danger">Unable to load assigned workout plans.</p> : <div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead><tr className="border-b border-border"><th className="py-3 px-4 text-sm font-medium text-secondary">Member Name</th><th className="py-3 px-4 text-sm font-medium text-secondary">Assigned Plan</th><th className="py-3 px-4 text-sm font-medium text-secondary">Assigned By</th><th className="py-3 px-4 text-sm font-medium text-secondary">Start Date</th><th className="py-3 px-4 text-sm font-medium text-secondary text-right">Action</th></tr></thead><tbody className="divide-y divide-border">{assignments.map((assignment) => <tr key={assignment.id}><td className="py-3 px-4 text-sm text-foreground">{assignment.memberName}</td><td className="py-3 px-4 text-sm text-secondary">{assignment.planName}</td><td className="py-3 px-4 text-sm text-secondary">{assignment.assignedBy}</td><td className="py-3 px-4 text-sm text-secondary">{new Intl.DateTimeFormat(undefined, { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(assignment.startDate))}</td><td className="py-3 px-4 text-right"><button type="button" className="px-3 py-1.5 text-xs font-semibold bg-input text-foreground rounded-lg">View Progress</button></td></tr>)}</tbody></table></div>}
       </div>
     )}
