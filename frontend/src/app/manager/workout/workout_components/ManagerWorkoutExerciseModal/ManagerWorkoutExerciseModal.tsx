    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-overlay-backdrop p-4" role="presentation">
      <div className="bg-overlay rounded-2xl shadow-dialog w-full max-w-md overflow-hidden border-2 border-warning" role="dialog" aria-modal="true" aria-labelledby="manager-workout-exercise-title">
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          <div><label htmlFor="manager-workout-exercise-name" className="block text-sm font-medium text-secondary mb-1">Exercise Name *</label><input id="manager-workout-exercise-name" type="text" {...register('name')} className={`w-full px-3 py-2 border rounded-lg focus-visible:outline-none focus-visible:ring-2 ${errors.name ? 'border-danger focus-visible:ring-danger' : 'border-border focus-visible:ring-warning'} bg-input text-primary`} />{errors.name && <p role="alert" className="text-danger text-xs mt-1">{String(errors.name.message ?? '')}</p>}</div>
          <div><label htmlFor="manager-workout-exercise-muscle" className="block text-sm font-medium text-secondary mb-1">Primary Muscle *</label><input id="manager-workout-exercise-muscle" type="text" placeholder="e.g. Chest, Quadriceps" {...register('muscle')} className={`w-full px-3 py-2 border rounded-lg focus-visible:outline-none focus-visible:ring-2 ${errors.muscle ? 'border-danger focus-visible:ring-danger' : 'border-border focus-visible:ring-warning'} bg-input text-primary`} />{errors.muscle && <p role="alert" className="text-danger text-xs mt-1">{String(errors.muscle.message ?? '')}</p>}</div>
