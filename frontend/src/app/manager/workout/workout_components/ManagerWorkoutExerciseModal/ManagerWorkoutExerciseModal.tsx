    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-overlay-backdrop p-4" role="presentation">
      <div className="bg-overlay rounded-2xl shadow-dialog w-full max-w-md overflow-hidden border-2 border-warning" role="dialog" aria-modal="true" aria-labelledby="manager-workout-exercise-title">
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          <div><label htmlFor="manager-workout-exercise-name" className="block text-sm font-medium text-secondary mb-1">Exercise Name *</label><input id="manager-workout-exercise-name" type="text" {...register('name')} className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 ${errors.name ? 'border-danger focus:ring-danger' : 'border-border focus:ring-warning'} bg-input text-on-primary`} />{errors.name && <p role="alert" className="text-danger text-xs mt-1">{String(errors.name.message ?? '')}</p>}</div>
          <div><label htmlFor="manager-workout-exercise-muscle" className="block text-sm font-medium text-secondary mb-1">Primary Muscle *</label><input id="manager-workout-exercise-muscle" type="text" placeholder="e.g. Chest, Quadriceps" {...register('muscle')} className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 ${errors.muscle ? 'border-danger focus:ring-danger' : 'border-border focus:ring-warning'} bg-input text-on-primary`} />{errors.muscle && <p role="alert" className="text-danger text-xs mt-1">{String(errors.muscle.message ?? '')}</p>}</div>
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 pt-2"><button type="button" onClick={handleClose} className="flex-1 min-h-10 py-2.5 border border-border rounded-xl text-sm font-medium text-on-primary hover:bg-primary-subtle motion-safe:transition-colors">Cancel</button><button type="submit" disabled={saving} className="min-w-32 flex-1 min-h-10 py-2.5 rounded-xl text-sm font-bold text-on-primary bg-primary flex items-center justify-center gap-2 disabled:opacity-70 motion-safe:transition-colors">{saving ? <Loader2 size={18} className="motion-safe:animate-spin" aria-hidden="true" /> : <Save size={18} aria-hidden="true" />}<span>{saving ? 'Saving…' : editExId ? 'Update' : 'Save'}</span></button></div>
        </form>
      </div>
    </div>
  );
