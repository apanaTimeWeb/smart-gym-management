...
import { MANAGER_WORKOUT_LEVELS, MANAGER_WORKOUT_MAX_DAYS, MANAGER_WORKOUT_MAX_EXERCISES } from '@/app/manager/workout/workout_constants/ManagerWorkoutFormConstants';
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-overlay-backdrop p-4" role="presentation">
      <div className="bg-overlay rounded-2xl shadow-dialog w-full max-w-md overflow-hidden border-2 border-warning" role="dialog" aria-modal="true" aria-labelledby="manager-workout-title">
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          <div><label htmlFor="manager-workout-name" className="block text-sm font-medium text-secondary mb-1">Plan Name *</label><input id="manager-workout-name" type="text" {...register('name')} className={`w-full px-3 py-2 border rounded-lg focus-visible:outline-none focus-visible:ring-2 ${errors.name ? 'border-danger focus-visible:ring-danger' : 'border-border focus-visible:ring-warning'} bg-input text-primary`} />{errors.name && <p role="alert" className="text-danger text-xs mt-1">{String(errors.name.message ?? '')}</p>}</div>
          <div><label className="block text-sm font-medium text-secondary mb-1">Level</label><Controller name="level" control={control} render={({ field }) => <ManagerSearchableDropdown options={MANAGER_WORKOUT_LEVELS} value={field.value} onChange={field.onChange} />} /></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label htmlFor="manager-workout-days" className="block text-sm font-medium text-secondary mb-1">Days</label><input id="manager-workout-days" type="number" min="1" max={MANAGER_WORKOUT_MAX_DAYS} step="1" {...register('days', { valueAsNumber: true })} className="w-full px-3 py-2 border border-border rounded-lg bg-input text-primary" />{errors.days && <p role="alert" className="text-danger text-xs mt-1">{String(errors.days.message ?? '')}</p>}</div><div><label htmlFor="manager-workout-exercises" className="block text-sm font-medium text-secondary mb-1">Exercises</label><input id="manager-workout-exercises" type="number" min="1" max={MANAGER_WORKOUT_MAX_EXERCISES} step="1" {...register('exercises', { valueAsNumber: true })} className="w-full px-3 py-2 border border-border rounded-lg bg-input text-primary" />{errors.exercises && <p role="alert" className="text-danger text-xs mt-1">{String(errors.exercises.message ?? '')}</p>}</div></div>
