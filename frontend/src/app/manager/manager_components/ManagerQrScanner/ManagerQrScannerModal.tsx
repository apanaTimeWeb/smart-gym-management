...
import {
  ...
  MANAGER_QR_MOCK_PT_INFO,
  MANAGER_QR_MOCK_AVATAR_URL,
  MANAGER_QR_STATUS_LABELS,
} from '@/app/manager/manager_components/ManagerQrScanner/ManagerQrScannerConstants';
import type { ManagerQrScannerModalProps } from '@/app/manager/manager_components/ManagerQrScanner/ManagerQrScannerTypes';
      ...
      <div className="w-full max-w-5xl h-[85vh] bg-overlay rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row border border-border motion-safe:animate-in motion-safe:fade-in motion-safe:zoom-in-95">
        ...
        <div className="flex-1 bg-black/95 relative flex flex-col items-center justify-center p-8 border-b md:border-b-0 md:border-r border-border">
          ...
          <div className="relative w-64 h-64 sm:w-80 sm:h-80 mb-10">
            ...
            {/* Scanning Line Animation — only when idle or actively scanning */}
            {(status === 'IDLE' || status === 'SCANNING') && (
              <div className="absolute top-0 left-0 w-full h-1 bg-primary shadow-[0_0_15px_rgba(250,204,21,0.8)] motion-safe:animate-qr-scan" />
            )}
        ...
        <div className="w-full md:w-96 flex flex-col overflow-hidden">
          ...
          {/* Verification Panel */}
          <div className="p-6 border-b border-border min-h-72 flex flex-col justify-center">
            {status === 'IDLE' && (
            ...
            {(status === 'ACTIVE' || status === 'EXPIRED') && (
              <div className="flex flex-col items-center text-center motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-4">
                {/* Member Avatar — uses next/image for optimization (Rule 33) */}
                <div className={`w-32 h-32 rounded-full mb-4 border-4 overflow-hidden relative ${status === 'ACTIVE' ? 'border-success shadow-[0_0_20px_rgba(34,197,94,0.3)]' : 'border-danger shadow-[0_0_20px_rgba(239,68,68,0.3)]'}`}>
                  <Image
                    src={MANAGER_QR_MOCK_AVATAR_URL}
                    alt={`${MANAGER_QR_MOCK_ACTIVE_NAME} profile photo for face verification`}
                    fill
                    sizes="128px"
                ...
                {status === 'ACTIVE' && (
                  <p className="text-xs text-warning font-medium mt-3 bg-warning/10 px-3 py-1 rounded-lg">
                    {MANAGER_QR_MOCK_PT_INFO}
                  </p>
                )}
          ...
          <div className="flex-1 bg-input/30 p-6 overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              {history.map((h) => (
                <div key={h.id} className="bg-card border border-border p-3 rounded-xl flex items-center justify-between shadow-sm">
