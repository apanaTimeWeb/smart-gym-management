...
| `finance_api/useManagerFinanceQueries.ts` | TanStack Query definitions for API calls |
...
3. **Never embed hardcoded colors in UI components**: Follow `finance_theme_contract.md`. E.g., `text-success`, not `text-[#22C55E]`. (Hex strings are permitted *only* strictly inside ApexChart configuration blocks).
