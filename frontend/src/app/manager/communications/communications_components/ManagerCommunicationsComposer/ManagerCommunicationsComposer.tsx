...
export default function ManagerCommunicationsComposer() {
  ...
  return <div className="bg-card border border-border rounded-xl p-6 space-y-6">
    ...
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-4 border-t border-border"><div className="flex items-center gap-2 text-sm text-secondary">{loadingRecipients ? <Loader2 size={15} className="motion-safe:animate-spin" /> : <Users size={15} />}{loadingRecipients ? 'Loading recipients...' : `${segmentRecipients.length} recipient${segmentRecipients.length === 1 ? '' : 's'}`}</div><div className="flex gap-3">{segmentRecipients.length > 0 && <button type="button" onClick={() => setShowPreview(true)} className="flex items-center gap-2 px-4 py-2 bg-input border border-border rounded-lg text-sm font-medium"><Eye size={15} />Preview & Send</button>}<button type="button" onClick={submit} disabled={sending || loadingRecipients || segmentRecipients.length === 0} className="flex items-center gap-2 px-5 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-semibold disabled:opacity-50">{sending ? <Loader2 size={15} className="motion-safe:animate-spin" /> : <Send size={15} />}{sending ? 'Sending...' : 'Send Campaign'}</button></div></div>
