...
export function useManagerCommunicationsForm() {
  ...
  const form = useForm<CommFormValues>({ resolver: zodResolver(managerCommunicationsFormSchema as any), defaultValues: { title: '', channel: selectedChannel, segment: selectedSegment, message: templateDefaults.message, subject: templateDefaults.subject } });
