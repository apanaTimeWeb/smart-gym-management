...
2. **Isolated State Management (No Prop-Drilling):**
   - The state is managed locally via React Context in `plans_context/` or a module-scoped store.
   - The heavy logic (data fetching, calculations) is extracted into custom hooks.

3. **Centralized Hardcoded Data:**
   - Any UI text, default arrays, dropdown options, or mock data MUST be placed in `plans_utils/`. 
   - Never hardcode these inside the `.tsx` view files.

4. **Types and Interfaces:**
   - All TypeScript interfaces and types are strictly isolated in `plans_types/`.
...
6. **Absolute Imports:**
   ...
   - Always use absolute imports starting with `@/` (e.g., `@/app/manager/plans/...`).
