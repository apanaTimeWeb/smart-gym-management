    ...
    <>
      <div className="flex flex-wrap gap-2 mb-4 bg-card border border-border p-1 rounded-xl w-fit">
        {MANAGER_PLANS_TABS.map((tab) => (
          <button key={tab} type="button" onClick={() => handleTabChange(tab)} className={`px-4 py-2 text-sm font-semibold rounded-lg motion-safe:transition-colors ${activeTab === tab ? 'bg-primary-subtle text-on-info shadow' : 'text-secondary hover:text-primary hover:bg-primary-subtle'}`}>{tab}</button>
        ))}
      </div>
      ...
      {activeTab === 'Membership Activate' && (
        <form onSubmit={submitActivate} className="bg-card border border-border rounded-xl p-6">
          ...
          <div className="space-y-4 max-w-md">
            ...
            <button type="submit" disabled={activateMutation.isPending} className="min-w-32 w-full py-2 bg-primary-subtle text-on-info font-semibold rounded-lg disabled:opacity-60 motion-safe:transition-colors">{activateMutation.isPending ? 'Activating…' : 'Activate Membership'}</button>
          </div>
        </form>
      )}
