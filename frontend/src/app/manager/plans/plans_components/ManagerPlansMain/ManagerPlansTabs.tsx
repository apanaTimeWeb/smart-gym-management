    ...
    <>
      <div className="flex flex-wrap gap-2 mb-4 bg-card border border-border p-1 rounded-xl w-fit">
        {MANAGER_PLANS_TABS.map((tab) => (
          <button key={tab} type="button" onClick={() => handleTabChange(tab)} className={`px-4 py-2 text-sm font-semibold rounded-lg motion-safe:transition-colors ${activeTab === tab ? 'bg-primary text-on-info shadow' : 'text-secondary hover:text-on-info hover:bg-primary-subtle'}`}>{tab}</button>
        ))}
      </div>

      {activeTab === 'Membership Activate' && (
        <form onSubmit={submitActivate} className="bg-card border border-border rounded-xl p-6">
          <h3 className="text-lg font-bold text-on-info mb-4">Activate New Membership</h3>
          <div className="space-y-4 max-w-md">
            ...
            <div><label htmlFor="manager-plan-start-date" className="block text-sm font-medium mb-1">Start Date</label><input id="manager-plan-start-date" type="date" {...activateForm.register('startDate')} className="w-full bg-input border border-border rounded-lg px-3 py-2 text-sm text-on-info" /></div>
            <button type="submit" disabled={activateMutation.isPending} className="min-w-32 w-full py-2 bg-primary text-on-info font-semibold rounded-lg disabled:opacity-60 motion-safe:transition-colors">{activateMutation.isPending ? 'Activating…' : 'Activate Membership'}</button>
          </div>
        </form>
      )}
      ...
      {activeTab === 'Membership Renew' && (
        <div className="bg-card border border-border rounded-xl p-6">
          <h3 className="text-lg font-bold text-on-info mb-4">Renew Memberships</h3>
