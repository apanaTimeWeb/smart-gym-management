...
export function useManagerHrQueries(params: { search: string; role: string; month: string }, initialData?: HrInitialData | null) {
  const queryClient = useQueryClient();
  const queryParams = useMemo(() => ({ search: params.search, role: params.role }), [params.role, params.search]);
  ...
  const staffQuery = useQuery({ queryKey: staffKey, queryFn: async () => (await hrApi.getStaff(queryParams)).data ?? { staff: [], total: 0 }, initialData: initialData ? { staff: initialData.staff, total: initialData.staff.length } : undefined });
  const payrollQuery = useQuery({ queryKey: payrollKey, queryFn: async () => (await hrApi.getPayrolls({ month: params.month })).data ?? { payrolls: [], total: 0 }, initialData: initialData ? { payrolls: initialData.payrolls, total: initialData.payrolls.length } : undefined });
  const summaryQuery = useQuery({ queryKey: summaryKey, queryFn: async () => (await hrApi.getSummary()).data ?? null, initialData: initialData?.summary ?? undefined });
  ...
  return {
    staff: staffQuery.data?.staff ?? [], payrolls: payrollQuery.data?.payrolls ?? [], summary: summaryQuery.data ?? null,
