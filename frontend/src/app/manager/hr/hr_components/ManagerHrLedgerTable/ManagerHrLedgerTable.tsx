              ...
              {loading ? (
                ...
                ledger.map(l => (
                  <tr key={l.id} className="hover:bg-secondary/5 motion-safe:transition-colors">
                    ...
                    <td className="p-4 text-right text-on-primary font-medium">{l.credit > 0 ? `+${formatCurrencyFromMinorUnits(l.credit, ManagerEnvConfig.currencyCode)}` : '—'}</td>
                    <td className="p-4 text-right text-on-primary font-medium">{l.debit > 0 ? `-${formatCurrencyFromMinorUnits(l.debit, ManagerEnvConfig.currencyCode)}` : '—'}</td>
                    <td className="p-4 text-right font-bold text-on-primary bg-primary/5">{formatCurrencyFromMinorUnits(l.balance, ManagerEnvConfig.currencyCode)}</td>
                  </tr>
                ))
              )}
            </tbody>
