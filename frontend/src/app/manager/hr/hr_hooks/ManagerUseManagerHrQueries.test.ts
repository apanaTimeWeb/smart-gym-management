...
import * as moduleUnderTest from '@/app/manager/hr/hr_hooks/ManagerUseManagerHrQueries';
...
describe('ManagerUseManagerHrQueries co-located hook contract', () => {
  it('exports the expected callable hook', () => {
    expect(typeof moduleUnderTest.useManagerHrQueries).toBe('function');
  });

  it('can initialize its public hook contract without executing a real backend request', () => {
    const hook = moduleUnderTest.useManagerHrQueries as (...args: never[]) => unknown;
    const { result } = renderHook(() => hook(1, '' as never));
