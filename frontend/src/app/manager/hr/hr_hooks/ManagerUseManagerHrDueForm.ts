...
export function useManagerHrDueForm() {
  ...
  const staffOptions = useMemo(() => staff.map((member) => ({ value: String(member.id), label: `${member.name} (${member.role}) — Due: ${formatCurrency(member.currentDue || 0, ManagerEnvConfig.currencyCode, locale)}` })), [staff]);
