...
describe('ManagerUseManagerStoreProducts co-located hook contract', () => {
  it('exports the expected callable hook', () => {
    expect(typeof moduleUnderTest.useManagerStoreProducts).toBe('function');
  });

  it('can initialize its public hook contract without executing a real backend request', () => {
    const hook = moduleUnderTest.useManagerStoreProducts as (...args: never[]) => unknown;
    const { result } = renderHook(() => hook('' as never, '' as never, '' as never, '' as never));
