...
import * as moduleUnderTest from '@/app/manager/store/store_hooks/ManagerUseManagerStoreQueries';
...
describe('ManagerUseManagerStoreQueries co-located hook contract', () => {
  it('exports the expected callable hook', () => {
    expect(typeof moduleUnderTest.useManagerStoreQueries).toBe('function');
  });

  it('can initialize its public hook contract without executing a real backend request', () => {
    const hook = moduleUnderTest.useManagerStoreQueries as (...args: never[]) => unknown;
    const { result } = renderHook(() => hook(1, '' as never, '' as never, '' as never, '' as never, '' as never, '' as never, '' as never, '' as never));
