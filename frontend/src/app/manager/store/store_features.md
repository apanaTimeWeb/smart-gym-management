...
| `store_context/useManagerStoreLogic.ts` | URL and local state orchestration |
| `store_context/useManagerStoreQueries.ts` | Data fetching and local filtering |
| `store_context/useManagerStoreProducts.ts` | Product mutation operations |
| `store_context/useManagerStoreOrder.ts` | Order placement logic |
...
- **Server-state:** Fetched via `storeApi` inside `useManagerStoreQueries.ts`. No client-generated Order IDs or Product IDs — backend/MSW owns all IDs.
...
## MSW Mock Layer
- **Handler file**: `src/mocks/handlers/manager-store.handlers.ts`
...
- **URL sync:** Filters sync directly to URL via `useManagerStoreLogic.ts`. Never filter/paginate client-side for list views.
