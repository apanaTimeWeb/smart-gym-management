...
import * as moduleUnderTest from '@/app/manager/members/members_hooks/ManagerUseManagerMembersStatusMutations';
...
describe('ManagerUseManagerMembersStatusMutations co-located hook contract', () => {
  it('exports the expected callable hook', () => {
    expect(typeof moduleUnderTest.useManagerMembersStatusMutations).toBe('function');
  });

  it('can initialize its public hook contract without executing a real backend request', () => {
    const hook = moduleUnderTest.useManagerMembersStatusMutations as (...args: never[]) => unknown;
    const { result } = renderHook(() => hook('' as never, '' as never));
