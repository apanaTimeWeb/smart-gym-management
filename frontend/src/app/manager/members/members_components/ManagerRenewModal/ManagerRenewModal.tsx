  ...
  return <div className="fixed inset-0 bg-overlay-backdrop z-40 flex items-center justify-center p-4">
    <div className="bg-overlay rounded-2xl shadow-dialog w-full max-w-xl max-h-full overflow-y-auto border-2 border-primary">
      <div className="sticky top-0 px-8 py-5 border-b border-border bg-overlay flex items-center justify-between z-10">
        ...
        <button type="button" onClick={handleClose} aria-label="Close renewal form" className="p-2 rounded-full hover:bg-primary/10 motion-safe:transition-colors text-secondary hover:text-on-primary"><X size={18} /></button>
      </div>
      <form onSubmit={submit} className="p-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <fieldset className="sm:col-span-2 flex gap-4 p-1.5 bg-input rounded-xl border border-border w-fit">
            ...
            <label className={`flex-1 flex text-center cursor-pointer px-4 py-1.5 rounded-lg text-sm font-semibold motion-safe:transition-all ${actionType === 'renew' ? 'bg-primary text-on-success shadow-card' : 'text-secondary hover:text-on-success'}`}><input type="radio" value="renew" {...register('actionType')} className="sr-only" />Renew Plan</label>
            <label className={`flex-1 flex text-center cursor-pointer px-4 py-1.5 rounded-lg text-sm font-semibold motion-safe:transition-all ${actionType === 'upgrade' ? 'bg-primary text-on-success shadow-card' : 'text-secondary hover:text-on-success'}`}><input type="radio" value="upgrade" {...register('actionType')} className="sr-only" />Upgrade Plan</label>
          </fieldset>
          ...
          {billingCycle === 'CUSTOM' && <div className="sm:col-span-2"><label htmlFor="manager-renew-custom-days" className="block text-sm font-medium text-secondary mb-1.5">Custom Days</label><input id="manager-renew-custom-days" type="number" min="1" step="1" {...register('customDays', { valueAsNumber: true })} className="w-full border rounded-xl px-4 py-3 text-sm bg-input text-on-success border-border" />{errors.customDays && <p className="text-danger text-xs mt-1">{errors.customDays.message}</p>}</div>}
          {planId && <div className="sm:col-span-2 bg-primary/10 rounded-xl p-4 text-sm border border-primary/30 flex justify-between items-center"><span className="font-semibold text-on-success">{actionType === 'renew' ? 'Renewal' : 'Upgrade'} Price</span><span className="text-on-success font-bold">{formatCurrencyFromMinorUnits(calculatedPrice, ManagerEnvConfig.currencyCode)}</span>{!selectedPlan && <span className="sr-only">Selected plan details unavailable</span>}</div>}
          <div><label htmlFor="manager-renew-paid" className="block text-sm font-medium text-secondary mb-1.5">Amount Paid (₹)</label><input id="manager-renew-paid" type="number" min="0" step="0.01" {...register('paidAmount', { valueAsNumber: true })} className="w-full border rounded-xl px-4 py-3 text-sm bg-input text-on-success border-border" />{errors.paidAmount && <p className="text-danger text-xs mt-1">{errors.paidAmount.message}</p>}</div>
