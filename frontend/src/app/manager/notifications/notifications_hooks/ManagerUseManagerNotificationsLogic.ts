...
import type { ManagerNotificationsViewModel } from '@/app/manager/notifications/notifications_types/ManagerNotificationsViewModelTypes';
...
export function useManagerNotificationsLogic(): ManagerNotificationsViewModel {
  ...
  const handleMarkRead = useCallback(async (id: string) => { await readMutation.mutateAsync(id); }, [readMutation]);
  const handleMarkAllRead = useCallback(async () => { await readAllMutation.mutateAsync(); }, [readAllMutation]);
  return { notifications: listQuery.data?.notifications ?? [], kpis: kpiQuery.data ?? null, isPending: listQuery.isPending || kpiQuery.isPending, isError: listQuery.isError || kpiQuery.isError, errorMessage, saving: readMutation.isPending || readAllMutation.isPending || deleteMutation.isPending, search, setSearch: (value) => setUrlParam("search", value), typeFilter, setTypeFilter: (value) => setUrlParam("type", value), priorityFilter, setPriorityFilter: (value) => setUrlParam("priority", value), statusFilter, setStatusFilter: (value) => setUrlParam("status", value), handleMarkRead, handleMarkAllRead, handleDelete, reload: async () => { await Promise.all([listQuery.refetch(), kpiQuery.refetch()]); } };
}
