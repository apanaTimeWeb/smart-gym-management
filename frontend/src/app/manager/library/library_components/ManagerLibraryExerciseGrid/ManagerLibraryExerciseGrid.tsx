      ...
      <div className="grid flex-1 grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
        {exercises.map((exercise) => (
          <article key={exercise.id} className="flex cursor-pointer flex-col rounded-xl border border-border bg-card p-5 shadow-card motion-safe:transition-all motion-safe:duration-base motion-safe:hover:-translate-y-1" tabIndex={0} onClick={() => openEditExercise(exercise)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openEditExercise(exercise); } }}>
            <div className="mb-3 flex items-start justify-between gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-info-bg text-on-info"><Dumbbell size={18} aria-hidden="true" /></div><div className="flex gap-1">
              ...
              <button type="button" onClick={(event) => { event.stopPropagation(); void deleteExercise(exercise.id); }} aria-label={`Delete ${exercise.name}`} className="min-h-11 min-w-11 rounded-lg text-on-danger hover:bg-danger-bg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danger motion-safe:transition-all motion-safe:duration-base"><Trash2 size={18} className="mx-auto" aria-hidden="true" /></button>
            </div></div>
            ...
            <div className="mt-auto flex items-center justify-between border-t border-border pt-3"><span className="rounded-full bg-info-bg px-2.5 py-1 text-badge font-semibold text-on-info">{exercise.difficulty}</span><span className="text-xs text-secondary">{exercise.duration ? `${exercise.duration} min` : `${exercise.sets ?? '—'} sets · ${exercise.reps ?? '—'} reps`}</span></div>
          </article>
        ))}
      </div>
