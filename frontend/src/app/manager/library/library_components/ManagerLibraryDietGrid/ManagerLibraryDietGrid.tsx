      ...
      <div className="grid flex-1 grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
        {dietPlans.map((diet) => (
          <article key={diet.id} className="flex cursor-pointer flex-col rounded-xl border border-border bg-card p-5 shadow-card motion-safe:transition-all motion-safe:duration-base motion-safe:hover:-translate-y-1" tabIndex={0} onClick={() => openEditDiet(diet)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openEditDiet(diet); } }}>
            <div className="mb-3 flex items-start justify-between gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-success-bg text-on-success"><Apple size={18} aria-hidden="true" /></div><div className="flex gap-1">
              ...
              <button type="button" onClick={(event) => { event.stopPropagation(); void deleteDietPlan(diet.id); }} aria-label={`Delete ${diet.name}`} className="min-h-11 min-w-11 rounded-lg text-on-danger hover:bg-danger-bg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danger motion-safe:transition-all motion-safe:duration-base"><Trash2 size={18} className="mx-auto" aria-hidden="true" /></button>
            </div></div>
