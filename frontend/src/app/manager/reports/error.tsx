...
// EFFECT: Effect lifecycle and dependency list are intentionally scoped to values that control this side effect.
  useEffect(() => {
    logger.error('Manager module route error', {
      route: ManagerReportsUrlConfig.PAGES.LIST,
  ...
  return (
    <div className="min-h-full flex items-center justify-center p-6 bg-page">
      <div className="bg-overlay border border-danger p-8 rounded-2xl shadow-dialog max-w-md w-full text-center space-y-4">
        ...
        <h2 className="text-xl font-bold text-primary">Reports Unavailable</h2>
        <p className="text-sm text-secondary">We couldn't load the reports module. Please try again.</p>
                <button
          onClick={reset}
