...
import { useTrainerProfileLogic } from '@/app/trainer/profile/profile_hooks/useTrainerProfileLogic';
...
export default function TrainerProfileMain() {
  ...
  return <div className="min-h-full pb-10"><div className="max-w-3xl mx-auto space-y-6 p-6">
    <div className="bg-card border border-border rounded-xl p-6 flex items-center gap-5"><div className="w-16 h-16 rounded-full bg-primary flex items-center justify-center text-2xl font-bold text-on-primary shrink-0">{logic.displayInitial}</div><div><p className="text-lg font-bold text-primary">{user.name}</p><p className="text-sm text-secondary">{user.email}</p></div></div>
    <div className="flex flex-wrap gap-1 bg-input border border-border rounded-xl p-1 w-fit">
      <button type="button" onClick={() => void guardNavigation(() => setActiveTab('personal'))} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium motion-safe:transition-colors ${activeTab === 'personal' ? 'bg-card text-on-primary shadow-card' : 'text-secondary hover:text-on-primary'}`}><User size={16} />Personal Info</button>
      <button type="button" onClick={() => void guardNavigation(() => setActiveTab('security'))} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium motion-safe:transition-colors ${activeTab === 'security' ? 'bg-card text-on-primary shadow-card' : 'text-secondary hover:text-on-primary'}`}><Lock size={16} />Security</button>
    </div>
    {activeTab === 'personal' && (<form onSubmit={profileForm.handleSubmit((values) => profileMutation.mutate(values))} className="bg-card border border-border rounded-xl p-6 space-y-5"><h2 className="text-base font-semibold text-on-primary">Personal Information</h2><div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
      <div><label htmlFor="trainer-profile-name" className="block text-sm font-medium text-secondary mb-1.5">Full Name</label><input id="trainer-profile-name" {...profileForm.register('name')} className="w-full px-4 py-2.5 bg-input border border-border rounded-lg text-on-primary text-sm" />{profileForm.formState.errors.name && <p className="text-xs text-danger mt-1">{profileForm.formState.errors.name.message}</p>}</div>
      ...
      <div><label htmlFor="trainer-profile-phone" className="block text-sm font-medium text-secondary mb-1.5">Phone Number</label><input id="trainer-profile-phone" {...profileForm.register('phone')} className="w-full px-4 py-2.5 bg-input border border-border rounded-lg text-on-primary text-sm" />{profileForm.formState.errors.phone && <p className="text-xs text-danger mt-1">{profileForm.formState.errors.phone.message}</p>}</div>
      <Controller control={profileForm.control} name="specialization" render={({ field }) => <div><span className="block text-sm font-medium text-secondary mb-2">Specialization</span><div className="flex flex-wrap gap-2">{TRAINER_SPECIALIZATIONS.map((item) => { const selected = field.value.includes(item); return <button key={item} type="button" aria-pressed={selected} onClick={() => field.onChange(selected ? field.value.filter((value) => value !== item) : [...field.value, item])} className={`px-3 py-1.5 rounded-full text-xs font-semibold border motion-safe:transition-all ${selected ? 'bg-primary text-on-primary border-primary' : 'bg-input text-secondary border-border'}`}>{item}</button>; })}</div></div>} />
    </div><div className="flex justify-end"><button type="submit" disabled={profileMutation.isPending} className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-page min-w-32 flex items-center justify-center gap-2 px-5 py-2.5 bg-primary text-on-primary font-semibold rounded-lg disabled:opacity-60">{profileMutation.isPending ? <><Loader2 size={16} className="motion-safe:animate-spin" />Saving…</> : <><Save size={16} />Save Changes</>}</button></div></form>)}
    {activeTab === 'security' && (<form onSubmit={passwordForm.handleSubmit((values) => passwordMutation.mutate(values))} className="bg-card border border-border rounded-xl p-6 space-y-5"><h2 className="text-base font-semibold text-on-primary">Change Password</h2>{passwordFields.map((field) => { const visible = passwordVisibility[field.key]; return <div key={field.name} className="max-w-md"><label htmlFor={`trainer-${field.name}`} className="block text-sm font-medium text-secondary mb-1.5">{field.label}</label><div className="relative"><input id={`trainer-${field.name}`} type={visible ? 'text' : 'password'} {...passwordForm.register(field.name)} className="w-full px-4 py-2.5 pr-10 bg-input border border-border rounded-lg text-on-primary text-sm" /> <button type="button" aria-label={visible ? 'Hide password' : 'Show password'} onClick={() => setPasswordVisibility((current) => ({ ...current, [field.key]: !current[field.key] }))} className="absolute right-3 top-1/2 -translate-y-1/2 text-secondary hover:text-on-primary">{visible ? <EyeOff size={16} /> : <Eye size={16} />}</button></div>{passwordForm.formState.errors[field.name] && <p className="text-xs text-danger mt-1">{passwordForm.formState.errors[field.name]?.message}</p>}</div>; })}<div className="flex justify-end"><button type="submit" disabled={passwordMutation.isPending} className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-page min-w-36 flex items-center justify-center gap-2 px-5 py-2.5 bg-primary text-on-primary font-semibold rounded-lg disabled:opacity-60">{passwordMutation.isPending ? <><Loader2 size={16} className="motion-safe:animate-spin" />Updating…</> : <><Lock size={16} />Update Password</>}</button></div></form>)}
  </div></div>;
}
