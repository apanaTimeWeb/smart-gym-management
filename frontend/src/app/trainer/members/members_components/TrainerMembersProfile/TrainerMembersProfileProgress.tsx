...
export default function TrainerMembersProfileProgress() {
  ...
  const { data: entries = [], isPending, isError } = useTrainerProgressEntriesQuery(memberId ?? '');
  ...
  return <div className="space-y-6">
    <div className="flex items-center justify-between"><h3 className="font-bold text-foreground">Progress & Measurements</h3><span className="text-xs text-secondary">Source: Trainer Progress API</span></div>
    {latest ? <>
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">{metrics.map(([label,current,change]) => <div key={label} className="bg-card border border-border rounded-xl p-4"><p className="text-xs text-secondary mb-1">{label}</p><p className="text-lg font-bold text-foreground">{current}</p><p className="text-xs text-secondary mt-1">{displayValue(change)}</p></div>)}</div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-card border border-border rounded-xl p-5"><h4 className="font-semibold text-foreground mb-4">Measurement History</h4><div className="space-y-3">{ordered.map((entry) => <div key={entry.id} className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3"><span className="text-sm text-secondary">{new Date(entry.date).toLocaleDateString()}</span><span className="text-sm font-semibold">{formatNumber(entry.weightKg)} kg</span><span className="text-sm text-secondary">BMI {formatNumber(entry.bmi)}</span><span className="text-sm text-secondary">Body Fat {entry.bodyFatPercent == null ? '—' : `${formatNumber(entry.bodyFatPercent)}%`}</span></div>)}</div></div>
        <div className="bg-card border border-border rounded-xl p-5 flex flex-col items-center justify-center text-center"><div className="w-12 h-12 rounded-full bg-info-bg text-info flex items-center justify-center mb-3"><Calculator size={24}/></div><h4 className="text-sm font-semibold text-secondary">Current BMI</h4><p className="text-3xl font-bold text-foreground">{bmi == null ? '—' : formatNumber(bmi)}</p></div>
      </div>
      <div className="bg-card border border-border rounded-xl p-5"><div className="flex items-center gap-2 mb-4"><Camera size={16} className="text-primary"/><h4 className="font-semibold text-foreground">Progress Photos</h4></div>{latest.progressPhotos?.length ? <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{latest.progressPhotos.map((src, index) => <div key={src} className="aspect-[3/4] rounded-lg overflow-hidden bg-input"><Image src={src} alt={`Progress photo ${index + 1}`} fill sizes="(max-width: 768px) 50vw, 25vw" className="object-cover" /></div>)}</div> : <p className="text-sm text-secondary">No progress photos recorded.</p>}</div>
