...
export default function TrainerMembersProfileProgress() {
  ...
  if (isError) return <div className="rounded-xl border border-danger bg-danger-bg p-4 text-on-danger">Unable to load progress data. Retry the progress request.</div>;
  ...
  return <div className="space-y-6">
    ...
    {latest ? <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5 flex flex-col items-center justify-center text-center"><div className="w-12 h-12 rounded-full bg-info-bg text-on-info flex items-center justify-center mb-3"><Calculator size={24}/></div><h4 className="text-sm font-semibold text-secondary">Current BMI</h4><p className="text-3xl font-bold text-primary">{bmi == null ? '—' : formatNumber(bmi)}</p></div>
      </div>
