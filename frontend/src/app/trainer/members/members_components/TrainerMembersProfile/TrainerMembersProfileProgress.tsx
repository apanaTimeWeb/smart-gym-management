...
export default function TrainerMembersProfileProgress() {
  ...
  return <div className="space-y-6">
    ...
    {latest ? <>
      ...
      <div className="bg-card border border-border rounded-xl p-5"><div className="flex items-center gap-2 mb-4"><Camera size={16} className="text-primary"/><h4 className="font-semibold text-foreground">Progress Photos</h4></div>{latest.progressPhotos?.length ? <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{latest.progressPhotos.map((src, index) => <div key={src} className="aspect-square rounded-lg overflow-hidden bg-input"><Image src={src} alt={`Progress photo ${index + 1}`} fill sizes="(max-width: 768px) 50vw, 25vw" className="object-cover" /></div>)}</div> : <p className="text-sm text-secondary">No progress photos recorded.</p>}</div>
