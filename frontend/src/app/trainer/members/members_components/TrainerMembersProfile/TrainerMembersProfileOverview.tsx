...
export default function TrainerMembersProfileOverview() {
  ...
  return <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
    ...
    <div><h3 className="font-semibold text-primary mb-3">Member Actions</h3><div className="bg-floating rounded-xl p-4 mb-4 border border-border"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-primary-subtle text-primary flex items-center justify-center"><Target size={18} /></div><div><p className="text-xs text-secondary">Coaching Status</p><p className="text-sm font-semibold text-primary">{displayValue(selectedMember.progressStatus)}</p></div></div></div><div className="flex flex-col gap-2"><button type="button" onClick={() => openMsg({ name: selectedMember.name, phone: selectedMember.phone, email: selectedMember.email }, 'whatsapp', '')} className="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-on-success rounded-xl justify-center bg-success motion-safe:transition-colors hover:bg-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-page"><MessageCircle size={18} />Send WhatsApp</button><button type="button" onClick={() => openMsg({ name: selectedMember.name, phone: selectedMember.phone, email: selectedMember.email }, 'email', '')} className="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-on-success rounded-xl justify-center bg-info text-on-info motion-safe:transition-colors hover:bg-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-page"><Mail size={18} />Send Email</button></div></div>
  </div>;
}
