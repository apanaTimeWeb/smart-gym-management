...
export const TrainerMembersApi = {
  ...
  updateMember: async (id: string, body: Partial<Member>) => {
    const response = await apiFetch<ApiResponse<unknown>>(MembersUrlConfig.BACKEND_API.UPDATE(id), {
      method: 'PATCH',
  ...
  fetchMemberAttendance: async (memberId: string) => {
    ...
    const response = await apiFetch<ApiResponse<unknown>>(`${MembersUrlConfig.BACKEND_API.GET_ONE(memberId)}/attendance`);
