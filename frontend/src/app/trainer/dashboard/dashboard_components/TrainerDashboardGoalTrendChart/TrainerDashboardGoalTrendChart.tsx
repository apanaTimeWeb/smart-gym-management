...
export default function TrainerDashboardGoalTrendChart() {
  ...
  if (isPending) return <div className="bg-card rounded-xl border border-border p-5 min-h-[300px] motion-safe:animate-pulse" aria-label="Loading goal trend" />;
  if (isError || !stats?.goalCompletionTrend?.length) return <div className="bg-card rounded-xl border border-border p-5 min-h-[300px] flex items-center justify-center"><p className="text-secondary text-sm">No goal completion data available.</p></div>;
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-5 min-h-[300px]">
      ...
      <div className="w-full overflow-x-auto" role="img" aria-label="Goal completion trend chart">
        <svg viewBox={`0 0 ${width} ${height + 40}`} className="w-full min-w-[520px] h-56" preserveAspectRatio="none">
          ...
          {stats.goalCompletionTrend.map((point, index) => { const x = pad + index * step; const y = height - pad - ((point.rate - min) / Math.max(max - min, 1)) * (height - pad * 2); return <g key={point.month}><circle cx={x} cy={y} r="5" fill="currentColor" className="text-primary" /><text x={x} y={height + 20} textAnchor="middle" className="fill-secondary text-[11px]">{point.month}</text></g>; })}
        </svg>
      </div>
    </div>
  );
