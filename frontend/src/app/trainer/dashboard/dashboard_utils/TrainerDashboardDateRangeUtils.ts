...
import type { TrainerDashboardDateRange } from '@/app/trainer/dashboard/dashboard_utils/TrainerDashboardDateRangeConstants';

const toDateValue = (date: Date) => `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;

export function resolveTrainerDashboardDateRange(range: TrainerDashboardDateRange, referenceDate = new Date()) {
