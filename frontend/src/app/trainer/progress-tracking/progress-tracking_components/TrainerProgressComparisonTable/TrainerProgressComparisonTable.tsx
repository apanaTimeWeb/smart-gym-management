...
import TrainerProgressComparisonDelta from '@/app/trainer/progress-tracking/progress-tracking_components/TrainerProgressComparisonDelta/TrainerProgressComparisonDelta';
            ...
            {snapshots.map(s => {
              ...
              return (
                <tr key={s.memberId} className="hover:bg-surface-hover motion-safe:transition-colors motion-safe:duration-base">
                  ...
                  <td className="px-4 py-3 text-primary">{s.latestWeightKg == null ? '—' : formatNumber(s.latestWeightKg)}</td>
                  <td className="px-4 py-3 text-primary">{s.latestBmi == null ? '—' : formatNumber(s.latestBmi)}</td>
                  <td className="px-4 py-3 text-primary">{s.latestBodyFatPercent == null ? '—' : `${formatNumber(s.latestBodyFatPercent)}%`}</td>
                  <td className="px-4 py-3 text-primary">{s.latestMuscleMassKg == null ? '—' : formatNumber(s.latestMuscleMassKg)}</td>
                  ...
                  <td className="px-4 py-3 text-secondary">{formatNumber(s.totalEntries)}</td>
                </tr>
              );
            })}
          </tbody>
