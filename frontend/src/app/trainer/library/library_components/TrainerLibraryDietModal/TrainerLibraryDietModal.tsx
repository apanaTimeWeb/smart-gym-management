...
export default function TrainerLibraryDietModal({ isOpen, plan, onClose, onAssign }: TrainerLibraryDietModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-overlay/80 z-40 flex items-center justify-center p-4" role="presentation">
      <div className="bg-overlay border border-border rounded-2xl shadow-card w-full max-w-lg max-h-screen overflow-y-auto" role="dialog" aria-modal="true" aria-labelledby="trainer-diet-plan-title">
        <div className="sticky top-0 bg-overlay px-6 py-4 border-b border-border flex items-center justify-between">
          <div className="min-w-0">
            <h3 id="trainer-diet-plan-title" className="text-lg font-bold text-primary truncate">{plan.name}</h3>
            <p className="text-xs text-secondary mt-0.5">{plan.goal} · {plan.isActive ? 'Active' : 'Inactive'}</p>
          </div>
          {onAssign && <button type="button" onClick={onAssign} className="w-full py-2.5 bg-primary text-on-primary rounded-xl text-sm font-semibold hover:bg-primary-hover motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">Assign to Member</button>}
        ...
        <div className="p-6 space-y-4">
          ...
          {plan.meals?.length ? <div><p className="text-xs font-semibold text-secondary uppercase tracking-wider mb-2"><Utensils size={18} className="inline mr-1" />Meal Plan</p><ul className="space-y-1.5">{plan.meals.map((meal, index) => <li key={`${typeof meal === 'string' ? meal : meal.name}-${typeof meal === 'string' ? 'text' : meal.description ?? 'meal'}`} className="flex items-start gap-2 text-sm text-primary"><span className="shrink-0 mt-0.5 w-5 h-5 rounded-full bg-primary-subtle text-on-primary flex items-center justify-center text-xs font-bold">{index + 1}</span><div><span className="font-medium">{typeof meal === 'string' ? meal : meal.name}</span>{typeof meal !== 'string' && meal.description ? <span className="block text-xs text-secondary">{meal.description}</span> : null}</div></li>)}</ul></div> : null}
          ...
          <button type="button" onClick={onClose} className="w-full py-2.5 border border-border rounded-xl text-sm font-medium text-on-primary hover:bg-primary-subtle motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">Close</button>
        </div>
      </div>
    </div>
  );
