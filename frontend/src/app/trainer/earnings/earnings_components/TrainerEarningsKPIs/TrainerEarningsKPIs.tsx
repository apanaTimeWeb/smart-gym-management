...
export default function TrainerEarningsKPIs() {
  ...
  const cards = [
    { label: 'Total Earnings' + dateSuffix, value: formatCurrency(kpis.totalEarnings), icon: IndianRupee, color: 'text-success', bg: 'bg-success-bg' },
    { label: 'Pending Payouts' + dateSuffix, value: formatCurrency(kpis.pendingPayouts), icon: Clock, color: 'text-warning', bg: 'bg-warning-bg' },
    { label: 'Sessions Completed' + dateSuffix, value: kpis.sessionsCompleted.toString(), icon: Activity, color: 'text-info', bg: 'bg-info-bg' },
    { label: 'Commission Rate' + dateSuffix, value: `${kpis.commissionRate}%`, icon: Target, color: 'text-primary', bg: 'bg-primary-subtle' },
    { label: 'Tax Deducted (TDS)' + dateSuffix, value: formatCurrency(kpis.taxDeduction), icon: Minus, color: 'text-danger', bg: 'bg-danger-bg' },
  ];
