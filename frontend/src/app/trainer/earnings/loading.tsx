export default function TrainerEarningsLoading() {
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-card rounded-xl border border-border p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex gap-2">
          {[1, 2, 3].map((i) => (
            <div key={`skeleton-${i}`} className="h-8 w-16 bg-skeleton-base motion-safe:animate-pulse rounded-lg" />
          ))}
        </div>
      ...
      {[1, 2, 3, 4].map((i) => (
        <div key={`skeleton-${i}`} className="bg-card rounded-xl border border-border p-5 flex gap-4">
