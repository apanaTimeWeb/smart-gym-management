...
const SOCIAL_ICONS = [FbIcon, InstaIcon, XIcon, YtIcon] as const;
const QUICK_LINKS  = ['About Us', 'Membership Plans', 'Trainers', 'Classes Schedule', 'Gallery', 'Contact Us'];
const PROGRAMS     = ['Bodybuilding', 'Weight Loss', 'CrossFit', 'Yoga Classes', 'Zumba Dance', 'Personal Training'];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {PROGRAMS.map(l => (
                <li key={l}>
                  <Link href={LandingUrlConfig.ANCHORS.SERVICES} className="text-secondary hover:text-warning text-sm transition-colors">{l}</Link>
                </li>
              ))}
            </ul>
          </div>
