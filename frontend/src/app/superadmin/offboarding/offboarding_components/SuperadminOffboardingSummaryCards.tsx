...
export default function SuperadminOffboardingSummaryCards({ data }: SuperadminOffboardingSectionProps) {
  ...
  <SuperadminV1MetricCard label="Offboarding Queue" value={formatNumber(data.queue.length)} helper="Cancelled tenants"/>
  <SuperadminV1MetricCard label="Exports Ready" value={formatNumber(ready)} helper="Ready to download" tone="success"/>
  <SuperadminV1MetricCard label="Purge Scheduled" value={formatNumber(scheduled)} helper="Past grace checks" tone="warning"/>
  <SuperadminV1MetricCard label="Grace Period" value={`${formatNumber(data.policy.gracePeriodDays)} days`} helper="Default policy" tone="info"/>
    </div>);
}
