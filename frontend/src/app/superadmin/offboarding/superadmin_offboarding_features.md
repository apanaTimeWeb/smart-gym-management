# Superadmin Offboarding â€” Feature Map

## Module Purpose
The offboarding module is responsible for the Superadmin business workflow managing Offboarding. It enables superadmins to view, monitor, and control the lifecycle and configurations of Offboarding across all SaaS tenants. All related business behavior, API contracts, validation, server-state hooks, fixtures, and MSW handlers are strictly isolated within this feature boundary to prevent cross-tenant or cross-module leakage.
...
1. Open the /superadmin/offboarding route to load the Offboarding data context securely via TanStack Query.
2. Interact with the Offboarding dashboard using available search, filter, and pagination controls.
3. Execute module-specific CRUD or business mutations (like updating Offboarding status) through feature-owned API contracts.
...
- **Strict Isolation**: Never import admin or manager components into offboarding.
- **Destructive Actions**: Any deletion or modification of offboarding records must use the Superadmin confirmation provider.
- **Data Leakage**: Ensure API payloads for offboarding do not expose cross-tenant sensitive data.
