# Platform Governance Settings — Feature Map

## Module Purpose
This Superadmin-only feature gives platform operators a focused workspace for platform governance settings. It exists at `/superadmin/settings` and is intentionally limited to platform-level tenant/SaaS operations; gym-staff daily operations are outside this boundary. The V1 layer keeps the UI backend-ready by flowing server data through its module API, Zod contract, module-owned MSW fixture/handler, and TanStack Query hook.

## Directory Structure
| Folder | Responsibility | Key Files |
|---|---|---|
| `settings/` | Owns this Superadmin route and all feature-specific artifacts. | `page.tsx`, `loading.tsx`, `error.tsx`, `SuperadminSettingsV1Client.tsx` |
| `settings_api/` | Calls the feature endpoint and validates the response data. | API service for `GET /api/superadmin/settings/governance` |
| `settings_types/` | Owns the response data contract and runtime validation. | V1 type/schema file |
| `settings_mocks/handlers/` | Intercepts the V1 endpoint in frontend-first development. | V1 MSW handler |
| `settings_mocks/fixtures/` | Owns realistic V1 server-like data. | V1 mock fixture |
| `settings_utils/` | Orchestrates TanStack Query for this feature. | `useSuperadminSettingsV1.ts` |

## Feature Inventory
| Feature | Route | What the User Can Do | Main API Calls | Status |
|---|---|---|---|---|
| Platform Governance Settings | `/superadmin/settings` | billing controls, security controls, data controls, communication defaults. | `GET /api/superadmin/settings/governance` | ✅ V1 mocked and rendered |

## User Flows & Interactions
1. Superadmin opens `/superadmin/settings` and the client view requests the feature payload through `useSuperadminSettingsV1.ts`.
...
- **Mock ownership:** fixture and MSW handler both remain inside `settings/`.
...
|---|---|---|---|---|
| `fetchSettingsGovernance` | GET | `GET /api/superadmin/settings/governance` | None in V1 | Module V1 data schema |
...
|---|---|---|
| `data.billing` | GET /api/superadmin/settings/governance | Module-owned Zod contract + MSW fixture |
| `data.label` | GET /api/superadmin/settings/governance | Module-owned Zod contract + MSW fixture |
| `data.value` | GET /api/superadmin/settings/governance | Module-owned Zod contract + MSW fixture |
| `data.security` | GET /api/superadmin/settings/governance | Module-owned Zod contract + MSW fixture |
| `data.data` | GET /api/superadmin/settings/governance | Module-owned Zod contract + MSW fixture |
| `data.communication` | GET /api/superadmin/settings/governance | Module-owned Zod contract + MSW fixture |
| `data.message` | GET /api/superadmin/settings/governance | Module-owned Zod contract + MSW fixture |
| `data.success` | GET /api/superadmin/settings/governance | Module-owned Zod contract + MSW fixture |
...
|---|---|
| `SuperadminSettingsV1Client.tsx` | Renders the Superadmin-only feature view and consumes the query state. |
| `useSuperadminSettingsV1.ts` | Owns TanStack Query orchestration and exposes server state without JSX. |
