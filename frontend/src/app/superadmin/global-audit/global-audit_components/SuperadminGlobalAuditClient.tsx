...
export default function SuperadminGlobalAuditClient() {
    ...
    const { logs: displayLogs, total: totalLogs, totalPages, isPending, isFetching, error: queryError, refetch } = useSuperadminGlobalAuditData(search, severityFilter, actorTypeFilter, currentPage, ITEMS_PER_PAGE);
    ...
    if (queryError) return <div className="p-8 text-center text-danger" role="alert">{queryError instanceof Error ? queryError.message : ''}</div>;
    return (<div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-primary">Global Audit Logs</h1>
        ...
        <div className="p-4 border-b border-border bg-surface-hover/50 flex flex-col sm:flex-row items-center gap-4 justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="w-48 z-20 border-none bg-input rounded-lg">
              <SearchableDropdown options={severityOptions} value={severityFilter} onChange={(val) => setSeverityFilter(val as AuditSeverityFilter)} className="bg-transparent border-transparent"/>
            </div>
            <div className="w-48 z-20 border-none bg-input rounded-lg">
              <SearchableDropdown options={actorTypeOptions} value={actorTypeFilter} onChange={(val) => setActorTypeFilter(val as AuditActorFilter)} className="bg-transparent border-transparent"/>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-border">
              {paginatedLogs.map((log: AuditLog) => (<tr key={log.id} className="hover:bg-surface-hover motion-safe:transition-colors">
