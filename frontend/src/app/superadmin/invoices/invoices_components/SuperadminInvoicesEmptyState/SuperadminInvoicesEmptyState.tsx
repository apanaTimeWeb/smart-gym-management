...
export default function SuperadminInvoicesEmptyState({ onLogPaymentClick }: InvoicesEmptyStateProps) {
    return (<div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <button onClick={onLogPaymentClick} className="mt-4 px-4 py-2 bg-primary hover:bg-primary-hover text-white text-sm font-medium rounded-lg motion-safe:transition-all motion-safe:duration-base motion-safe:ease-in-out motion-safe:active:scale-95">
        Log Manual Payment
      </button>
    </div>);
}
