      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {Object.entries(agingBuckets).map(([bucket, data]) => (<div key={bucket} className="bg-input border border-border rounded-xl p-4 shadow-card flex flex-col justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-primary mt-2">{formatCurrency(data.amount)}</p>
            ...
            {data.invoices.length > 0 && (<div className="mt-4 pt-4 border-t border-border/50 max-h-32 overflow-y-auto scrollbar-thin">
                {data.invoices.map(inv => (<div key={inv.id} className="flex justify-between text-xs py-1">
                    <span className="text-primary truncate max-w-24" title={inv.tenantName}>{inv.tenantName}</span>
