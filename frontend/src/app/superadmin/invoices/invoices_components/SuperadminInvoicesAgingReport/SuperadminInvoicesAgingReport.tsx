...
export default function SuperadminInvoicesAgingReport({ invoices }: {
    ...
    return (<div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {Object.entries(agingBuckets).map(([bucket, data]) => (<div key={bucket} className="bg-input border border-border rounded-xl p-4 shadow-sm flex flex-col justify-between">
            <div>
            ...
            {data.invoices.length > 0 && (<div className="mt-4 pt-4 border-t border-border/50 max-h-32 overflow-y-auto scrollbar-thin">
                {data.invoices.map(inv => (<div key={inv.id} className="flex justify-between text-xs py-1">
                    <span className="text-foreground truncate max-w-24" title={inv.tenantName}>{inv.tenantName}</span>
