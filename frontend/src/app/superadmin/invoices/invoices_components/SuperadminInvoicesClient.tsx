      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
        <div className="p-4 border-b border-border flex flex-col md:flex-row gap-4 justify-between items-center bg-input/20">
          <div className="flex bg-input border border-border rounded-lg p-1 w-full md:w-auto">
            <button onClick={() => setActiveTab('ALL')} className={`flex-1 md:flex-none px-4 py-2 text-sm rounded-md font-medium motion-safe:transition-colors ${activeTab === 'ALL' ? 'bg-background text-foreground shadow-sm' : 'text-secondary hover:text-foreground hover:bg-background/50'}`}>
              All Invoices
            </button>
            <button onClick={() => setActiveTab('AGING')} className={`flex-1 md:flex-none px-4 py-2 text-sm rounded-md font-medium motion-safe:transition-colors ${activeTab === 'AGING' ? 'bg-background text-foreground shadow-sm' : 'text-secondary hover:text-foreground hover:bg-background/50'}`}>
              Aging Report
            </button>
          </div>
        </div>
        ...
        {activeTab === 'AGING' ? (<SuperadminInvoicesAgingReport invoices={invoices}/>) : (<>
            <div className="p-4 border-b border-border flex gap-4">
              <div className="relative flex-1 max-w-md">
                ...
                <input type="text" placeholder="Search by invoice ID or gym name..." className="w-full pl-9 pr-4 py-2 bg-input border border-border rounded-lg text-sm text-foreground focus:outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-bg-page focus:border-primary" value={search} onChange={(e) => setSearch(e.target.value)}/>
              </div>
