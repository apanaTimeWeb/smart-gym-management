# Forbidden Patterns for `superadmin/invoices`
...
2. **No Relative Imports:** Never use `./` or `../../` in any file. Always use absolute paths starting with `@/app/superadmin/invoices/...`.
...
8. **No Localized API Calls:** UI components should never call `apiFetch` directly. They must trigger actions in custom hooks or stores, which then communicate with `invoices_api`.
