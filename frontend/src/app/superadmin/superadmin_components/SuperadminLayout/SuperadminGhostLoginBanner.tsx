...
export default function SuperadminGhostLoginBanner() {
    ...
    if (!ghostTenant)
        return null;
    return (<div className="sticky top-0 z-20 w-full bg-warning/15 border-b-2 border-warning/40 px-4 py-2.5 flex items-center justify-between gap-3">
      ...
      <button onClick={exitGhostLogin} className="flex items-center gap-1.5 px-3 py-1.5 bg-warning/20 hover:bg-warning/30 border border-warning/40 rounded-lg text-xs font-bold text-warning motion-safe:transition-all motion-safe:duration-base motion-safe:active:scale-95 shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-warning" aria-label="Exit ghost login session">
