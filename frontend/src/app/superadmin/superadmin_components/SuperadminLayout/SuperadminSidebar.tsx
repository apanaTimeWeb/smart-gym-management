...
import { LayoutDashboard, Building2, ServerCog, LogOut, ChevronLeft, ChevronRight, CreditCard, Ticket, Activity, HardDrive, DatabaseBackup, Receipt, ToggleLeft, DatabaseZap, Megaphone, Tag, Users, BarChart2, BarChart3, Gauge, Settings, History, UserPlus, MessageSquare, FileBarChart, TrendingDown, UserCircle, Send, Search, Dumbbell, Server, UsersRound, PlugZap, ArchiveX, Landmark, ListFilter, } from 'lucide-react';
...
const ALL_NAV_GROUPS = [
    ...
    },
    {
        ...
        items: [
            ...
            { name: 'Saved Tenant Segments', href: SuperadminSegmentsUrlConfig.PAGES.MAIN, icon: ListFilter },
        ]
    },
    {
        ...
        items: [
            ...
            { name: 'Tenant Offboarding', href: SuperadminOffboardingUrlConfig.PAGES.MAIN, icon: ArchiveX },
    ...
    return (<>
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 flex flex-col bg-sidebar border-r border-border motion-safe:transition-all motion-safe:duration-slow ${isCollapsed ? 'lg:w-16' : 'lg:w-60'} ${isMobileOpen ? 'w-60 translate-x-0' : 'w-60 -translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="hidden lg:flex items-center justify-end px-4 py-2 border-b border-border">
          <button onClick={() => setIsCollapsed(!isCollapsed)} aria-label={isCollapsed ? 'Expand sidebar' : 'Collapse sidebar'} className="p-1.5 rounded-lg text-secondary hover:text-foreground hover:bg-input motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
        ...
        {!isCollapsed ? (<div className="px-4 py-3 border-b border-border shrink-0">
            <div className="relative">
              ...
              <input type="text" placeholder="Search menu..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="block w-full pl-9 pr-3 py-2 border border-border rounded-lg leading-5 bg-input text-foreground placeholder-secondary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:border-primary sm:text-sm motion-safe:transition-colors"/>
            </div>
          </div>) : (<div className="flex items-center justify-center px-4 py-3 border-b border-border shrink-0">
            <button onClick={() => setIsCollapsed(false)} aria-label="Search menu" className="p-2 rounded-lg text-secondary hover:text-foreground hover:bg-input motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
              <Search size={18}/>
            </button>
          </div>)}
