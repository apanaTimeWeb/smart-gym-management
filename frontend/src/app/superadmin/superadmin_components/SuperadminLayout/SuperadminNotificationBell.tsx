...
function NotifIcon({ type }: {
    ...
    if (type === 'INFO')
        return <Info className="w-5 h-5 text-info shrink-0" strokeWidth={2}/>;
    if (type === 'WARNING')
        return <AlertTriangle className="w-5 h-5 text-warning shrink-0" strokeWidth={2}/>;
...
export default function SuperadminNotificationBell() {
    ...
    return (<div className="relative" ref={popoverRef}>
      <button onClick={() => setOpen(!open)} className="relative p-2 text-secondary hover:text-foreground hover:bg-input rounded-full motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label="View notifications">
        <Bell size={18} strokeWidth={2}/>
        {unreadCount > 0 && (<span className="absolute top-2 right-2 w-4 h-4 bg-danger text-white text-xs font-bold flex items-center justify-center rounded-full border-2 border-card">
            {unreadCount > 9 ? '9+' : unreadCount}
          </span>)}
      </button>

      {open && (<div className="absolute right-0 mt-2 w-80 sm:w-96 bg-popover rounded-2xl shadow-2xl border border-border overflow-hidden z-30 motion-safe:animate-superadmin-fade-in-up">
          <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-header">
            ...
            {unreadCount > 0 && (<button onClick={handleMarkAllRead} disabled={markAllReadMutation.isPending} className="text-xs font-semibold text-primary hover:text-primary-hover motion-safe:transition-colors flex items-center gap-1 focus-visible:outline-none focus-visible:underline">
                <CheckCheck className="w-3.5 h-3.5"/> Mark all read
              </button>)}
          </div>

          <div className="max-h-96 overflow-y-auto custom-scrollbar">
            {notifications.length === 0 ? (<div className="p-6 text-center text-secondary text-sm">
                No notifications right now.
              </div>) : (<div className="divide-y divide-border">
                {notifications.map(notif => (<div key={notif.id} onClick={() => handleMarkRead(notif.id)} className={`flex items-start gap-3 p-4 hover:bg-input cursor-pointer motion-safe:transition-colors ${notif.read ? 'opacity-70' : ''}`}>
                    ...
                    {!notif.read && (<div className="w-2 h-2 rounded-full bg-primary shrink-0 mt-1.5 shadow-sm shadow-primary/50"/>)}
                  </div>))}
              </div>)}
          </div>

          <div className="p-2 border-t border-border bg-header">
            <Link href={`${MessagingUrlConfig.PAGES.MAIN}?tab=notifications`} onClick={() => setOpen(false)} className="block w-full text-center py-2 text-sm font-semibold text-primary hover:text-primary-hover motion-safe:transition-colors rounded-lg hover:bg-primary/5 focus-visible:outline-none focus-visible:bg-primary/5">
