// DATA FLOW: Superadmin UI → useSuperadminGhostLoginStore → Superadmin module API/state → consuming component
'use client';
...
export const useSuperadminGhostLoginStore = create<SuperadminGhostLoginState>()(persist((set) => ({
    ghostTenant: null,
    startGhostLogin: (tenant) => set({ ghostTenant: tenant }),
    exitGhostLogin: async () => {
        set({ ghostTenant: null });
        try {
            await apiFetch(SuperadminUrlConfig.INFRASTRUCTURE.GHOST_LOGIN_EXIT_PROXY, {
                method: 'POST',
            });
        }
        catch { }
