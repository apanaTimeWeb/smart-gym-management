'use client';
// RESPONSIBILITY: Error boundary fallback for the Background Jobs page. Displays a module-specific error UI with a retry button.
...
export default function JobsError({
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-96 p-8 bg-card border border-border rounded-xl">
      ...
      <h2 className="text-xl font-bold text-foreground mb-2">Failed to load Background Jobs</h2>
      <p className="text-secondary text-sm max-w-md text-center mb-6">
        {error.message || 'An unexpected error occurred while loading the jobs queue.'}
      </p>
      <button
        onClick={reset}
