'use client';
// RESPONSIBILITY: Route-level error boundary for the Superadmin Profile module.

import { useEffect } from 'react';
import { AlertTriangle, RefreshCcw } from 'lucide-react';
import { logger } from '@/lib/logger';

export default function SuperadminProfileError({
  error,
  reset,
}: {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-96 p-8 bg-card border border-border rounded-xl shadow-sm">
      ...
      <button
        onClick={reset}
        className="flex items-center gap-2 bg-primary hover:bg-primary-hover text-black px-6 py-2.5 rounded-lg font-semibold motion-safe:transition-colors shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
      >
