...
export default function SuperadminProfilePersonalForm({ profile, isSaving, onSave, }: SuperadminProfilePersonalFormProps) {
    ...
    return (<form onSubmit={handleSubmit(onSave)} className="space-y-5">
      ...
      <div>
        ...
        <input type="email" value={profile.email} readOnly className={`${inputClass} cursor-not-allowed opacity-60`}/>
      ...
      <div>
        ...
        {errors.phone && (<p className="mt-1 text-xs text-danger" role="alert">{errors.phone.message}</p>)}
      </div>
