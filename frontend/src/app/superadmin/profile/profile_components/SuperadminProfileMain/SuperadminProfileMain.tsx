...
export default function SuperadminProfileMain() {
    const { activeTab, setActiveTab, profile, profileLoading, personalState, updatePersonalMutation, passwordState, updatePasswordMutation, twoFAState, toggle2FAMutation, } = useSuperadminProfilePage();
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 bg-card border border-border rounded-xl shadow-card overflow-hidden">
          ...
          {/* Tab Content */}
          <div className="p-6">
            {activeTab === 'personal' && (<SuperadminProfilePersonalForm profile={profile} isSaving={personalState === 'loading'} onSave={(data) => updatePersonalMutation.mutate(data)}/>)}
            {activeTab === 'security' && (<SuperadminProfileSecurityForm profile={profile} isSavingPassword={passwordState === 'loading'} isTogglingTwoFA={twoFAState === 'loading'} onSavePassword={(data) => updatePasswordMutation.mutate(data)} onToggle2FA={(data) => toggle2FAMutation.mutate(data)}/>)}
          </div>
        </div>
      </div>
    </div>);
