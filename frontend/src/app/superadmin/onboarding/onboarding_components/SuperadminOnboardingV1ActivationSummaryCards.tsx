...
export default function SuperadminOnboardingV1ActivationSummaryCards({ data }: SuperadminOnboardingV1SectionProps) {
  ...
  <SuperadminMetricCard label="Activation score" value={`${formatNumber(data.activation.score)}/100`} helper="Setup completion quality" tone="success"/>
  <SuperadminMetricCard label="Average activation time" value={`${data.activation.averageDays} days`} helper="From signup to active"/>
  <SuperadminMetricCard label="Gyms stalled" value={formatNumber(data.activation.stalled)} helper="Need help" tone="warning"/>
  <SuperadminMetricCard label="Trial to paid" value={formatPercent1dp(data.activation.trialToPaid)} helper="Conversion after trial" tone="success"/>
    </div>;
}
