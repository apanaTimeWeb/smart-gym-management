      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {filtered.map((tenant) => (<tr key={tenant.id} className="group">
                    ...
                    <tbody>
                      <tr tabIndex={0} aria-label={`${expandedId === tenant.id ? 'Collapse' : 'Expand'} onboarding ${tenant.gymName}`} className="hover:bg-input/30 motion-safe:transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset" onClick={() => setExpandedId(expandedId === tenant.id ? null : tenant.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setExpandedId(expandedId === tenant.id ? null : tenant.id); } }}>
                        ...
                        <td className="px-4 py-3 w-2/12">
                          ...
                          {tenant.trialStatus === 'TRIAL' && (<p className="text-xs text-secondary mt-0.5">{tenant.trialDaysLeft}d left</p>)}
                        </td>
                        <td className="px-4 py-3 w-2/12">
                          <div className="flex items-center gap-1.5">
                            <div className="w-20 h-1.5 bg-input rounded-full overflow-hidden">
                              <div className="h-full bg-primary rounded-full" style={{ width: `${(completedCount(tenant.checklist) / tenant.checklist.length) * 100}%` }}/>
                            </div>
                        ...
                        <td className="px-4 py-3 w-2/12">
                          <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                            ...
                            {tenant.trialStatus !== 'CONVERTED' && tenant.trialStatus !== 'ACTIVE' && (<button onClick={() => setConvertConfirmId(tenant.id)} aria-label="Convert to paid" title="Convert to paid" className="p-1.5 rounded-lg bg-success/10 text-success hover:bg-success/20 motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-success">
                      ...
                      {expandedId === tenant.id && (<tr className="bg-input/20">
