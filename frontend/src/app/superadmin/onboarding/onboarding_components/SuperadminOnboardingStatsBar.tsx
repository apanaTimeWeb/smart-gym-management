...
export function SuperadminOnboardingStatsBar({ stats, }: {
    stats: {
        total: number;
        completed: number;
        inProgress: number;
    ...
    return (<div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
      {statCards.map((s) => (<div key={s.label} className="bg-card border border-border rounded-xl p-4 shadow-sm motion-safe:hover:-translate-y-1 motion-safe:hover:shadow-lg motion-safe:transition-all motion-safe:duration-base" style={{ background: KPI_CARD_GRADIENT }}>
