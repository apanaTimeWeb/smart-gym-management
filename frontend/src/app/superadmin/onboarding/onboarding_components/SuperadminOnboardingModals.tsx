...
export function SuperadminOnboardingModals({ extendModalId, setExtendModalId, extendDays, setExtendDays, handleExtendTrial, convertConfirmId, setConvertConfirmId, handleConvertToPaidConfirmed, }: {
    ...
    return (<>
      ...
      {extendModalId && (<div className="fixed inset-0 z-40 flex items-center justify-center bg-overlay/80 backdrop-blur-sm" role="dialog" aria-modal="true">
          <div className="bg-overlay border border-border rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setExtendModalId(null)} className="px-4 py-2 rounded-lg bg-input text-secondary hover:text-foreground text-sm motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
                Cancel
              </button>
              <button onClick={() => handleExtendTrial(extendModalId)} className="px-4 py-2 rounded-lg bg-primary text-black font-semibold text-sm hover:bg-primary-hover motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
                Extend Trial
              </button>
            </div>
          </div>
      ...
      {convertConfirmId && (<div className="fixed inset-0 z-40 flex items-center justify-center bg-overlay/80 backdrop-blur-sm" role="dialog" aria-modal="true">
          <div className="bg-overlay border border-border rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setConvertConfirmId(null)} className="px-4 py-2 rounded-lg bg-input text-secondary hover:text-foreground text-sm motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
                Cancel
              </button>
              <button onClick={() => handleConvertToPaidConfirmed(convertConfirmId)} className="px-4 py-2 rounded-lg bg-success text-white font-semibold text-sm hover:opacity-90 motion-safe:transition-opacity focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-success">
                Yes, Convert to Paid
              </button>
            </div>
          </div>
