...
## Module Purpose
This Superadmin feature owns the `onboarding` route and its feature-specific UI, client logic, API boundary, types, schemas, constants, mocks, tests, and documentation. It is intended to be operable by the Superadmin role without importing sibling Superadmin business modules. The feature exposes only the controls represented by the current route and code in this folder. Backend authorization remains outside the frontend audit scope.

## Directory Structure

| Folder | Responsibility | Key files |
|---|---|---|
| `__tests__/` | Owns the feature responsibility represented by this folder. | `superadmin_onboarding_basic.test.tsx` |
| `onboarding_components/` | Owns the feature responsibility represented by this folder. | `SuperadminConversionFunnel.tsx`, `SuperadminOnboardingClient.tsx`, `SuperadminOnboardingModals.tsx`, `SuperadminOnboardingStatsBar.tsx`, `SuperadminOnboardingTable.tsx` |
| `onboarding_mocks/` | Owns the feature responsibility represented by this folder. | `SuperadminOnboardingMockHandlers.ts` |
| `onboarding_types/` | Owns the feature responsibility represented by this folder. | `SuperadminOnboardingConstants.ts`, `superadmin_onboarding_types.ts` |
| `superadmin_onboarding_api/` | Owns the feature responsibility represented by this folder. | `superadmin_onboarding_api.ts` |

## Feature Inventory

| Feature | Route | User action | Key API/client owner | Status |
|---|---|---|---|---|
| `onboarding` | `/superadmin/onboarding` | Use the route's controls to perform the operations implemented by the current client UI. | `feature-local API files` | Implemented in source; runtime integration **NOT VERIFIED** without installing project dependencies. |
...
| `signupDate` | Feature-owned schema/type file |
| `trialEndsAt` | Feature-owned schema/type file |
| `trialStatus` | Feature-owned schema/type file |
| `onboardingStatus` | Feature-owned schema/type file |
| `emailVerified` | Feature-owned schema/type file |
| `welcomeEmailSent` | Feature-owned schema/type file |
| `checklist` | Feature-owned schema/type file |
| `key` | Feature-owned schema/type file |
| `label` | Feature-owned schema/type file |
| `done` | Feature-owned schema/type file |
| `daysInTrial` | Feature-owned schema/type file |
| `trialDaysLeft` | Feature-owned schema/type file |
...
| File | Responsibility |
|---|---|
| `__tests__/superadmin_onboarding_basic.test.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
...
| `onboarding_components/SuperadminConversionFunnel/SuperadminConversionFunnel.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
| `onboarding_components/SuperadminOnboardingClient.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
| `onboarding_components/SuperadminOnboardingModals.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
| `onboarding_components/SuperadminOnboardingStatsBar.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
| `onboarding_components/SuperadminOnboardingTable.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
