...
## Module Purpose
This feature owns the Superadmin business workflow implemented under `infrastructure/`. The active route is `/superadmin/infrastructure`. Business behavior, API contracts, validation, server-state hooks, fixtures, MSW handlers, and tests are kept within this feature boundary. Cross-feature business logic is outside this module.
...
|---|---|---|---|---|
| Superadmin Infrastructure Api Health | `/superadmin/infrastructure` | flush; flush all; flush specific | `SuperadminInfrastructureApiHealthApi.ts`, `SuperadminInfrastructureApi.ts` | Source-verified; host runtime pending |
