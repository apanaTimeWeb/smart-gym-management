...
## Module Purpose
This Superadmin feature owns the `migrations` route and its feature-specific UI, client logic, API boundary, types, schemas, constants, mocks, tests, and documentation. It is intended to be operable by the Superadmin role without importing sibling Superadmin business modules. The feature exposes only the controls represented by the current route and code in this folder. Backend authorization remains outside the frontend audit scope.

## Directory Structure

| Folder | Responsibility | Key files |
|---|---|---|
| `__tests__/` | Owns the feature responsibility represented by this folder. | `superadmin_migrations_basic.test.tsx` |
| `migrations_components/` | Owns the feature responsibility represented by this folder. | `SuperadminMigrationsClient.tsx` |
| `migrations_mocks/` | Owns the feature responsibility represented by this folder. | `SuperadminMigrationsMockHandlers.ts` |
| `migrations_utils/` | Owns the feature responsibility represented by this folder. | `SuperadminMigrationsConstants.ts` |
| `superadmin_migrations_api/` | Owns the feature responsibility represented by this folder. | `superadmin_migrations_api.test.ts`, `superadmin_migrations_api.ts` |
| `superadmin_migrations_types/` | Owns the feature responsibility represented by this folder. | `superadmin_migrations_types.ts` |

## Feature Inventory

| Feature | Route | User action | Key API/client owner | Status |
|---|---|---|---|---|
| `migrations` | `/superadmin/migrations` | Use the route's controls to perform the operations implemented by the current client UI. | `feature-local API files` | Implemented in source; runtime integration **NOT VERIFIED** without installing project dependencies. |
...
| `appliedAt` | Feature-owned schema/type file |
| `status` | Feature-owned schema/type file |
| `targetTenants` | Feature-owned schema/type file |
| `durationMs` | Feature-owned schema/type file |
| `errorLog` | Feature-owned schema/type file |
| `executedAt` | Feature-owned schema/type file |
| `executedBy` | Feature-owned schema/type file |
| `errorDetails` | Feature-owned schema/type file |
| `name` | Feature-owned schema/type file |
| `plan` | Feature-owned schema/type file |
| `migrations` | Feature-owned schema/type file |
| `tenants` | Feature-owned schema/type file |
...
| File | Responsibility |
|---|---|
| `__tests__/superadmin_migrations_basic.test.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
...
| `migrations_components/SuperadminMigrationsClient.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
