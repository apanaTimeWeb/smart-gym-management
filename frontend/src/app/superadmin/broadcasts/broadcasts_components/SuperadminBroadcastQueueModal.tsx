...
export default function SuperadminBroadcastQueueModal({ isOpen, onClose, recipients, broadcastTitle, onComplete }: SuperadminBroadcastQueueModalProps) {
    ...
    if (!isOpen)
        return null;
    return (<div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-overlay/80 backdrop-blur-sm" role="dialog" aria-modal="true">
      <div className="bg-overlay rounded-2xl shadow-2xl w-full max-w-lg border border-border flex flex-col motion-safe:animate-in motion-safe:zoom-in-95 motion-safe:duration-base overflow-hidden">
        ...
        <div className="p-6 max-h-96 overflow-y-auto space-y-3 bg-overlay custom-scrollbar">
          {recipients.map((rec, idx) => {
            ...
            return (<div key={rec.id} className={`p-4 rounded-xl border flex items-center justify-between motion-safe:transition-colors ${isProcessing ? 'border-primary bg-primary-subtle' :
