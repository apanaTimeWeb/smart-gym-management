# Broadcasts Feature Map

## Module Purpose
Handles broadcasts operations, UI display, and logic isolation as part of the Smart Gym 360 platform.

## Directory Structure
- `broadcasts_components/`: Contains all isolated micro-components for the module.
- `broadcasts_types/` (if applicable): TypeScript definitions.
- `broadcasts_utils/` (if applicable): Shared constants and hardcoded data.
- `broadcasts_context/` (if applicable): Module-scoped React Context or Zustand store.

## Feature Inventory
| Feature | Path | Purpose | Main API Calls | Owner |
|---|---|---|---|---|
| Core UI | `/broadcasts` | Main module view | TBD | Frontend Team |

## Data and State Architecture
- Server-state query keys: `['broadcasts']`
- Zustand stores: TBD
