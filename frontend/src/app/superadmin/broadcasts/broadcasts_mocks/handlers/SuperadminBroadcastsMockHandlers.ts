...
const TENANTS = [
  ...
  { id: 't4', name: 'Zenith Fitness', plan: 'Pro' },
  { id: 't5', name: 'Peak Performance', plan: 'Enterprise' },
];
let mockBroadcasts = [...MOCK_SUPERADMIN_BROADCASTS];

export const superadminBroadcastsHandlers = [
  http.get(BroadcastsUrlConfig.BACKEND_API.TENANTS, async () => HttpResponse.json({ success: true, message: 'Success', data: TENANTS })),
  http.get(BASE_URL, async ({ request }) => {
    ...
    if (search) filtered = filtered.filter((b) => b.title.toLowerCase().includes(search) || b.content.toLowerCase().includes(search));
    if (status !== 'ALL' && status !== 'FAILED') filtered = filtered.filter((b) => b.status === status);
    if (status === 'FAILED') filtered = filtered.filter((b) => (b.failedCount ?? 0) > 0);
