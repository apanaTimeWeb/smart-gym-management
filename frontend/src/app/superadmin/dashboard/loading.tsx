    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        {Array.from({ length: 10 }, (_, i) => (
          <div
            key={`kpi-skeleton-${i}`}
            className="bg-skeleton-base border border-border rounded-xl p-6 h-32 motion-safe:animate-pulse"
          />
        ))}
      </div>
      ...
      <div className="bg-skeleton-base border border-border rounded-xl p-6 motion-safe:animate-pulse">
        ...
        <div className="space-y-4">
          {Array.from({ length: 5 }, (_, i) => (
            <div key={`onboard-skeleton-${i}`} className="flex items-center justify-between p-4 bg-skeleton-highlight rounded-lg">
