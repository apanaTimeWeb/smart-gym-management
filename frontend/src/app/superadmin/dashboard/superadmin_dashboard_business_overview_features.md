...
## Module Purpose
This feature owns the Superadmin business workflow implemented under `dashboard/`. The active route is `/superadmin/dashboard`. Business behavior, API contracts, validation, server-state hooks, fixtures, MSW handlers, and tests are kept within this feature boundary. Cross-feature business logic is outside this module.
