    ...
    return (<div className="bg-card border border-border rounded-xl p-6 shadow-sm">
      ...
      <div className="space-y-4">
        {recentOnboards.map((tenant) => {
            ...
            return (<div key={tenant.id} onClick={() => router.push(`${DashboardUrlConfig.PAGES.GYMS}?id=${tenant.id}`)} className="flex items-center justify-between p-4 bg-background rounded-lg border border-border hover:bg-input motion-safe:transition-colors motion-safe:duration-base cursor-pointer">
