...
export default function SuperadminSegmentsSummaryCards({ data }: SuperadminSegmentsSectionProps) {
  ...
  <MetricCard label="Saved Segments" value={formatNumber(data.segments.length)} helper="Reusable groups"/>
  <MetricCard label="Gyms Covered" value={formatNumber(covered)} helper="Across saved groups"/>
  <MetricCard label="Quick Presets" value={formatNumber(data.presets.length)} helper="Common questions" tone="info"/>
  <MetricCard label="Largest Group" value={formatNumber(largestGroup)} helper="Gyms in one group" tone="success"/>
    </div>);
}
