...
export default function SuperadminSegmentsSummaryCards({ data }: SuperadminSegmentsSectionProps) {
  ...
  <SuperadminV1MetricCard label="Saved Segments" value={formatNumber(data.segments.length)} helper="Reusable groups"/>
  <SuperadminV1MetricCard label="Gyms Covered" value={formatNumber(covered)} helper="Across saved groups"/>
  <SuperadminV1MetricCard label="Quick Presets" value={formatNumber(data.presets.length)} helper="Common questions" tone="info"/>
  <SuperadminV1MetricCard label="Largest Group" value={formatNumber(largestGroup)} helper="Gyms in one group" tone="success"/>
    </div>);
}
