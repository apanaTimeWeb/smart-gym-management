...
export default function SuperadminBranchesClient() {
    ...
    return (<div className="space-y-6">
      ...
      <div className="relative max-w-sm flex-1">
        ...
        <input type="text" placeholder="Search branch, gym, city..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="w-full pl-9 pr-4 py-2 bg-input border border-border rounded-lg text-sm text-foreground focus:outline-none focus:border-primary focus-visible:ring-2 focus-visible:ring-primary"/>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {branches.map((branch: SuperadminBranch) => (<tr key={branch.id} className="hover:bg-input/30 motion-safe:transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {branch.status === 'SUSPENDED' ? (<button onClick={() => onActivateClick(branch.id)} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-success/10 text-success text-xs font-medium hover:bg-success/20 motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-success">
                        <CheckCircle2 className="w-3 h-3"/> Activate
                      </button>) : (<button onClick={() => onSuspendClick(branch.id)} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-danger/10 text-danger text-xs font-medium hover:bg-danger/20 motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danger">
