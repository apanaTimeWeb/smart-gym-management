...
## Module Purpose
This Superadmin feature owns the `branches` route and its feature-specific UI, client logic, API boundary, types, schemas, constants, mocks, tests, and documentation. It is intended to be operable by the Superadmin role without importing sibling Superadmin business modules. The feature exposes only the controls represented by the current route and code in this folder. Backend authorization remains outside the frontend audit scope.

## Directory Structure

| Folder | Responsibility | Key files |
|---|---|---|
| `__tests__/` | Owns the feature responsibility represented by this folder. | `superadmin_branches_basic.test.tsx` |
| `branches_components/` | Owns the feature responsibility represented by this folder. | `SuperadminBranchesClient.tsx` |
| `branches_mocks/` | Owns the feature responsibility represented by this folder. | `SuperadminBranchesMockHandlers.ts` |
| `branches_types/` | Owns the feature responsibility represented by this folder. | `superadmin_branches_types.ts` |
| `branches_utils/` | Owns the feature responsibility represented by this folder. | `SuperadminBranchesConstants.ts`, `useSuperadminBranchesPage.test.ts`, `useSuperadminBranchesPage.ts` |
| `superadmin_branches_api/` | Owns the feature responsibility represented by this folder. | `superadmin_branches_api.ts` |

## Feature Inventory

| Feature | Route | User action | Key API/client owner | Status |
|---|---|---|---|---|
| `branches` | `/superadmin/branches` | Use the route's controls to perform the operations implemented by the current client UI. | `feature-local API files` | Implemented in source; runtime integration **NOT VERIFIED** without installing project dependencies. |
...
| `managerId` | Feature-owned schema/type file |
| `location` | Feature-owned schema/type file |
...
| `branchName` | Feature-owned schema/type file |
| `city` | Feature-owned schema/type file |
| `state` | Feature-owned schema/type file |
| `managerName` | Feature-owned schema/type file |
| `managerEmail` | Feature-owned schema/type file |
...
| File | Responsibility |
|---|---|
| `__tests__/superadmin_branches_basic.test.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
| `branches_components/SuperadminBranchesClient.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
