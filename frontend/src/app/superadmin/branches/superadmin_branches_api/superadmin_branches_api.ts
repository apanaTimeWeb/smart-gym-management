...
export const superadminBranchesApi = {
    ...
    updateBranch: (id: string, body: Partial<SuperadminBranch>) => apiFetch<ApiResponse<SuperadminBranch>>(`${BranchesUrlConfig.BACKEND_API.BASE}/${id}`, {
        method: 'PATCH',
        body: JSON.stringify(body),
        dataSchema: SuperadminBranchSchema
    }),
    suspendBranch: (id: string) => apiFetch<ApiResponse<void>>(`${BranchesUrlConfig.BACKEND_API.BASE}/${id}/suspend`, {
        method: 'POST',
        dataSchema: z.object({}).passthrough()
    }),
    activateBranch: (id: string) => apiFetch<ApiResponse<void>>(`${BranchesUrlConfig.BACKEND_API.BASE}/${id}/activate`, {
        method: 'POST',
        dataSchema: z.object({}).passthrough()
    }),
};
