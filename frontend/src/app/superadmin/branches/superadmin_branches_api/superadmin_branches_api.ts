...
export const superadminBranchesApi = {
  fetchBranches: (params?: Record<string, string>) => {
    ...
    return apiFetch<ApiResponse<SuperadminBranch[]>>(`${BranchesUrlConfig.BACKEND_API.BASE}${q}`, { dataSchema: z.any() });
  },
  fetchBranchById: (id: string) =>
    apiFetch<ApiResponse<SuperadminBranch>>(`${BranchesUrlConfig.BACKEND_API.BASE}/${id}`, { dataSchema: z.any() }),
