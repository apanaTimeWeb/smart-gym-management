...
export const superadminBranchesApi = {
  fetchBranches: (params?: Record<string, string>) => {
    ...
    return apiFetch<ApiResponse<SuperadminBranch[]>>(`${BranchesUrlConfig.BACKEND_API.BASE}${q}`, { dataSchema: z.unknown() });
  },
  fetchBranchById: (id: string) =>
    apiFetch<ApiResponse<SuperadminBranch>>(`${BranchesUrlConfig.BACKEND_API.BASE}/${id}`, { dataSchema: z.unknown() }),
