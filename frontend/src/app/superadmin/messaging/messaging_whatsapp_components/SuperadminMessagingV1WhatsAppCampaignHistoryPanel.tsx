...
interface SuperadminMessagingV1WhatsAppCampaignHistoryPanelProps {
    campaigns: SuperadminWhatsAppCampaign[];
}
export default function SuperadminMessagingV1WhatsAppCampaignHistoryPanel({ campaigns }: SuperadminMessagingV1WhatsAppCampaignHistoryPanelProps) {
    return (<SuperadminV1Panel title="Campaign history" description="Recent free WhatsApp queues created from this Superadmin workspace.">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {campaigns.map((campaign) => (<tr key={campaign.id} className="border-b border-border">
                  ...
                  <td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-xs font-semibold uppercase ${getSuperadminStatusBadgeClasses(campaign.status)}`}>{campaign.status}</span></td>
