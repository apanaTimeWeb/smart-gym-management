...
interface SuperadminMessagingV1WhatsAppAudiencePanelProps {
...
export default function SuperadminMessagingV1WhatsAppAudiencePanel({ audiences, recipients, audienceId, tenantId, onAudienceChange, onTenantChange }: SuperadminMessagingV1WhatsAppAudiencePanelProps) {
    ...
    return (<SuperadminV1Panel title="Choose tenant audience" description="Superadmin WhatsApp is tenant-only: target gym owners, admins, and managers—not gym members.">
      <div className="grid gap-3 xl:grid-cols-3">
        <div className="grid gap-3 sm:grid-cols-2">
          {audiences.map((audience) => {
            ...
            return (<button key={audience.id} type="button" onClick={() => onAudienceChange(audience.id)} className={`rounded-xl border p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${selected ? 'border-primary/50 bg-primary/10' : 'border-border bg-input/30 hover:border-primary/30'}`} aria-pressed={selected}>
                ...
                <p className="mt-3 text-sm font-semibold text-foreground">{audience.label}</p>
        ...
        <div className="rounded-xl border border-border bg-input/30 p-4">
          ...
          <select id="whatsapp-tenant-scope" value={tenantId} onChange={(event) => onTenantChange(event.target.value)} className="mt-3 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
            <option value="ALL_TENANTS">All gyms</option>
            {tenants.map((tenant) => <option key={tenant.id} value={tenant.id}>{tenant.name}</option>)}
          </select>
          <p className="mt-4 text-xs leading-5 text-secondary">Selected audience: <span className="font-medium text-foreground">{selectedAudience?.label ?? '—'}</span></p>
