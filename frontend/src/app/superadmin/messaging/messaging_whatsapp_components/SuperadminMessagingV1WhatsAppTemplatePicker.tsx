    ...
    return (<Panel title="WhatsApp templates" description="Ready-to-use messages for fees, renewals, maintenance, updates, and custom outreach.">
      {templates.length === 0 ? (<div className="rounded-lg border border-dashed border-border p-5 text-sm text-secondary">
          ...
          {templates.map((template) => {
                ...
                return (<button key={template.id} type="button" onClick={() => onSelect(template)} className={`rounded-xl border p-4 text-left motion-safe:transition-all motion-safe:duration-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${selected ? 'border-border bg-primary-subtle shadow-card shadow-card' : 'border-border bg-input hover:border-border hover:bg-input'}`} aria-pressed={selected}>
