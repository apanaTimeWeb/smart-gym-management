...
export default function SuperadminMessagingV1WhatsAppQueuePanel({ queue, activeIndex, onOpen, onMarkSent, onSkip, onClear, }: SuperadminMessagingV1WhatsAppQueuePanelProps) {
    if (queue.length === 0) {
        return (<SuperadminPanel title="Bulk WhatsApp queue" description="Start a campaign to build a guided sending queue.">
    ...
    return (<SuperadminPanel title="Bulk WhatsApp queue" description="Open one chat at a time, press Send in WhatsApp, then mark the item sent or skip it." action={(<button type="button" onClick={onClear} className="rounded-lg border border-border bg-input px-3 py-2 text-xs font-medium text-secondary hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
          Clear queue
        </button>)}>
      <div className="space-y-5">
        <SuperadminProgressBar label={`${formatNumber(sent + skipped)} of ${formatNumber(queue.length)} completed`} value={progress}/>
