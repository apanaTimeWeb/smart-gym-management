          ...
          <div className="grid grid-cols-2 gap-4">
            {(['maxMembers', 'maxStaff'] as const).map(field => (
              <div key={field} className="space-y-2">
                ...
                <input type="number" min="0" onKeyDown={(e) => { if (e.key === '-' || e.key === 'e' || e.key === '+') e.preventDefault(); }} {...register(field, { valueAsNumber: true })} className="w-full bg-input border border-border rounded-xl px-4 py-3 text-foreground focus:border-primary outline-none transition-colors" />
                {errors[field] && <p className="text-danger text-xs">{errors[field]?.message}</p>}
              </div>
            ))}
          </div>
