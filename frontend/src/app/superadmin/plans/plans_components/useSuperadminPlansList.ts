...
export function useSuperadminPlansList() {
 ...
 const deleteMutation=useMutation({mutationFn:(id:string)=>plansApi.deletePlan(id),onSuccess:(r,id)=>{toast.success(r.message, { id: `plan-delete-${id}` });qc.invalidateQueries({queryKey:['superadmin','plans']})},onError:(e:unknown,id)=>toast.error(e instanceof Error ? e.message : 'Failed to delete plan', { id: `plan-delete-${id}` })});
 const archiveMutation=useMutation({mutationFn:(id:string)=>plansApi.archivePlan(id),onSuccess:(r,id)=>{toast.success(r.message, { id: `plan-archive-${id}` });qc.invalidateQueries({queryKey:['superadmin','plans']})},onError:(e:unknown,id)=>toast.error(e instanceof Error ? e.message : 'Failed to archive plan', { id: `plan-archive-${id}` })});
