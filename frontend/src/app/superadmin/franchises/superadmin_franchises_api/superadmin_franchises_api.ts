...
export const superadminFranchisesApi = {
  fetchFranchises: (params?: Record<string, string>) => {
    ...
    return apiFetch<ApiResponse<SuperadminFranchise[]>>(`${FranchisesUrlConfig.BACKEND_API.BASE}${q}`, { dataSchema: z.any() });
  },
  fetchFranchiseById: (id: string) =>
    apiFetch<ApiResponse<SuperadminFranchise>>(`${FranchisesUrlConfig.BACKEND_API.BASE}/${id}`, { dataSchema: z.any() }),
