...
export const superadminFranchisesApi = {
    ...
    suspendFranchise: (id: string) => apiFetch<ApiResponse<void>>(`${FranchisesUrlConfig.BACKEND_API.BASE}/${id}/suspend`, { method: 'POST',
        dataSchema: z.object({}).passthrough()
    }),
    activateFranchise: (id: string) => apiFetch<ApiResponse<void>>(`${FranchisesUrlConfig.BACKEND_API.BASE}/${id}/activate`, { method: 'POST',
        dataSchema: z.object({}).passthrough()
    }),
    updateFranchise: (id: string, body: Partial<SuperadminFranchise>) => apiFetch<ApiResponse<SuperadminFranchise>>(`${FranchisesUrlConfig.BACKEND_API.BASE}/${id}`, {
        method: 'PATCH',
