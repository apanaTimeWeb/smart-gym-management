...
export default function SuperadminIntegrationsSummaryCards({ data }: SuperadminIntegrationsSectionProps) {
  ...
  <MetricCard label="Connected Services" value={`${connected}/${data.integrations.length}`} helper="External services" tone="success"/>
  <MetricCard label="Failed Webhooks" value={formatNumber(failed)} helper="Need review" tone="danger"/>
  <MetricCard label="Active developer keys" value={formatNumber(data.keys.filter((k) => k.status === 'ACTIVE').length)} helper="Tenant developer access" tone="info"/>
  <MetricCard label="Average Health" value={`${formatNumber(averageHealth)}%`} helper="Connection health"/>
    </div>);
}
