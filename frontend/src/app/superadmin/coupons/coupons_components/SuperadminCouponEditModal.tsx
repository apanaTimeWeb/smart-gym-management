...
import type { Coupon, CouponFormData } from '@/app/superadmin/coupons/superadmin_coupons_types/superadmin_coupons_types';
      ...
      <div className="bg-overlay border border-border rounded-2xl w-full max-w-md shadow-xl overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="flex flex-col p-7 gap-5 modal-scroll-area">
          <div className="flex flex-col gap-1.5">
            ...
            <input {...register('code')} className="w-full px-4 py-2.5 bg-input border border-border rounded-lg text-sm text-foreground font-mono uppercase focus:outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-bg-page focus:border-primary motion-safe:transition-colors" placeholder="e.g. SUMMER2026"/>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <Controller name="discountType" control={control} render={({ field }) => (<SearchableDropdown value={field.value || ''} onChange={field.onChange} options={[
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                {watch('discountType') === 'EXACT' && (<span className="absolute left-4 top-1/2 -translate-y-1/2 text-secondary text-sm font-semibold">Rs</span>)}
                <input type="number" onKeyDown={(e) => { if (e.key === '-' || e.key === 'e' || e.key === '+')
        e.preventDefault(); }} min="0" {...register('discountValue', { valueAsNumber: true })} className={`w-full ${watch('discountType') === 'EXACT' ? 'pl-9 pr-4' : 'px-4'} py-2.5 bg-input border border-border rounded-lg text-sm text-foreground focus:outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-bg-page focus:border-primary motion-safe:transition-colors`} placeholder={watch('discountType') === 'PERCENTAGE' ? '25' : '500'}/>
              </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="number" onKeyDown={(e) => { if (e.key === '-' || e.key === 'e' || e.key === '+')
        e.preventDefault(); }} min="0" {...register('maxUses', { valueAsNumber: true })} className="w-full px-4 py-2.5 bg-input border border-border rounded-lg text-sm text-foreground focus:outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-bg-page focus:border-primary motion-safe:transition-colors" placeholder="100"/>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input type="date" {...register('expiryDate')} className="w-full px-4 py-2.5 bg-input border border-border rounded-lg text-sm text-foreground focus:outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-bg-page focus:border-primary motion-safe:transition-colors"/>
          ...
          <div className="flex justify-end gap-3 mt-2 pt-5 border-t border-border">
            <button type="button" onClick={onClose} className="px-5 py-2.5 bg-transparent border border-border hover:bg-border text-foreground font-medium rounded-lg motion-safe:transition-colors text-sm">
              Cancel
            </button>
            <button type="submit" className="px-5 py-2.5 bg-primary hover:bg-primary-hover text-white font-medium rounded-lg motion-safe:transition-colors text-sm">
              Save Changes
            </button>
          </div>
        </form>
