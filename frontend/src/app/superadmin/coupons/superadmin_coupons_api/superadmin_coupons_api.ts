...
export const couponsApi = {
    ...
    createCoupon: (body: Partial<Coupon>) => apiFetch<ApiResponse<Coupon>>(CouponsUrlConfig.BACKEND_API.BASE, {
        method: 'POST',
        body: JSON.stringify(body),
        dataSchema: CouponSchema
    }),
    updateCoupon: (id: string, body: Partial<Coupon>) => apiFetch<ApiResponse<Coupon>>(`${CouponsUrlConfig.BACKEND_API.BASE}/${id}`, {
        method: 'PATCH',
        body: JSON.stringify(body),
        dataSchema: CouponSchema
    }),
    deleteCoupon: (id: string) => apiFetch<ApiResponse<void>>(`${CouponsUrlConfig.BACKEND_API.BASE}/${id}`, {
        method: 'DELETE',
        dataSchema: z.null()
    }),
    restoreCoupon: (id: string) => apiFetch<ApiResponse<Coupon>>(`${CouponsUrlConfig.BACKEND_API.BASE}/${id}/restore`, {
        method: 'POST',
        dataSchema: CouponSchema
    }),
    toggleStatus: (id: string, status: 'ACTIVE' | 'INACTIVE') => apiFetch<ApiResponse<Coupon>>(`${CouponsUrlConfig.BACKEND_API.BASE}/${id}/status`, {
        method: 'PATCH',
