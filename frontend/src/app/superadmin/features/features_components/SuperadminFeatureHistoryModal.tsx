...
export default function SuperadminFeatureHistoryModal({ isOpen, onClose, flag }: Props) {
    ...
    if (!isOpen || !flag)
        return null;
    return (<div className="fixed inset-0 z-40 flex justify-end bg-overlay/80 backdrop-blur-sm motion-safe:animate-in motion-safe:fade-in motion-safe:duration-base">
      <div className="bg-card w-full max-w-md h-full flex flex-col shadow-2xl border-l border-border motion-safe:animate-in motion-safe:slide-in-from-right-full motion-safe:duration-slow" role="dialog" aria-modal="true" aria-labelledby="modal-title">
        <div className="flex items-center justify-between p-6 border-b border-border">
          ...
          <button onClick={onClose} className="p-2 text-secondary hover:bg-input hover:text-foreground rounded-lg motion-safe:transition-colors focus:outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-bg-page focus:ring-2 focus:ring-primary" aria-label="Close modal">
            <X size={18}/>
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-border before:to-transparent">
            {history.map((log) => (<div key={log.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                {/* Icon */}
