...
import type { SuperadminErrorBoundaryProps, ErrorBoundaryState } from '@/app/superadmin/superadmin_layout/SuperadminLayout/SuperadminErrorBoundaryTypes';


export class SuperadminErrorBoundary extends React.Component<SuperadminErrorBoundaryProps, ErrorBoundaryState> {
    ...
    render() {
        if (this.state.hasError) {
            ...
            <AlertTriangle size={18} className="w-5 text-danger mb-2"/>
            <h3 className="text-sm font-semibold text-primary">Section Error</h3>
          ...
          <h2 className="text-xl font-bold text-primary mb-2">A system error occurred</h2>
          ...
          <p className="text-xs text-secondary mb-6 bg-overlay px-3 py-1.5 rounded border border-border font-mono">
            Error ID: {this.state.errorId}
          </p>
          <button onClick={this.resetErrorBoundary} className="flex items-center gap-2 bg-primary hover:bg-primary-hover text-on-primary px-6 py-2.5 rounded-lg font-medium motion-safe:transition-colors shadow-card">
