...
export default function SuperadminSidebarNavItem({ name, href, icon: Icon, isActive, isCollapsed }: SuperadminSidebarNavItemProps) {
    return (<Link href={href} title={isCollapsed ? name : undefined} aria-current={isActive ? 'page' : undefined} className={`
        ...
        ${isActive
            ? 'bg-primary-subtle text-on-primary border-l-2 border-primary shadow-card shadow-primary/20 pl-2.5'
            : 'text-secondary hover:bg-card hover:text-on-primary border-l-2 border-transparent pl-2.5'}
        ${isCollapsed ? 'justify-center pl-0' : ''}
      `}>
      <Icon className={`w-5 h-5 shrink-0 ${isActive ? 'text-on-primary' : 'text-secondary group-hover:text-on-primary'}`} strokeWidth={2}/>
