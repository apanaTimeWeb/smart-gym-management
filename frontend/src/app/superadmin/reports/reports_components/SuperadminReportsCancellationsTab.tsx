    ...
    return (<div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-card space-y-3">
          ...
          <div className="flex justify-between text-sm"><span className="text-secondary">Total Lost Monthly Income</span><span className="text-danger font-medium">{formatCurrency(totalCancelledRevenue, metrics?.currency || 'INR', locale)}</span></div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {filteredCancellationsData.map((row) => (<tr key={row.id} className="hover:bg-input motion-safe:transition-colors">
                  ...
                  <td className="px-4 py-3 text-danger font-medium">{formatCurrency(row.mrr, metrics?.currency || 'INR', locale)}</td>
