...
export default function SuperadminReportsV1PlanAndRegionComparison({ data }: SuperadminReportsV1SectionProps) {
  ...
  <Panel title="Plan comparison" description="Current income by plan.">
    <div className="h-72">
      <ApexBarChart categories={data.planComparison.map((x) => x.name)} series={[{ name: 'Monthly income', data: data.planComparison.map((x) => x.income) }]} valueFormatter={(v) => formatCurrency(v)}/>
    </div>
  </Panel>
  <Panel title="Region comparison" description="Current vs previous income by region.">
    <div className="h-72">
      <ApexBarChart categories={data.regionComparison.map((x) => x.name)} series={[{ name: 'Current', data: data.regionComparison.map((x) => x.current) }, { name: 'Previous', data: data.regionComparison.map((x) => x.previous) }]} valueFormatter={(v) => formatCurrency(v)}/>
    </div>
  </Panel>
    </div>;
}
