...
export default function SuperadminReportsClient() {
  ...
  if (isError) return <div className="flex min-h-80 flex-col items-center justify-center gap-3 rounded-xl border border-danger/30 bg-danger-bg p-8 text-center"><p className="font-medium text-danger">Reports could not be loaded.</p><button type="button" onClick={() => { void revenue.refetch(); void cancellations.refetch(); void health.refetch(); }} className="min-h-11 rounded-md border border-border px-4 py-2 text-sm text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">Retry</button></div>;
