...
## Module Purpose
This Superadmin feature owns the `usage-meters` route and its feature-specific UI, client logic, API boundary, types, schemas, constants, mocks, tests, and documentation. It is intended to be operable by the Superadmin role without importing sibling Superadmin business modules. The feature exposes only the controls represented by the current route and code in this folder. Backend authorization remains outside the frontend audit scope.

## Directory Structure

| Folder | Responsibility | Key files |
|---|---|---|
| `__tests__/` | Owns the feature responsibility represented by this folder. | `superadmin_usage-meters_basic.test.tsx` |
| `superadmin_usage-meters_api/` | Owns the feature responsibility represented by this folder. | `superadmin_usage-meters_api.ts` |
...
|---|---|---|---|---|
| `usage-meters` | `/superadmin/usage-meters` | Use the route's controls to perform the operations implemented by the current client UI. | `feature-local API files` | Implemented in source; runtime integration **NOT VERIFIED** without installing project dependencies. |
...
| `metricName` | Feature-owned schema/type file |
| `currentValue` | Feature-owned schema/type file |
| `limitValue` | Feature-owned schema/type file |
| `resetDate` | Feature-owned schema/type file |
| `smsSent` | Feature-owned schema/type file |
| `smsLimit` | Feature-owned schema/type file |
| `whatsappMessagesSent` | Feature-owned schema/type file |
| `whatsappLimit` | Feature-owned schema/type file |
| `emailsSent` | Feature-owned schema/type file |
| `emailLimit` | Feature-owned schema/type file |
| `apiCallsCount` | Feature-owned schema/type file |
| `apiCallsLimit` | Feature-owned schema/type file |
| `databaseGb` | Feature-owned schema/type file |
| `mediaGb` | Feature-owned schema/type file |
| `storageLimitGb` | Feature-owned schema/type file |
| `activeMembers` | Feature-owned schema/type file |
| `totalMembers` | Feature-owned schema/type file |
| `memberLimit` | Feature-owned schema/type file |
| `staffCount` | Feature-owned schema/type file |
| `staffLimit` | Feature-owned schema/type file |
| `billingCycleEnd` | Feature-owned schema/type file |
...
| File | Responsibility |
|---|---|
| `__tests__/superadmin_usage-meters_basic.test.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
...
| `usage-meters_components/SuperadminUsageMetersClient.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
| `usage-meters_components/SuperadminUsageMetersEmptyState/SuperadminUsageMetersEmptyState.tsx` | Owns the UI responsibility represented by its filename and current JSX. |
