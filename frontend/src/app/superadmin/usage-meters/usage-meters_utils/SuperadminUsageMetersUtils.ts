...
export const getProgressColor = (used: number, limit: number) => { const percent = limit > 0 ? (used / limit) * 100 : 0; if (percent >= 90) return 'bg-danger'; if (percent >= 75) return 'bg-warning'; return 'bg-primary'; };
