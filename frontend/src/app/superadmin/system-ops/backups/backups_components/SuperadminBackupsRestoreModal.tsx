...
export default function SuperadminBackupsRestoreModal({ isOpen, onClose, selectedBackup, restoreConfirmText, setRestoreConfirmText }: SuperadminBackupsRestoreModalProps) {
  ...
  const confirmRestore = async () => {
    ...
    try {
      ...
      const response = await restoreBackup(selectedBackup.id, idempotencyKeyRef.current);
  ...
  return (<div className="fixed inset-0 z-40 flex items-center justify-center bg-overlay p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="superadmin-backups-restore-title">
    <div className="w-full max-w-md overflow-hidden rounded-xl border border-border bg-overlay shadow-dialog motion-safe:animate-in motion-safe:fade-in">
      <div className="p-6">
        ...
        <div className="flex justify-end gap-3">
          <button type="button" onClick={() => { idempotencyKeyRef.current = null; onClose(); setRestoreConfirmText(''); }} disabled={isRestoring} className="rounded-md border border-border px-4 py-2 font-medium text-primary hover:bg-surface-hover motion-safe:transition-all motion-safe:duration-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">Cancel</button>
