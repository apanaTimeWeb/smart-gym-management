...
import type { SuperadminBackupsTriggerModalProps } from '@/app/superadmin/system-ops/backups/backups_types/SuperadminBackupsTriggerModalTypes';
...
export default function SuperadminBackupsTriggerModal({ isOpen, onClose }: SuperadminBackupsTriggerModalProps) {
  ...
  const handleCreateBackup = async () => {
    ...
    try {
      ...
      const response = await triggerBackup(idempotencyKeyRef.current);
  ...
  return (<div className="fixed inset-0 z-40 flex items-center justify-center bg-overlay p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="superadmin-backups-trigger-title">
    <div className="w-full max-w-md overflow-hidden rounded-xl border border-border bg-overlay shadow-dialog motion-safe:animate-in motion-safe:fade-in">
      <div className="p-6">
        ...
        <div className="flex justify-end gap-3">
          <button type="button" onClick={() => { idempotencyKeyRef.current = null; onClose(); }} disabled={isTriggering} className="rounded-md border border-border px-4 py-2 font-medium text-primary hover:bg-surface-hover motion-safe:transition-all motion-safe:duration-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">Cancel</button>
...
export type { SuperadminBackupsTriggerModalProps } from '@/app/superadmin/system-ops/backups/backups_types/SuperadminBackupsTriggerModalTypes';
