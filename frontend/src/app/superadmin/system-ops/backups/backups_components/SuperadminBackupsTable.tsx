        ...
        <tbody className="divide-y divide-border">
          {paginatedBackups.map((backup: BackupRecord) => (<tr key={backup.id} className="hover:bg-input motion-safe:transition-colors">
              ...
              <td className="p-4 text-right flex items-center justify-end gap-2">
                <button aria-label={`Download backup ${backup.id}`} onClick={() => void handleDownload(backup.id)} className="p-2 text-secondary hover:text-primary hover:bg-primary-subtle rounded-lg motion-safe:transition-colors disabled:opacity-30" title="Download pg_dump" disabled={backup.status !== 'SUCCESS'}>
                  <Download size={18} className="w-4"/>
                </button>
                <button aria-label={`Restore backup ${backup.id}`} onClick={() => handleRestoreClick(backup)} className="p-2 text-secondary hover:text-danger hover:bg-danger-bg rounded-lg motion-safe:transition-colors disabled:opacity-30" title="Restore Snapshot" disabled={backup.status !== 'SUCCESS'}>
                  <RotateCcw size={18} className="w-4"/>
                </button>
              </td>
            </tr>))}
          {filteredLength === 0 && (<tr>
