...
import { BackgroundJobSchema } from '@/app/superadmin/system-ops/jobs/jobs_types/SuperadminJobsTypes';
...
export const jobsApi = {
    ...
    deleteJob: (id: string, idempotencyKey?: string) => apiFetch<ApiResponse<null>>(`${JobsUrlConfig.BACKEND_API.BASE}/${id}`, {
        method: 'DELETE',
        headers: idempotencyKey ? { 'Idempotency-Key': idempotencyKey } : undefined,
        dataSchema: z.null(),
    }),
    clearCompletedJobs: (idempotencyKey?: string) => apiFetch<ApiResponse<CountResponse>>(`${JobsUrlConfig.BACKEND_API.BASE}/clear-completed`, {
        method: 'POST',
        headers: idempotencyKey ? { 'Idempotency-Key': idempotencyKey } : undefined,
