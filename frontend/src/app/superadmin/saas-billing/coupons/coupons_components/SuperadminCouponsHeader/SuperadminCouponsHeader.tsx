...
import { SuperadminCouponsDateFilterDropdown } from '@/app/superadmin/saas-billing/coupons/coupons_components/SuperadminCouponsDateFilterDropdown';
import { SUPERADMIN_COUPON_STATUS_OPTIONS } from '@/app/superadmin/saas-billing/coupons/coupons_utils/SuperadminCouponsConstants';
import type { SuperadminCouponsHeaderProps } from '@/app/superadmin/saas-billing/coupons/coupons_types/SuperadminCouponsHeaderTypes';
    ...
    return (<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 className="text-xl font-bold text-primary flex items-center gap-2">
          <Tag size={18} className="w-6 text-primary"/>
          Promotional Coupons
        </h1>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {onStatusFilterChange && (<div className="w-40 border-none bg-input rounded-lg">
            <SearchableDropdown options={SUPERADMIN_COUPON_STATUS_OPTIONS as any} value={statusFilter || 'ALL'} onChange={(val) => onStatusFilterChange(String(val))} className="bg-transparent border-border"/>
          </div>)}
