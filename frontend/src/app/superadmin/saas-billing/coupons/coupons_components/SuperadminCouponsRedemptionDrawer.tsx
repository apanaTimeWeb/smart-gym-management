...
export default function SuperadminCouponsRedemptionDrawer({ coupon, isOpen, onClose }: SuperadminCouponsRedemptionDrawerProps) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-30 bg-overlay backdrop-blur-sm motion-safe:animate-in motion-safe:fade-in" onClick={onClose} aria-hidden="true" />
      <div role="dialog" aria-modal="true" aria-label={`Redemption history for coupon ${coupon.code}`} className="fixed right-0 top-0 z-40 flex h-full w-full max-w-md flex-col border-l border-border bg-card shadow-popover">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {query.isPending ? (
            ...
            <div role="alert" className="rounded-lg border border-border bg-danger-bg p-4 text-sm text-danger">Unable to load redemption history. <button type="button" onClick={() => void query.refetch()} className="ml-1 underline underline-offset-2">Retry</button></div>
          ) : redemptions.length === 0 ? (
            <div className="rounded-lg border border-border bg-input p-8 text-center"><History size={18} className="mx-auto mb-3 text-secondary"/><p className="font-medium text-primary">No Redemptions Yet</p><p className="mt-1 text-sm text-secondary">This coupon has not been used by a gym.</p></div>
          ) : (
