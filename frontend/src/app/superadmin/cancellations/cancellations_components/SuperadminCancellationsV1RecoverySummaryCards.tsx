...
export default function SuperadminCancellationsV1RecoverySummaryCards({ data }: SuperadminCancellationsV1SectionProps) {
  ...
  <SuperadminV1MetricCard label="Gyms saved" value={formatNumber(data.outcomes.savedGyms)} helper="Retention actions worked" tone="success"/>
  <SuperadminV1MetricCard label="Income saved" value={formatCurrency(data.outcomes.savedIncome)} helper="Monthly income protected" tone="success"/>
  <SuperadminV1MetricCard label="Gyms lost" value={formatNumber(data.outcomes.lostGyms)} helper="Still churned" tone="danger"/>
  <SuperadminV1MetricCard label="Income lost" value={formatCurrency(data.outcomes.lostIncome)} helper="Monthly income gone" tone="danger"/>
    </div>;
}
