// RESPONSIBILITY: Renders the paginated cancellations alerts table.
'use client';
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border bg-input/40">
            {['Gym', 'Plan', 'Risk Level', 'Risk Score', 'Last Login', 'Member Drop', 'Monthly Income', 'Last Payment', 'Contract End', 'Action Status', 'Actions'].map((h) => (<th key={h} className="text-left px-4 py-3 text-xs font-semibold text-secondary uppercase tracking-wider whitespace-nowrap">
                {h}
              </th>))}
          </tr>
        </thead>
        <tbody className="divide-y divide-border">
          {alerts.map((alert) => (<tr key={alert.id} className="hover:bg-input/30 motion-safe:transition-colors group">
              ...
              <td className="px-4 py-3">
                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${CANCELLATIONS_RISK_STYLES[alert.riskLevel]}`}>
              ...
              <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                  <div className="w-16 h-1.5 bg-input rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${alert.riskScore >= 80 ? 'bg-danger' : alert.riskScore >= 60 ? 'bg-warning' : 'bg-success'}`} style={{ width: `${alert.riskScore}%` }}/>
                  </div>
              ...
              <td className="px-4 py-3">
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${CANCELLATIONS_ACTION_STATUS_STYLES[alert.actionStatus]}`}>
                  {alert.actionStatus}
                </span>
              </td>
              <td className="px-4 py-3">
