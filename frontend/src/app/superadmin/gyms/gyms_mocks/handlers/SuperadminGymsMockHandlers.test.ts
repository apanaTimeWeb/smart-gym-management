...
describe('Superadmin Gyms MSW integration', () => {
  it('supports filter and pagination against the authoritative 24-record fixture', async () => {
    const response = await fetch('http://localhost/api/gyms?status=ACTIVE&page=1&limit=3&sortBy=memberCount&order=asc');
  ...
  it('sorts by the populated lastActiveAt field and returns 404 for an unknown gym', async () => {
    const asc = await fetch('http://localhost/api/gyms?page=1&limit=2&sortBy=lastActiveAt&order=asc');
    const ascBody = await asc.json();
    expect(asc.ok).toBe(true);
    expect(ascBody.data.map((gym: { id: string }) => gym.id)).toEqual(['t24', 't12']);

    const missing = await fetch('http://localhost/api/gyms/does-not-exist');
  ...
  it('mutates a selected gym status and exposes the changed row in the next query', async () => {
    ...
    const update = await fetch(`http://localhost/api/gyms/${target.id}/status`, {
      method: 'PATCH',
    ...
    const list = await fetch(`http://localhost/api/gyms?search=${encodeURIComponent(target.id)}&page=1&limit=20`);
    const listBody = await list.json();
    expect(listBody.data[0].status).toBe('SUSPENDED');
  });
});
