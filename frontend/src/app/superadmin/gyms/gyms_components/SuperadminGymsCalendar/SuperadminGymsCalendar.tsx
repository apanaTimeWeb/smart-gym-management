        ...
        {days.map(day => {
            ...
            return (<div key={day} className="bg-card min-h-32 p-2 hover:bg-input/50 motion-safe:transition-colors group border-t border-border">
              ...
              <div className="space-y-1 overflow-y-auto max-h-20 scrollbar-thin">
                {dayGyms.map(gym => (<div key={gym.id} onClick={() => handleRowClick(gym)} className="text-xs p-1.5 rounded bg-background border border-border cursor-pointer hover:border-primary motion-safe:transition-colors truncate" title={`${gym.name} (${gym.plan})`}>
