    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>{data.rows.map((row) => <tr key={row.id} className="border-b border-border hover:bg-surface-hover">
          ...
          <td className="px-3 py-3 text-secondary">{row.region}</td><td className="px-3 py-3 text-secondary">{row.plan}</td><td className="px-3 py-3 text-primary">{formatCurrency(row.income)}</td><td className="px-3 py-3 text-primary">{formatNumber(row.health)}/100</td><td className="px-3 py-3 text-primary">{row.usage}%</td><td className="px-3 py-3 text-secondary">{row.lastAction ?? '—'}</td>
        </tr>)}</tbody>
      </table>
    </div>
  </SuperadminPanel>;
