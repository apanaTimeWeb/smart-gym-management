    ...
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
      <Panel title="Advanced tenant filters" description="Apply a real filter to the tenant dataset returned by the module API.">
        <div className="grid grid-cols-2 gap-2">
          {data.filters.map((filter) => (
            <button key={filter.key} type="button" aria-pressed={selectedFilterKey === filter.key} onClick={() => onFilterChange(filter.key)} className={`min-h-11 rounded-md border px-3 py-2 text-left text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary motion-safe:transition-all ${selectedFilterKey === filter.key ? 'border-primary bg-primary-subtle text-on-primary' : 'border-border bg-input text-on-primary'}`}>
              {filter.label}
            </button>
          ))}
        </div>
      ...
      <Panel title="Saved views" description="Saved views apply the same server-backed filter definitions as the filter controls.">
        <div className="space-y-2">
          {data.saved.map((view) => (
            <button key={view.key} type="button" onClick={() => onSavedViewChange(view.key)} className={`flex min-h-11 w-full items-center justify-between rounded-md border px-3 py-2 text-left text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${selectedFilterKey === view.key ? 'border-primary bg-primary-subtle text-on-primary' : 'border-border text-on-primary'}`}>
              <span className="truncate">{view.label}</span><Filter size={18} aria-hidden="true" className="text-on-primary" />
            </button>
          ))}
        </div>
      </Panel>
      ...
      <Panel title="Bulk actions" description="Select visible tenants, choose an action, and apply it through the module-owned mutation.">
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <button type="button" onClick={() => onSelectionChange(selectableRows.map((row) => row.id))} disabled={selectableRows.length === 0 || isBulkPending} className="min-h-11 px-2 text-xs font-semibold text-on-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">Select visible</button>
          </div>
          <div className="space-y-2" aria-label="Tenant selection">
            {selectableRows.map((row) => (
              <label key={row.id} className="flex min-h-11 items-center gap-3 rounded-md border border-border px-3 py-2 text-sm text-on-primary">
          ...
          <select value={pendingAction} onChange={(event) => setPendingAction(event.target.value as SuperadminGymsV1BulkAction | '')} disabled={isBulkPending} className="min-h-11 w-full rounded-md border border-border bg-input px-3 py-2 text-sm text-on-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
            <option value="">Choose bulk action</option>
            {data.bulk.map((action) => <option key={action} value={action}>{action}</option>)}
          </select>
          {pendingAction === 'Move plan' ? <select value={targetPlan} onChange={(event) => setTargetPlan(event.target.value)} className="min-h-11 w-full rounded-md border border-border bg-input px-3 py-2 text-sm text-on-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">{SUPERADMIN_GYMS_V1_PLAN_OPTIONS.map((plan) => <option key={plan} value={plan}>{plan}</option>)}</select> : null}
