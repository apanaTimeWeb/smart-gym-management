...
export default function SuperadminGymDeleteModal() {
    ...
    return (<div className="fixed inset-0 z-40 flex items-center justify-center bg-overlay/80 p-4" role="dialog" aria-modal="true">
      <div className="bg-overlay rounded-2xl p-7 max-w-md w-full border border-destructive shadow-2xl relative">
        <button onClick={closeDeleteModal} className="absolute top-5 right-5 text-secondary hover:text-foreground motion-safe:transition-colors" disabled={isDeleting}>
          <X size={18}/>
        </button>
        ...
        <div className="bg-danger-bg/20 border border-destructive/30 rounded-lg p-4 mb-6">
          ...
          <input type="text" value={confirmText} onChange={(e) => setConfirmText(e.target.value)} className="w-full bg-input border border-border rounded-lg px-4 py-2.5 text-sm text-foreground focus:border-destructive focus:outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-bg-page motion-safe:transition-colors" placeholder="Type DELETE" disabled={isDeleting}/>
        </div>

        <div className="flex justify-end gap-3">
          <button type="button" onClick={closeDeleteModal} className="px-5 py-2.5 rounded-lg text-sm font-medium text-foreground border border-border hover:bg-background motion-safe:transition-colors" disabled={isDeleting}>
            Cancel
          </button>
          <button type="button" onClick={handleConfirmDelete} className="px-5 py-2.5 rounded-lg text-sm font-medium text-white bg-danger-bg hover:bg-danger-bg/90 motion-safe:transition-colors disabled:opacity-50 disabled:cursor-not-allowed" disabled={!isDeleteEnabled}>
