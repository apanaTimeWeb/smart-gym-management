...
export default function SuperadminGymsToolbar() {
    ...
    return (<div className="p-4 border-b border-border flex items-center gap-4">
      ...
      <div className="flex bg-input border border-border rounded-lg p-1">
        <button onClick={() => setViewMode('list')} className={`px-3 py-1.5 text-sm rounded-md motion-safe:transition-colors ${viewMode === 'list' ? 'bg-page text-primary shadow-card' : 'text-secondary hover:text-primary hover:bg-page/50'}`}>
          List
        </button>
        <button onClick={() => setViewMode('calendar')} className={`px-3 py-1.5 text-sm rounded-md motion-safe:transition-colors ${viewMode === 'calendar' ? 'bg-page text-primary shadow-card' : 'text-secondary hover:text-primary hover:bg-page/50'}`}>
          Calendar
        </button>
      </div>
