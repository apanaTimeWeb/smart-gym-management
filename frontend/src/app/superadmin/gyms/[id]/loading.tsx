  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[1, 2, 3, 4].map((i) => (
          <div key={`skeleton-${i}`} className="h-28 bg-skeleton-base motion-safe:animate-pulse rounded-xl border border-border"></div>
        ))}
      </div>
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="p-0">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={`skeleton-${i}`} className="h-16 border-b border-border bg-skeleton-highlight motion-safe:animate-pulse opacity-50"></div>
          ))}
        </div>
      </div>
    </div>
