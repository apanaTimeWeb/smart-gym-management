...
export function useSuperadminSystemClient() {
    ...
    return { tab, setTab, logSearch, setLogSearch: (value: string) => { setParam('logSearch', value); setParam('page', '1'); }, currentPage, setCurrentPage: (value: number) => setParam('page', String(value)), migratingTenants, tenants, logs, totalPages, isLoading: migrationsQuery.isLoading || auditQuery.isLoading, isError: migrationsQuery.isError || auditQuery.isError, handleRunMigration, handleExportCSV, refetch };
}
