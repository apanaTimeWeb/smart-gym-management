...
import { SUPERADMIN_SYSTEM_SLA_STATUS_STYLES } from '@/app/superadmin/system/system_utils/SuperadminSystemConstants';
...
export default function SuperadminSystemSlaTab() {
  ...
  const debouncedSlaSearch = useSuperadminDebouncedValue(slaSearch);
  const setSlaSearch = (val: string) => setParam('search', val);

  const handleGenerateCredit = async (tenantId: string) => {
    try { const response = await systemApi.generateDowntimeCredit(tenantId); toast.success(response.message, { id: `sla-credit-${tenantId}` }); }
    catch (error: unknown) { toast.error(error instanceof Error ? error.message : 'SLA credit request failed.', { id: `sla-credit-${tenantId}` }); }
  };

  const queryParams = useMemo(() => {
    const params: Record<string, string> = {};
    if (debouncedSlaSearch) params.search = debouncedSlaSearch;
    return params;
  }, [debouncedSlaSearch]);
    ...
    <div className="space-y-8 motion-safe:animate-superadmin-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-card border border-border rounded-xl p-6">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-primary/10 rounded-lg text-primary"><ServerCog size={18} strokeWidth={2} /></div>
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-danger/10 rounded-lg text-danger"><AlertCircle size={18} strokeWidth={2} /></div>
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-success/10 rounded-lg text-success"><CheckCircle size={18} strokeWidth={2} /></div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden flex flex-col min-h-96">
        <div className="p-4 border-b border-border flex justify-between items-center bg-header">
          <h2 className="text-lg font-bold text-foreground flex items-center gap-2"><Clock size={18} strokeWidth={2} className="text-primary"/> Gym Uptime Status</h2>
          <div className="relative">
            <Search size={18} strokeWidth={2} className="absolute left-3 top-1/2 -translate-y-1/2 text-disabled" />
            <input
              type="text"
            ...
            <tbody className="divide-y divide-border">
              {slaData.map((sla) => (
                <tr key={sla.id} className="superadmin-table-row group hover:bg-input motion-safe:transition-colors text-sm">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${SUPERADMIN_SYSTEM_SLA_STATUS_STYLES[sla.status]}`}>
                      {sla.status}
                    </span>
                  </td>
                  <td className="p-4 text-right">
                    {sla.status === 'BREACHED' && (
                      ...
                      >
                        <Ticket size={18} strokeWidth={2} /> Issue Credit
                      </button>
                    )}
                    {sla.status !== 'BREACHED' && (
