...
export const systemApi = {
    ...
    startMigration: (tenantId: string, idempotencyKey?: string) => apiFetch<ApiResponse<void>>(SystemUrlConfig.BACKEND_API.MIGRATION_TRIGGER, { method: 'POST', body: JSON.stringify({ tenantId }), headers: idempotencyKey ? { 'Idempotency-Key': idempotencyKey } : undefined, dataSchema: z.null() }),
    ...
    issueDowntimeCredit: (tenantId: string, idempotencyKey?: string) => apiFetch<ApiResponse<null>>(SystemUrlConfig.BACKEND_API.SLA_CREDIT(tenantId), { method: 'POST', headers: idempotencyKey ? { 'Idempotency-Key': idempotencyKey } : undefined, dataSchema: z.null() }),
};
