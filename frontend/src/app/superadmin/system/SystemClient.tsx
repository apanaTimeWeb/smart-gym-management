// RESPONSIBILITY: SystemClient.tsx handles the logic and UI for its corresponding feature.
'use client';
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tenants.map(tenant => {
            ...
            return (
              <div key={tenant.id} className={`border rounded-xl p-6 ${isOutdated ? 'bg-warning/5 border-orange-500/20' : 'bg-background border-border'}`}>
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="font-semibold text-foreground">{tenant.name}</h3>
                    <p className="text-xs text-disabled mt-1">DB: gym_{tenant.id.replace('-', '_')}</p>
                  </div>
                  <span className={`px-2 py-1 rounded text-xs font-mono font-medium ${isOutdated ? 'bg-warning/20 text-warning' : 'bg-emerald-500/10 text-success'}`}>
                    {tenant.databaseVersion}
                  </span>
                </div>
                
                {isOutdated ? (
                  <button className="w-full flex justify-center items-center gap-2 bg-warning hover:bg-orange-600 text-white py-2 rounded-lg text-sm font-medium transition-colors">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-800">
              {filteredLogs.map(log => (
                <tr key={log.id} className="hover:bg-card/50 transition-colors text-sm">
                  ...
                  <td className="p-4">
                    <p className="text-foreground">{log.actorName || log.actorEmail}</p>
                  ...
                  <td className="p-4 text-secondary max-w-md truncate" title={log.details}>
                    {log.details}
                  </td>
                </tr>
              ))}
              {filteredLogs.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-disabled">
                    No logs found.
                  </td>
                </tr>
              )}
