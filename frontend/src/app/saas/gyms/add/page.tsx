  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-100">Provision New Tenant</h1>
          <p className="text-gray-400 mt-1">This will spin up a completely isolated database for the new gym.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2">
          <form onSubmit={handleSubmit(onSubmit)} className="bg-gray-950 border border-gray-800 rounded-xl p-6 shadow-sm space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-300">Gym Name</label>
                <input 
                  {...register('gymName')}
                  className="w-full bg-gray-900 border border-gray-700 text-gray-100 rounded-lg px-4 py-2 focus:outline-none focus:border-blue-500 transition-colors"
                  placeholder="e.g. Titan Fitness"
                />
                {errors.gymName && <p className="text-red-400 text-xs">{errors.gymName.message}</p>}
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                {errors.temporaryPassword && <p className="text-red-400 text-xs">{errors.temporaryPassword.message}</p>}
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-300">SaaS Plan</label>
                <select 
                  {...register('plan')}
                  className="w-full bg-gray-900 border border-gray-700 text-gray-100 rounded-lg px-4 py-2 focus:outline-none focus:border-blue-500 transition-colors"
                >
                ...
                {errors.plan && <p className="text-red-400 text-xs">{errors.plan.message}</p>}
              </div>
            </div>
        ...
        <div className="bg-gray-950 border border-gray-800 rounded-xl p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-4 text-gray-100 font-bold border-b border-gray-800 pb-4">
            <Database className="w-5 h-5 text-blue-400" />
            Provisioning Console
          </div>
          <div className="bg-black/50 rounded-lg p-4 font-mono text-xs text-gray-400 h-64 overflow-y-auto space-y-2">
            {provisioningLogs.length === 0 ? (
              <p className="text-gray-600 italic">Awaiting submit...</p>
            ) : (
              provisioningLogs.map((log, i) => (
                <p key={i} className="animate-in fade-in slide-in-from-bottom-1 text-emerald-400">
