          ...
          <div>
            ...
            <div className="flex gap-4">
              {[FbIcon, InstaIcon, XIcon, YtIcon].map((Icon, i) => (
                <a key={i} href="#" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:bg-orange-500 hover:text-white transition-all">
                  <Icon />
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {['About Us', 'Membership Plans', 'Trainers', 'Classes Schedule', 'Gallery', 'Contact Us'].map(l => (
                <li key={l}><a href={`#${l.split(' ')[0].toLowerCase()}`} className="text-[var(--landing-text-secondary)] hover:text-orange-400 text-sm transition-colors">{l}</a></li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-3">
              {['Bodybuilding', 'Weight Loss', 'CrossFit', 'Yoga Classes', 'Zumba Dance', 'Personal Training'].map(l => (
                <li key={l}><a href="#services" className="text-[var(--landing-text-secondary)] hover:text-orange-400 text-sm transition-colors">{l}</a></li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <div className="flex bg-[var(--landing-bg-input)] rounded-lg border border-[var(--landing-border)] p-1">
              ...
              <button className="px-4 py-2 bg-white/10 hover:bg-orange-500 text-white text-sm font-medium rounded-md transition-colors">Subscribe</button>
            </div>
          </div>
        </div>
        <div className="pt-8 border-t border-[var(--landing-border)] flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-[var(--landing-text-muted)]">
          ...
          <div className="flex gap-6">
            ...
            <Link href="/dashboard" className="hover:text-white transition-colors flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-orange-500"></span> ERP Portal
            </Link>
          </div>
        </div>
      </div>
