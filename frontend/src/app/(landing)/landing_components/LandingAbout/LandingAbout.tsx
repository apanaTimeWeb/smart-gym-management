...
export default function LandingAbout() {
 return (
 <section id="about" className="py-24 px-4 bg-[var(--landing-bg-dark)]">
 ...
 <h2 className="text-4xl sm:text-5xl font-black text-[var(--landing-text-primary)] mb-6 leading-tight">
 ...
 <h3 className="text-xl font-bold text-[var(--landing-text-primary)] mb-2">Our Mission & Vision</h3>
 <p className="text-[var(--landing-text-secondary)] text-lg leading-relaxed mb-6">
 ...
 {STATS_CARDS.map((s, i) => (
 <div key={i} className="bg-[var(--landing-bg-card)] border border-[var(--landing-border)] rounded-2xl p-6 hover:border-[var(--warning)]/30 transition-all">
 <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${s.color} flex items-center justify-center mb-4`}>
 <s.icon size={22} className="text-white" />
 </div>
 <div className="text-3xl font-black text-[var(--landing-text-primary)] mb-1">{s.value}</div>
 <div className="text-[var(--landing-text-muted)] text-sm">{s.label}</div>
 </div>
 ))}
 </div>
 </div>
