            ...
            <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
              {products.filter(p => p.stock > 0).map(p => (
                <button 
                  ...
                  className="w-full text-left p-3 border border-[var(--store-border)] rounded-xl hover:border-orange-300 dark:hover:border-orange-700 hover:bg-orange-50 dark:hover:bg-orange-900/10 transition-all bg-[var(--store-bg-card)]"
                >
          ...
          <div>
              ...
              {orderItems.map(i => (
                <div key={i.productId} className="flex items-center justify-between p-2 bg-[var(--store-bg-input)] rounded-lg border border-[var(--store-border)]">
                  ...
                  <button 
                    ...
                    className="p-1 text-red-400 hover:text-red-600 dark:hover:text-red-500 transition-colors"
                  >
                    <X size={14} />
                  </button>
                </div>
            ...
            <div className="mt-4 pt-4 border-t border-[var(--store-border)]">
              <div className="flex justify-between mb-3">
                ...
                <span className="font-bold text-lg text-green-700 dark:text-green-500">{formatCurrency(orderTotal)}</span>
              </div>
              <select 
                value={orderMethod} 
                onChange={e => setOrderMethod(e.target.value)} 
                className="w-full border border-[var(--store-border)] rounded-xl px-4 py-2.5 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-orange-400 bg-[var(--store-bg-input)] text-[var(--store-text-primary)]"
              >
