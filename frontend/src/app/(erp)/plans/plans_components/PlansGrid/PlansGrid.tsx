      ...
      {plans.map((p, i) => (
        <div 
          ...
          className={`bg-[var(--plans-bg-card)] rounded-2xl shadow-sm border overflow-hidden relative ${
            i === 1 ? 'border-orange-500 shadow-orange-100/50 dark:shadow-orange-900/20' : 'border-[var(--plans-border)]'
          }`}
        >
          {i === 1 && (
            <div className="bg-orange-500 text-white text-[10px] font-bold uppercase tracking-wider text-center py-1 absolute top-0 w-full left-0">
              Most Popular
            </div>
          )}
          <div className={`p-6 ${i === 1 ? 'pt-8' : ''}`}>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6 bg-[var(--plans-bg-input)] p-4 rounded-xl">
              ...
              <div>
                ...
                <p className="font-bold text-green-600 dark:text-green-500">{formatCurrency(p.price12Month)}</p>
              </div>
            </div>
