            ...
            <tbody className="divide-y divide-[var(--members-border)]">
              {filtered.map(m => (
                <tr key={m.id} className="hover:bg-black/5 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-sm font-medium text-green-700">{formatCurrency(m.paidAmount)}</td>
                  <td className="px-5 py-3.5 text-sm font-medium text-red-600">{m.pendingAmount > 0 ? formatCurrency(m.pendingAmount) : '—'}</td>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => openEdit(m)} className="p-1.5 rounded-lg bg-gray-50 text-gray-600 hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-300" title="Edit"><Edit size={14} /></button>
